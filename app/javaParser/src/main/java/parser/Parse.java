...
public class Parse {
        ...
        for (CompilationUnit cu : compilationUnits) {
            ...
            for (ClassOrInterfaceDeclaration node : nodes) {
                node.getFullyQualifiedName().ifPresent(name -> {
                    ...
                    if (!m.find()) {
                        MyClass myClass = new MyClass(node,cu.getImports(),cu.getRange().get());
    ...
    public Map<String, MyClassDTO> parseClasses(Map<String, MyClass> myClasses) {
        // TODO : Method declarations are being identified elsewhere, consider refactoring it to a visitor and add it here.
            //System.out.println("&&&&& "+myClasses); {pl.edu.wat.wcy.pz.restaurantServer.security.WebSecurityConfiguration=MyClass{qualifiedName='pl.edu.wat.wcy.pz.restaurantServer.security.WebSecurityConfiguration'}, pl.edu.wat.wcy.pz.restaurantServer.entity.Bill=MyClass{qualifiedName='pl.edu.wat.wcy.pz.restaurantServer.entity.Bill'}}
        for (MyClass myClass : myClasses.values()) {
            ...
            List<FieldDeclaration> instance_variables = new ArrayList<>();
        ...
        try {
            ...
            FileUtils.jsonDump(myClassDTOS, Constants.DIRECTORY + "/output.json");
        } catch (IOException e) {
            e.printStackTrace();
        }
