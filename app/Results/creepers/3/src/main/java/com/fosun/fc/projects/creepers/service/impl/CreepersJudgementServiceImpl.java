...
public Page<CreepersJudgementDTO> queryJudgementList(Map<String,Object> searchParams,int pageNumber,int pageSize,String sortType){
    ...
    Page<CreepersJudgementDTO> judgementDTOPage = new PageImpl<CreepersJudgementDTO>(new ArrayList<CreepersJudgementDTO>(judgementDTOList), pageable, judgementPage.getTotalElements());
...
public void processByMerName(String merName){
    ...
    Spider.create(judgementPostRequestProcessor).addPipeline(judgementPipline).setDownloader(new HttpRequestDownloader().setParam(param)).thread(2).addRequest(request).run();
}
