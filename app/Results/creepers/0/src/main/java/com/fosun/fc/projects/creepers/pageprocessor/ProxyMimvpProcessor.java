...
public void process(Page page){
    ...
    for (Selectable eachTr : trList) {
        ...
        String resultXml = CommonMethodUtils.imageAnalytical(url, BaseConstant.ImageAnalyticalType.URL_PATH.getValue(), url);
        String portValue = "";
        try {
            portValue = CommonMethodUtils.getNodeValue(CommonMethodUtils.getChildElement(CommonMethodUtils.getChildElement(CommonMethodUtils.getRootElement(resultXml), "ResultList"), "Item"), "Result");
        } catch (Exception e) {
...
@Override
public Site getSite(){
    site = Site.me().setDomain("proxy.mimvp.com").setRetryTimes(3).setCycleRetryTimes(3).setTimeOut(10000).setSleepTime(CommonMethodUtils.randomSleepTime()).setUserAgent("Mozilla/5.0 (Windows NT 6.1; WOW64; rv:47.0) Gecko/20100101 Firefox/47.0");
    return site;
}


public void main(String[] args){
    Spider.create(new ProxyMimvpProcessor()).thread(2).addUrl("http://proxy.mimvp.com/free.php?proxy=in_tp").addUrl("http://proxy.mimvp.com/free.php?proxy=in_hp").addPipeline(new ResultListToFileModelPipline("/webmagic/")).runAsync();
}
