...
public class PatentProcessor extends BaseCreepersListProcessorimplements PageProcessor{
...
public PatentProcessor() {
    if (null == site) {
        site = Site.me().setDomain("cpquery.sipo.gov.cn").setUserAgent("Mozilla/5.0 (Windows NT 6.1; WOW64; rv:47.0) Gecko/20100101 Firefox/47.0");
    }
}
@Override
public void process(Page page){
    try {
        ...
        for (Selectable eachSelectable : selectableList) {
            ...
            map.put(CreepersConstant.TCreepersPatentColumn.PATENT_TYPE.getValue(), eachSelectable.xpath("//*span[@name=\"record:zhuanlilx\"]/allText()").toString());
            logger.info("申请号/专利号:" + eachSelectable.xpath("//td[2]/allText()"));
            map.put(CreepersConstant.TCreepersPatentColumn.PATENT_NO.getValue(), eachSelectable.xpath("//td[2]/allText()").toString());
            logger.info("发明名称:" + eachSelectable.xpath("//*span[@name=\"record:zhuanlimc\"]/allText()"));
            map.put(CreepersConstant.TCreepersPatentColumn.PATENT_NAME.getValue(), eachSelectable.xpath("//*span[@name=\"record:zhuanlimc\"]/allText()").toString());
            ...
            map.put(CreepersConstant.TCreepersPatentColumn.APPLY_DT.getValue(), eachSelectable.xpath("//span[@name=\"record:shenqingr\"]/@title").toString());
            logger.info("主分类号 :" + eachSelectable.xpath("//span[@name=\"record:zhufenlh\"]/@title"));
            map.put(CreepersConstant.TCreepersPatentColumn.CATEGORY_NO.getValue(), eachSelectable.xpath("//span[@name=\"record:zhufenlh\"]/@title").toString());
            resultList.add(map);
        }
...
public void main(String[] args){
    ...
    /*
         ...
         */
    Spider.create(new PatentProcessor()).addPipeline(new BusinessInfoPipline()).setDownloader(new SeleniumDownloader()).addUrl("http://cpquery.sipo.gov.cn//txnQueryOrdinaryPatents.do?select-key%3Ashenqingh=&select-key%3Azhuanlimc=&select-key%3Ashenqingrxm=" + companyName + "&select-key%3Azhuanlilx=&select-key%3Ashenqingr_from=&select-key%3Ashenqingr_to=&attribute-node:record_start-row=1&attribute-node:record_page-row=1000&").thread(1).runAsync();
}
