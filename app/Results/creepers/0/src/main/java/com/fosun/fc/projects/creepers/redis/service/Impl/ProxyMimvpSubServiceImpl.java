...
public void handleMessage(Serializable message){
    ...
    try {
        Spider.create(proxyMimvpProcessor).addPipeline(proxyMimvpPipline).setDownloader(httpRequestDownloader.setParam(param)).addUrl("http://proxy.mimvp.com/free.php?proxy=in_tp").addUrl("http://proxy.mimvp.com/free.php?proxy=in_hp").thread(1).run();
    } catch (Exception e) {
        e.printStackTrace();
