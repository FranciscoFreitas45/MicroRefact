...
public class TouristBlackListProcessor extends BaseCreepersListProcessorimplements PageProcessor{

 private  String SPILT_COLON;

 private  String TYPE_CONTENT_1;

 private  String URL_TYPE_CONTENT_1;

 private  String TYPE_CONTENT_2;

 private  String URL_TYPE_CONTENT_2;
...
public TouristBlackListProcessor() {
    if (null == site) {
        site = Site.me().setDomain("qualitytourism.cnta.gov.cn").setUserAgent("Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.106 Safari/537.36").setAcceptStatCode(Sets.newHashSet(200, 302)).setSleepTime(5000);
    }
}
@Override
public void process(Page page){
    ...
    if (param.getOrderUrl(BaseConstant.OrderUrlKey.INDEX_URL).equals(page.getUrl().toString())) {
        ...
        try {
            ...
            param.setErrorInfo("TouristBlackListProcessor process error:" + e.getCause().getClass() + e.getCause().getMessage());
        ...
        if (page.getUrl().regex(URL_TYPE_CONTENT_2).match()) {
            ...
            periodDate.append(divSelectable.xpath("//*[@id=\"per_sy\"]/allText()")).append("年").append(divSelectable.xpath("//*[@id=\"per_sm\"]/allText()")).append("月").append(divSelectable.xpath("//*[@id=\"per_sd\"]/allText()")).append("日至").append(divSelectable.xpath("//*[@id=\"per_ey\"]/allText()")).append("年").append(divSelectable.xpath("//*[@id=\"per_em\"]/allText()")).append("月").append(divSelectable.xpath("//*[@id=\"per_ed\"]/allText()")).append("日");
            ...
            // 起止日期
            StringBuffer periodDate = new StringBuffer();
            periodDate.append(divSelectable.xpath("//*[@id=\"cop_sy\"]/allText()")).append("年").append(divSelectable.xpath("//*[@id=\"cop_sm\"]/allText()")).append("月").append(divSelectable.xpath("//*[@id=\"cop_sd\"]/allText()")).append("日至").append(divSelectable.xpath("//*[@id=\"cop_ey\"]/allText()")).append("年").append(divSelectable.xpath("//*[@id=\"cop_em\"]/allText()")).append("月").append(divSelectable.xpath("//*[@id=\"cop_ed\"]/allText()")).append("日");
            period = periodDate.toString();
        }
        // 明细链接
...
public void main(String[] args){
    ...
    Spider.create(new TouristBlackListProcessor()).setDownloader(new HttpRequestDownloader().setParam(param)).addUrl("http://qualitytourism.cnta.gov.cn/tools/submit_ajax.ashx?action=searchbadinfo&type=旅行社&bdate=&edate=&key=").addUrl("http://qualitytourism.cnta.gov.cn/tools/submit_ajax.ashx?action=searchbadinfo&type=导%20游&bdate=&edate=&key=").thread(2).runAsync();
}
