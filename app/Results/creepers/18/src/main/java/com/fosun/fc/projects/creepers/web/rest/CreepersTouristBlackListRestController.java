...
public ResponseEntity<JsonResult<Map>> queryByAgentName(String inputJson,HttpServletRequest request,HttpServletResponse response){
    ...
    if (StringUtils.isNotBlank(param.getAgentName())) {
        jsonResult = creepersListServiceImpl.processByMerName(BaseConstant.TaskListType.TOUR_AGENCY_BLACK_LIST.getValue(), param.getAgentName());
        return new ResponseEntity<JsonResult<Map>>(jsonResult, HttpStatus.OK);
    } else {
        jsonResult = new JsonResult<Map<String, Object>>(JSON_RESULT_TYPE.failure, "旅行社单位名称，不能为空！", new HashMap<String, Object>());
        return new ResponseEntity<JsonResult<Map>>(jsonResult, HttpStatus.OK);
    }
}
...
public ResponseEntity<JsonResult<Map>> queryByGuideNo(String inputJson,HttpServletRequest request,HttpServletResponse response){
    // 用执业证号唯一索引查询都用这个接口
    JsonResult jsonResult = null;
    CreepersTourBlackListDTO param = JSONObject.parseObject(inputJson, CreepersTourBlackListDTO.class);
    if (StringUtils.isNotBlank(param.getGuideNo())) {
        jsonResult = creepersListServiceImpl.processByMerName(BaseConstant.TaskListType.TOUR_BLACK_LIST.getValue(), param.getGuideNo());
        return new ResponseEntity<JsonResult<Map>>(jsonResult, HttpStatus.OK);
    } else {
        jsonResult = new JsonResult<Map<String, Object>>(JSON_RESULT_TYPE.failure, "执业证号，不能为空！", new HashMap<String, Object>());
        return new ResponseEntity<JsonResult<Map>>(jsonResult, HttpStatus.OK);
    }
}
