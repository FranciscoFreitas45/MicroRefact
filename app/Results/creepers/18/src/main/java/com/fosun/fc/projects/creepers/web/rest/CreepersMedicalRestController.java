...
public ResponseEntity<JsonResult<Map>> queryMedicalGMPBykey(String inputJson,HttpServletRequest request,HttpServletResponse response){
    ...
    if (StringUtils.isNotBlank(param.getKey())) {
        ...
        jsonResult = new JsonResult<Map<String, Object>>(JSON_RESULT_TYPE.failure, "key(唯一标识号)，不能为空！", new HashMap<String, Object>());
        return new ResponseEntity<JsonResult<Map>>(jsonResult, HttpStatus.OK);
    }
}
...
public ResponseEntity<JsonResult<Map>> queryMedicalGSPBykey(String inputJson,HttpServletRequest request,HttpServletResponse response){
    JsonResult jsonResult = null;
    CreepersMedicalDTO param = JSONObject.parseObject(inputJson, CreepersMedicalDTO.class);
    if (StringUtils.isNotBlank(param.getKey())) {
...
public ResponseEntity<JsonResult<Map>> queryMedicalInstrumentForeignBykey(String inputJson,HttpServletRequest request,HttpServletResponse response){
    JsonResult jsonResult = null;
    CreepersMedicalDTO param = JSONObject.parseObject(inputJson, CreepersMedicalDTO.class);
    if (StringUtils.isNotBlank(param.getKey())) {
...
public ResponseEntity<JsonResult<Map>> queryMedicalInstrumentDomesticBykey(String inputJson,HttpServletRequest request,HttpServletResponse response){
    JsonResult jsonResult = null;
    CreepersMedicalDTO param = JSONObject.parseObject(inputJson, CreepersMedicalDTO.class);
    if (StringUtils.isNotBlank(param.getKey())) {
