...
public String list(int pageNumber,int pageSize,String sortType,Model model,ServletRequest request){
    ...
    logger.info("========>查询参数：" + searchParams + "<========");
    Page<CreepersListFundDTO> resultList = (Page<CreepersListFundDTO>) creepersListServiceImpl.findList(searchParams, pageNumber, pageSize, sortType, BaseConstant.TaskListType.FUND_LIST.getValue());
    model.addAttribute("resultList", resultList);
...
public String queryInfo(Model model,String userCode){
    ...
    model.addAttribute("basicResult", resultMap.get(CreepersConstant.TableNamesFund.T_CREEPERS_FUND_BASIC.getMapKey()));
    model.addAttribute("basicDetailResultList", resultMap.get(CreepersConstant.TableNamesFund.T_CREEPERS_FUND_BASIC_DETAIL.getMapKey()));
    model.addAttribute("extraResult", resultMap.get(CreepersConstant.TableNamesFund.T_CREEPERS_FUND_EXTRA.getMapKey()));
    model.addAttribute("extraDetailResultList", resultMap.get(CreepersConstant.TableNamesFund.T_CREEPERS_FUND_EXTRA_DETAIL.getMapKey()));
