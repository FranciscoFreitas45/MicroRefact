...
public ResponseEntity<JsonResult<Map>> queryByAccount(String inputJson,HttpServletRequest request,HttpServletResponse response){
    ...
    if (StringUtils.isNotBlank(param.getGuideNo()) || StringUtils.isNotBlank(param.getCardNo())) {
        ...
        loginParam.putSearchKeyWord(CreepersConstant.TCreepersTourGuideColumn.GUIDE_NO.getValue(), param.getGuideNo());
        loginParam.putSearchKeyWord(CreepersConstant.TCreepersTourGuideColumn.CARD_NO.getValue(), param.getCardNo());
        loginParam.putSearchKeyWord(CreepersConstant.TCreepersTourGuideColumn.CERT_NO.getValue(), param.getCertNo());
        ...
        jsonResult = new JsonResult<Map<String, Object>>(JSON_RESULT_TYPE.failure, "导游证号，导游卡号，至少需要录入一个！", new HashMap<String, Object>());
        return new ResponseEntity<JsonResult<Map>>(jsonResult, HttpStatus.OK);
    }
}
