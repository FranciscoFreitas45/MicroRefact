...
public String list(int pageNumber,int pageSize,String sortType,Model model,ServletRequest request){
    ...
    logger.info("========>查询参数：" + searchParams + "<========");
    Page<CreepersListInsuranceDTO> resultList = (Page<CreepersListInsuranceDTO>) creepersListServiceImpl.findList(searchParams, pageNumber, pageSize, sortType, BaseConstant.TaskListType.INSURANCE_LIST.getValue());
    model.addAttribute("resultList", resultList);
...
public String queryInfo(Model model,String userCode){
    ...
    model.addAttribute("basicResultList", resultMap.get(CreepersConstant.TableNamesInsurance.T_CREEPERS_INSURANCE_BASIC.getMapKey()));
    model.addAttribute("paymentResultList", resultMap.get(CreepersConstant.TableNamesInsurance.T_CREEPERS_INSURANCE_PAYMENT.getMapKey()));
    model.addAttribute("unitResultList", resultMap.get(CreepersConstant.TableNamesInsurance.T_CREEPERS_INSURANCE_UNIT.getMapKey()));
    model.addAttribute("sumResultList", resultMap.get(CreepersConstant.TableNamesInsurance.T_CREEPERS_INSURANCE_SUM.getMapKey()));
