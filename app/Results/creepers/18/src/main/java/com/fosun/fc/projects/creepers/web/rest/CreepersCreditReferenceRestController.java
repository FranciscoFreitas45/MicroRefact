...
public ResponseEntity<JsonResult<Map>> queryByAccount(String inputJson,HttpServletRequest request,HttpServletResponse response){
    ...
    if (StringUtils.isNotBlank(param.getLoginName()) && StringUtils.isNotBlank(param.getPassword()) && StringUtils.isNotBlank(param.getMessageCaptchaValue())) {
        ...
        jsonResult = new JsonResult<Map<String, Object>>(JSON_RESULT_TYPE.failure, "账号，密码，验证码均不能为空！", new HashMap<String, Object>());
        return new ResponseEntity<JsonResult<Map>>(jsonResult, HttpStatus.OK);
    }
}
