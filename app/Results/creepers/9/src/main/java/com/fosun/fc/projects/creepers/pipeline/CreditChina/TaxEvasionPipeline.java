...
public void process(ResultItems resultItems,Task task){
    ...
    try {
        // 获取爬虫解析的数据
        List<Map<String, Object>> taxEvasionMapList = resultItems.get(CreepersConstant.TableNamesCreditChina.T_CREEPERS_TAX_EVASION.getMapKey());
        if (!CommonMethodUtils.isEmpty(taxEvasionMapList)) {
            // map转entity
            List<TCreepersTaxEvasion> entityList = CommonMethodUtils.mapList(taxEvasionMapList, new TCreepersTaxEvasion());
        ...
        // 获取爬虫解析的数据
        List<Map<String, Object>> taxEvasionDetailMapList = resultItems.get(CreepersConstant.TableNamesCreditChina.T_CREEPERS_TAX_EVASION_DETAIL.getMapKey());
        if (!CommonMethodUtils.isEmpty(taxEvasionDetailMapList)) {
            // map转entity
            List<TCreepersTaxEvasionDetail> entityList = CommonMethodUtils.mapList(taxEvasionDetailMapList, new TCreepersTaxEvasionDetail());
