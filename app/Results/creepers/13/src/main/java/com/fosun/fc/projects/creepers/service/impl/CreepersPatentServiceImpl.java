...
public Page<CreepersPatentDTO> queryPatentList(Map<String,Object> searchParams,int pageNumber,int pageSize,String sortType){
    ...
    Page<CreepersPatentDTO> resultPage = new PageImpl<CreepersPatentDTO>(new ArrayList<CreepersPatentDTO>(patentDTOList), pageable, patentPage.getTotalElements());
    return resultPage;
}
...
public void processByMerName(String merName){
    ...
    // 启动爬虫
    Spider.create(patentProcessor).addPipeline(patentPipline).setDownloader(new SeleniumDownloader().setParam(param)).thread(10).addUrl("http://cpquery.sipo.gov.cn/txnQueryOrdinaryPatents.do?select-key%3Ashenqingh=&select-key%3Azhuanlimc=&select-key%3Ashenqingrxm=" + merName + "&select-key%3Azhuanlilx=&select-key%3Ashenqingr_from=&select-key%3Ashenqingr_to=&attribute-node:record_start-row=1&attribute-node:record_page-row=1000&").run();
}
