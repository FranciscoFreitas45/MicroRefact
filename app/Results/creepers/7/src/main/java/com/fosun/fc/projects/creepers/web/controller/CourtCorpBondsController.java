...
@Controller
@RequestMapping(value = "/courtCorpBonds")
public class CourtCorpBondsController extends BaseController{
...
public String init(){
    logger.info("========>CourtCorpBondsController-->init<========");
    return "courtCorpBonds/courtCorpBondsList";
}
...
public String list(int pageNumber,int pageSize,String sortType,Model model,String name){
    ...
    logger.info("========>CourtCorpBondsController-->list<========");
    logger.info("========>查询参数：" + searchParams + "<========");
    Page<CreepersCourtCorpBondsDTO> resultList = (Page<CreepersCourtCorpBondsDTO>) creepersCourtCorpBondsServiceImpl.findList(searchParams, pageNumber, pageSize, sortType);
