...
public void process(ResultItems resultItems,Task task){
    ...
    try {
        // 获取爬虫解析的数据
        List<Map<String, Object>> corpBondsMapList = resultItems.get(CreepersConstant.TableNamesCreditChina.T_CREEPERS_COURT_CORP_BONDS.getMapKey());
        if (!CommonMethodUtils.isEmpty(corpBondsMapList)) {
            // map转entity
            List<TCreepersCourtCorpBonds> entityList = CommonMethodUtils.mapList(corpBondsMapList, new TCreepersCourtCorpBonds());
