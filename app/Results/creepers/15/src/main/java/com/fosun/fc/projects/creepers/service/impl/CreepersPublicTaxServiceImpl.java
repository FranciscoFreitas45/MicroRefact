...
public Map<String,Object> findByRptNoForMap(String rptNo){
    ...
    if ((!CreepersConstant.TableNamesCreditReference.T_CREEPERS_PUBLIC_TAX.isList()) && dtoList != null && dtoList.size() > 0) {
        result.put(CreepersConstant.TableNamesCreditReference.T_CREEPERS_PUBLIC_TAX.getMapKey(), dtoList.get(0));
    } else if (CreepersConstant.TableNamesCreditReference.T_CREEPERS_PUBLIC_TAX.isList() && dtoList != null && dtoList.size() > 0) {
