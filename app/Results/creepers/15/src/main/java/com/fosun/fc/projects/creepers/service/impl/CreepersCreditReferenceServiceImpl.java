...
public void processByParam(CreepersLoginParamDTO param){
    ...
    // 启动爬虫
    logger.info("=============>启动爬虫!");
    Spider.create(creditReferenceCenterProcessor).addPipeline(creditReferencePipline).addPipeline(creditReferenceUploadFilePipline).setDownloader(new CreditReferenceSeleniumDownloader().setParam(param)).thread(1).addUrl(param.getOrderUrl(BaseConstant.OrderUrlKey.INDEX_URL)).run();
