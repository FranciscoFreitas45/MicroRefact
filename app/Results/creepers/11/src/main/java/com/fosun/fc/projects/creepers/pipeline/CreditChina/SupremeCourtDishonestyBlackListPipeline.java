...
public void process(ResultItems resultItems,Task task){
    ...
    try {
        // 获取爬虫解析的数据
        List<Map<String, Object>> dishonestyMapList = resultItems.get(CreepersConstant.TableNamesCreditChina.T_CREEPERS_COURT_DISHONEST.getMapKey());
        List<Map<String, Object>> disInfoMapList = resultItems.get(CreepersConstant.TableNamesCreditChina.T_CREEPERS_COURT_DIS_INFO.getMapKey());
        if (!CommonMethodUtils.isEmpty(dishonestyMapList)) {
            // map转entity
            List<TCreepersCourtDishonest> entityList = CommonMethodUtils.mapList(dishonestyMapList, new TCreepersCourtDishonest());
        ...
        if (!CommonMethodUtils.isEmpty(disInfoMapList)) {
            // map转entity
            List<TCreepersCourtDisInfo> entityList = CommonMethodUtils.mapList(disInfoMapList, new TCreepersCourtDisInfo());
