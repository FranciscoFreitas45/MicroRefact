...
public class MedicalInstrumentDomesticProcessor implements PageProcessor{

 private  String URL_PAGE_TEMPLATE;

 private  String REGEX_URL_PAGE;

 private  String REGEX_URL_DETAIL;
...
public MedicalInstrumentDomesticProcessor() {
    if (null == site) {
        site = Site.me().setDomain("qy1.sfda.gov.cn").setUserAgent("Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.106 Safari/537.36").setSleepTime(3100).setTimeOut(90000);
    }
}
@Override
public void process(Page page){
    ...
    if (page.getUrl().toString().equals(param.getOrderUrl(BaseConstant.OrderUrlKey.INDEX_URL))) {
        ...
        for (Selectable td : tdList) {
            ...
            String detailURL = "http://qy1.sfda.gov.cn/datasearch/face3/content.jsp?tableId=26&tableName=TABLE26&tableView=国产器械&Id=" + td.regex(";Id=(.*)',null");
...
public void main(String[] args){
    ...
    Spider.create(new MedicalInstrumentDomesticProcessor()).setDownloader(new HttpRequestDownloader().setParam(param)).addRequest(request).thread(1).runAsync();
}
