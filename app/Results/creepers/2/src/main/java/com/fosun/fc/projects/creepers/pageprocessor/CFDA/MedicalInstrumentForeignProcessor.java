...
public MedicalInstrumentForeignProcessor() {
    if (null == site) {
        site = Site.me().setDomain("qy1.sfda.gov.cn").setUserAgent("Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.106 Safari/537.36").setSleepTime(1800).setTimeOut(90000);
    }
}
@Override
public void process(Page page){
    ...
    if (page.getUrl().toString().equals(param.getOrderUrl(BaseConstant.OrderUrlKey.INDEX_URL))) {
        ...
        for (Selectable td : tdList) {
            ...
            String detailURL = "http://qy1.sfda.gov.cn/datasearch/face3/content.jsp?tableId=27&tableName=TABLE27&tableView=进口器械&Id=" + td.regex(";Id=(.*)',null");
...
public void main(String[] args){
    ...
    Spider.create(new MedicalInstrumentForeignProcessor()).setDownloader(new HttpRequestDownloader().setParam(param)).addRequest(request).thread(1).runAsync();
}
