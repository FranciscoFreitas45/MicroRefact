...
public class TouristGuideProcessor extends BaseCreepersListProcessorimplements PageProcessor{

 private  String SPILT_COLON;

 private  String REGEX_URL_ERROR_CAPTCHA;

 private  String REGEX_URL_INFO_NOT_EXIST;

 private  String REGEX_CONTENT_COLSPAN;

 private  String REGEX_CONTENT_BASE_INFO;

 private  String REGEX_CONTENT_GUIDE_NO;

 private  String REGEX_CONTENT_SEX;

 private  String REGEX_CONTENT_QULIFY_NO;

 private  String REGEX_CONTENT_GUIDE_LEVEL;

 private  String REGEX_CONTENT_CARD_NO;

 private  String REGEX_CONTENT_EDUCATION;

 private  String REGEX_CONTENT_CERT_NO;

 private  String REGEX_CONTENT_LAN;

 private  String REGEX_CONTENT_AREA;

 private  String REGEX_CONTENT_PEOPLE;

 private  String REGEX_CONTENT_CERT_DT;

 private  String REGEX_CONTENT_SCORE;

 private  String REGEX_CONTENT_PUBLISH_DT;

 private  String REGEX_CONTENT_PUBLISH_TYPE;

 private  String REGEX_CONTENT_AGENT_NAME;

 private  String REGEX_CONTENT_PHONE;

 private  String REGEX_CONTENT_ERROR_CAPTCHA;

 private  String REGEX_CONTENT_INFO_NOT_EXIST;

 private  String REGEX_CLASS_TD_LEFT_5;

 private  String REGEX_STYLE_BACKGROUND_IMAGE_URL;

 private  String XPATH_CLASS_TD_00;

 private  String XPATH_ALLTEXT_FONT;

 private  String XPATH_ALLTEXT;

 private  String XPATH_DIV_CENTER;
...
public TouristGuideProcessor() {
    if (null == site) {
        site = Site.me().setDomain("daoyou-chaxun.cnta.gov.cn").setUserAgent("Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.106 Safari/537.36").setAcceptStatCode(Sets.newHashSet(200, 302)).setSleepTime(5000);
    }
}
@Override
public void process(Page page){
    ...
    if (page.getUrl().toString().equals(param.getOrderUrl(BaseConstant.OrderUrlKey.INDEX_URL))) {
        ...
        page.setSkip(true);
    } else if (page.getUrl().toString().equals(REGEX_URL_ERROR_CAPTCHA) && page.getHtml().xpath(XPATH_DIV_CENTER).regex(REGEX_CONTENT_ERROR_CAPTCHA).match()) {
        ...
        page.addTargetRequest(param.getOrderUrl(BaseConstant.OrderUrlKey.CAPTCHA_URL));
        page.setSkip(true);
    } else if (page.getUrl().toString().equals(REGEX_URL_INFO_NOT_EXIST) && page.getHtml().xpath(XPATH_DIV_CENTER).regex(REGEX_CONTENT_INFO_NOT_EXIST).match()) {
        page.putField(CreepersConstant.TableNamesOthers.T_CREEPERS_TOUR_GUIDE.getMapKey(), new HashMap<String, String>());
    } else {
        ...
        try {
            ...
            for (Selectable eachTd : tdList) {
                ...
                if (eachTd.regex(REGEX_STYLE_BACKGROUND_IMAGE_URL).match()) {
                ...
                } else if (eachTd.regex(REGEX_CONTENT_BASE_INFO).match() || eachTd.regex(REGEX_CLASS_TD_LEFT_5).match() || eachTd.regex(REGEX_CONTENT_COLSPAN).match()) {
                    continue;
                    ...
                    logger.info(CreepersConstant.TCreepersTourGuideColumn.GUIDE_NO.getValue() + SPILT_COLON + currentContent);
                    ...
                    logger.info(CreepersConstant.TCreepersTourGuideColumn.SEX.getValue() + SPILT_COLON + currentContent);
                    ...
                    logger.info(CreepersConstant.TCreepersTourGuideColumn.QULIFY_NO.getValue() + SPILT_COLON + currentContent);
                    ...
                    logger.info(CreepersConstant.TCreepersTourGuideColumn.GUIDE_LEVEL.getValue() + SPILT_COLON + currentContent);
                    ...
                    logger.info(CreepersConstant.TCreepersTourGuideColumn.CARD_NO.getValue() + SPILT_COLON + currentContent);
                    ...
                    logger.info(CreepersConstant.TCreepersTourGuideColumn.EDUCATION.getValue() + SPILT_COLON + currentContent);
                    ...
                    logger.info(CreepersConstant.TCreepersTourGuideColumn.CERT_NO.getValue() + SPILT_COLON + currentContent);
                    ...
                    logger.info(CreepersConstant.TCreepersTourGuideColumn.LAN.getValue() + SPILT_COLON + currentContent);
                    ...
                    logger.info(CreepersConstant.TCreepersTourGuideColumn.AREA.getValue() + SPILT_COLON + currentContent);
                    ...
                    logger.info(CreepersConstant.TCreepersTourGuideColumn.PEOPLE.getValue() + SPILT_COLON + currentContent);
                    ...
                    logger.info(CreepersConstant.TCreepersTourGuideColumn.CERT_DT.getValue() + SPILT_COLON + currentContent);
                    ...
                    logger.info(CreepersConstant.TCreepersTourGuideColumn.SCORE.getValue() + SPILT_COLON + currentContent);
                    ...
                    logger.info(CreepersConstant.TCreepersTourGuideColumn.PUBLISH_DT.getValue() + SPILT_COLON + currentContent);
                    ...
                    logger.info(CreepersConstant.TCreepersTourGuideColumn.PUBLISH_TYPE.getValue() + SPILT_COLON + currentContent);
                    ...
                    logger.info(CreepersConstant.TCreepersTourGuideColumn.AGENT_NAME.getValue() + SPILT_COLON + currentContent);
                    ...
                    logger.info(CreepersConstant.TCreepersTourGuideColumn.PHONE.getValue() + SPILT_COLON + currentContent);
                }
            }
            ...
            param.setErrorInfo("TouristGuideProcessor process error:" + e.getCause().getClass() + e.getCause().getMessage());
            param.setErrorPath(getClass().toString());
            if (StringUtils.isNotBlank(param.getSearchKeyWord(CreepersConstant.TCreepersListTourGuideColumn.GUIDE_NO.getValue()))) {
                param.setLoginName(param.getSearchKeyWord(CreepersConstant.TCreepersListTourGuideColumn.GUIDE_NO.getValue()));
            } else {
                param.setLoginName(param.getSearchKeyWord(CreepersConstant.TCreepersListTourGuideColumn.CARD_NO.getValue()));
            }
            creepersExceptionHandleServiceImpl.handleExceptionAndPrintLog(param);
...
public void main(String[] args){
    ...
    param.putOrderUrl(BaseConstant.OrderUrlKey.INDEX_URL, "http://daoyou-chaxun.cnta.gov.cn/single_info/selectlogin_1.asp?11");
    param.putOrderUrl(BaseConstant.OrderUrlKey.CAPTCHA_URL, "http://daoyou-chaxun.cnta.gov.cn/single_info/validatecode.asp");
    param.putOrderUrl(BaseConstant.OrderUrlKey.DO_LOGIN_URL, "http://daoyou-chaxun.cnta.gov.cn/single_info/selectlogin_1.asp");
    param.setTaskType(BaseConstant.TaskListType.TOUR_GUIDE_LIST.getValue());
    param.putNameValuePair("text_dyzh", param.getSearchKeyWord(CreepersConstant.TCreepersTourGuideColumn.GUIDE_NO.getValue()));
    param.putNameValuePair("text_dykh", param.getSearchKeyWord(CreepersConstant.TCreepersTourGuideColumn.CARD_NO.getValue()));
    param.putNameValuePair("text_dysfzh", param.getSearchKeyWord(CreepersConstant.TCreepersTourGuideColumn.CERT_NO.getValue()));
    ...
    Spider.create(new TouristGuideProcessor()).setDownloader(new SimulateLoginDownloader().setParam(param)).addRequest(request).thread(1).runAsync();
}
