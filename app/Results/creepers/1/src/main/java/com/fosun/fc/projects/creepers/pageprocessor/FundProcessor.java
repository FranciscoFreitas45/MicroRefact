...
@Autowired
 ...
 private  String REGEX_TAB_BASIC;

 private  String REGEX_TAB_BASIC_DETAIL;

 private  String REGEX_TAB_EXTRA;

 private  String REGEX_TAB_EXTRA_DETAIL;

 private  String REGEX_HTTPS_COOKIES;

 private  String REGEX_HTTP_COOKIES;

 private  String REGEX_HTTP_CROSS_DOMAIN_SETCOOKIE;

 private  String REGEX_HTTP_WSWQ_COOKIES;

 private  String XPATH_FUND_TABLE_BY_COLOR;

 private  String XPATH_FUND_TABLE_DETAIL_BY_COLOR;

 private  String XPATH_TR_BY_COLOR;

 private  String XPATH_ERROR_TIP;

 private  String XPATH_TBODY;

 private  String XPATH_ALLTEXT;

 private  String XPATH_TEXT;

 private  String XPATH_TR;

 private  String XPATH_TD;

 private  String XPATH_TD_1;

 private  String XPATH_TD_2;

 private  String XPATH_TD_3;

 private  String XPATH_TD_4;

 private  String XPATH_TD_5;

 private  String XPATH_TD_6;

 private  String XPATH_TD_7;

 private  String XPATH_TD_8;

 private  String XPATH_TD_9;

 private  String XPATH_TD_10;

 private  String CONTENT_NAME;

 private  String CONTENT_ACCOUNT_DT;

 private  String CONTENT_ACCOUNT_NO;

 private  String CONTENT_UNIT;

 private  String CONTENT_END_DT;

 private  String CONTENT_SUM_AMOUNT;

 private  String CONTENT_MONTHLY_AMOUNT;

 private  String CONTENT_ACCOUNT_STATUS;

 private  String CONTENT_MOBILE;

 private  String CONTENT_CERTIFICATE_STATUS;

 private  String CONTENT_EMPTY;

 private  String SPILT_COLON;

public FundProcessor() {
    if (null == site) {
        site = Site.me().setDomain("persons.shgjj.com").setUserAgent("Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/53.0.2785.21 Safari/537.36").setAcceptStatCode(Sets.newHashSet(200, 302)).setRetryTimes(5);
    }
}
@Override
    ...
    try {
        ...
        if (param.getOrderUrl(BaseConstant.OrderUrlKey.INDEX_URL).equals(page.getUrl().toString())) {
            ...
            if (page.getHtml().xpath("//*[@id=\"myform\"]").match()) {
                ...
                page.addTargetRequest(request);
            } else if (page.getHtml().xpath(XPATH_ERROR_TIP + XPATH_ALLTEXT).toString().contains("验证码") || page.getHtml().xpath(XPATH_ERROR_TIP + XPATH_ALLTEXT).toString().equals("")) {
                logger.error(// 
            ...
            page.setSkip(true);
        } else if ("http://bbs.shgjj.com/sso/sso.php?url=https://persons.shgjj.com/MainServlet?ID=1".equals(page.getUrl().toString())) {
            ...
            if (REGEX_TAB_BASIC.equals(page.getUrl().toString())) {
                ...
                if (page.getHtml().xpath(XPATH_FUND_TABLE_DETAIL_BY_COLOR + XPATH_TBODY + XPATH_TR_BY_COLOR).nodes().size() > 0)
...
public void methodFundBasicDetail(Page page){
    ResultItems resultItems = page.getResultItems();
    CreepersLoginParamDTO param = resultItems.get(BaseConstant.PARAM_DTO_KEY);
    List<Selectable> trList = page.getHtml().xpath(XPATH_FUND_TABLE_DETAIL_BY_COLOR + XPATH_TBODY + XPATH_TR_BY_COLOR).nodes();
    List<Map<String, String>> mapList = resultItems.get(CreepersConstant.TableNamesFund.T_CREEPERS_FUND_BASIC_DETAIL.getMapKey());
    for (Selectable trSel : trList) {
        ...
        logger.info(CreepersConstant.TCreepersFundBasicDetailColumn.LOGIN_NAME.getValue() + SPILT_COLON + param.getLoginName());
        // 日期
        ...
        logger.info(CreepersConstant.TCreepersFundBasicDetailColumn.OPERATION_DT.getValue() + SPILT_COLON + operationDt);
        // 单位名称
        ...
        logger.info(CreepersConstant.TCreepersFundBasicDetailColumn.OPERATION_DESC.getValue() + SPILT_COLON + operationDesc);
        // 业务原因
        ...
        logger.info(CreepersConstant.TCreepersFundBasicDetailColumn.OPERATION_REASON.getValue() + SPILT_COLON + operationReason);
        mapList.add(map);
    }
}
...
public void initPageUrl(Page page){
...
public void methodFundBasic(Page page){
    ResultItems resultItems = page.getResultItems();
    CreepersLoginParamDTO param = resultItems.get(BaseConstant.PARAM_DTO_KEY);
    ...
    logger.info(CreepersConstant.TCreepersFundBasicColumn.LOGIN_NAME.getValue() + SPILT_COLON + param.getLoginName());
    // 密码
    ...
    for (Selectable tdSel : tdList) {
        ...
        if (CONTENT_NAME.equals(currentContent)) {
            ...
            // 姓名
            map.put(CreepersConstant.TCreepersFundBasicColumn.NAME.getValue(), currentContent.substring(0, currentContent.indexOf(">")));
            logger.info(CreepersConstant.TCreepersFundBasicColumn.NAME.getValue() + SPILT_COLON + currentContent.substring(0, currentContent.indexOf(">")));
            ...
            logger.info(CreepersConstant.TCreepersFundBasicColumn.ACCOUNT_DT.getValue() + SPILT_COLON + currentContent);
            ...
            logger.info(CreepersConstant.TCreepersFundBasicColumn.ACCOUNT_NO.getValue() + SPILT_COLON + currentContent);
            ...
            lastContent = CONTENT_EMPTY;
            // 账户余额
            map.put(CreepersConstant.TCreepersFundBasicColumn.SUM_AMOUNT.getValue(), currentContent.substring(0, currentContent.indexOf("元")));
            logger.info(CreepersConstant.TCreepersFundBasicColumn.SUM_AMOUNT.getValue() + SPILT_COLON + currentContent.substring(0, currentContent.indexOf("元")));
            ...
            lastContent = CONTENT_EMPTY;
            // 月缴存额
            map.put(CreepersConstant.TCreepersFundBasicColumn.MONTHLY_AMOUNT.getValue(), currentContent.substring(0, currentContent.indexOf("元")));
            logger.info(CreepersConstant.TCreepersFundBasicColumn.MONTHLY_AMOUNT.getValue() + SPILT_COLON + currentContent.substring(0, currentContent.indexOf("元")));
            ...
            logger.info(CreepersConstant.TCreepersFundBasicColumn.ACCOUNT_STATUS.getValue() + SPILT_COLON + currentContent);
            ...
            lastContent = CONTENT_EMPTY;
            // 绑定手机号
            map.put(CreepersConstant.TCreepersFundBasicColumn.MOBILE.getValue(), currentContent.substring(0, currentContent.indexOf("【")));
            logger.info(CreepersConstant.TCreepersFundBasicColumn.MOBILE.getValue() + SPILT_COLON + currentContent.substring(0, currentContent.indexOf("【")));
            ...
            logger.info(CreepersConstant.TCreepersFundBasicColumn.CERTIFICATE_STATUS.getValue() + SPILT_COLON + currentContent);
        }
    }
}
...
public void main(String[] args){
    ...
    Spider.create(new FundProcessor()).setDownloader(new SimulateLoginDownloader().setParam(param)).addPipeline(new FundPipline()).addRequest(request).thread(1).run();
// String str = "李占平 >>>住房公积金本年度账户明细";
// System.out.println(str.substring(0, str.indexOf(" ")));
}


public void methodFundExtra(Page page){
    ResultItems resultItems = page.getResultItems();
    CreepersLoginParamDTO param = resultItems.get(BaseConstant.PARAM_DTO_KEY);
    List<Selectable> tdList = page.getHtml().xpath(XPATH_FUND_TABLE_BY_COLOR + XPATH_TBODY + XPATH_TD).nodes();
    ...
    logger.info(CreepersConstant.TCreepersFundExtraColumn.LOGIN_NAME.getValue() + SPILT_COLON + param.getLoginName());
    String lastContent = CONTENT_EMPTY;
    for (Selectable tdSel : tdList) {
        if (tdSel.regex("colspan=\"2\"").match())
            continue;
        ...
        if (CONTENT_NAME.equals(currentContent)) {
            ...
            lastContent = CONTENT_EMPTY;
            // 姓名
            map.put(CreepersConstant.TCreepersFundExtraColumn.NAME.getValue(), currentContent.substring(0, currentContent.indexOf(">")));
            logger.info(CreepersConstant.TCreepersFundExtraColumn.NAME.getValue() + SPILT_COLON + currentContent.substring(0, currentContent.indexOf(">")));
        } else if (CONTENT_ACCOUNT_DT.equals(currentContent)) {
            lastContent = CONTENT_ACCOUNT_DT;
            continue;
        } else if (CONTENT_ACCOUNT_DT.equals(lastContent)) {
            ...
            logger.info(CreepersConstant.TCreepersFundExtraColumn.ACCOUNT_DT.getValue() + SPILT_COLON + currentContent);
        } else if (CONTENT_ACCOUNT_NO.equals(currentContent)) {
            lastContent = CONTENT_ACCOUNT_NO;
            continue;
        } else if (CONTENT_ACCOUNT_NO.equals(lastContent)) {
            ...
            logger.info(CreepersConstant.TCreepersFundExtraColumn.ACCOUNT_NO.getValue() + SPILT_COLON + currentContent);
        } else if (CONTENT_UNIT.equals(currentContent)) {
            lastContent = CONTENT_UNIT;
            continue;
        } else if (CONTENT_UNIT.equals(lastContent)) {
            ...
            lastContent = CONTENT_EMPTY;
            // 账号余额
            map.put(CreepersConstant.TCreepersFundExtraColumn.SUM_AMOUNT.getValue(), currentContent.substring(0, currentContent.indexOf("元")));
            logger.info(CreepersConstant.TCreepersFundExtraColumn.SUM_AMOUNT.getValue() + SPILT_COLON + currentContent.substring(0, currentContent.indexOf("元")));
        } else if (CONTENT_MONTHLY_AMOUNT.equals(currentContent)) {
            lastContent = CONTENT_MONTHLY_AMOUNT;
            continue;
        } else if (CONTENT_MONTHLY_AMOUNT.equals(lastContent)) {
            lastContent = CONTENT_EMPTY;
            // 月缴存额
            map.put(CreepersConstant.TCreepersFundExtraColumn.MONTHLY_AMOUNT.getValue(), currentContent.substring(0, currentContent.indexOf("元")));
            logger.info(CreepersConstant.TCreepersFundExtraColumn.MONTHLY_AMOUNT.getValue() + SPILT_COLON + currentContent.substring(0, currentContent.indexOf("元")));
        } else if (CONTENT_ACCOUNT_STATUS.equals(currentContent)) {
            lastContent = CONTENT_ACCOUNT_STATUS;
            continue;
        } else if (CONTENT_ACCOUNT_STATUS.equals(lastContent)) {
            ...
            logger.info(CreepersConstant.TCreepersFundExtraColumn.ACCOUNT_STATUS.getValue() + SPILT_COLON + currentContent);
        }
    }
}


public void methodFundExtraDetail(Page page){
    ResultItems resultItems = page.getResultItems();
    CreepersLoginParamDTO param = resultItems.get(BaseConstant.PARAM_DTO_KEY);
    List<Selectable> trList = page.getHtml().xpath(XPATH_FUND_TABLE_DETAIL_BY_COLOR + XPATH_TBODY + XPATH_TR_BY_COLOR).nodes();
    List<Map<String, String>> mapList = resultItems.get(CreepersConstant.TableNamesFund.T_CREEPERS_FUND_EXTRA_DETAIL.getMapKey());
    for (Selectable trSel : trList) {
        Map<String, String> map = new HashMap<String, String>();
        // 登录名，用于和其他表关联
        map.put(CreepersConstant.TCreepersFundExtraDetailColumn.LOGIN_NAME.getValue(), param.getLoginName());
        logger.info(CreepersConstant.TCreepersFundExtraDetailColumn.LOGIN_NAME.getValue() + SPILT_COLON + param.getLoginName());
        // 日期
        String operationDt = trSel.xpath(XPATH_TD_1 + XPATH_ALLTEXT).toString().trim();
        map.put(CreepersConstant.TCreepersFundExtraDetailColumn.OPERATION_DT.getValue(), operationDt);
        logger.info(CreepersConstant.TCreepersFundExtraDetailColumn.OPERATION_DT.getValue() + SPILT_COLON + operationDt);
        // 单位名称
        String unit = trSel.xpath(XPATH_TD_2 + XPATH_ALLTEXT).toString().trim();
        ...
        logger.info(CreepersConstant.TCreepersFundExtraDetailColumn.OPERATION_DESC.getValue() + SPILT_COLON + operationDesc);
        // 业务原因
        byte[] bytes = { (byte) 0xC2, (byte) 0xA0 };
        String UTFSpace = new String(bytes, "utf-8");
        String operationReason = trSel.xpath(XPATH_TD_5 + XPATH_ALLTEXT).toString().replace(UTFSpace, "").trim();
        map.put(CreepersConstant.TCreepersFundExtraDetailColumn.OPERATION_REASON.getValue(), operationReason);
        logger.info(CreepersConstant.TCreepersFundExtraDetailColumn.OPERATION_REASON.getValue() + SPILT_COLON + operationReason);
        mapList.add(map);
    }
}
