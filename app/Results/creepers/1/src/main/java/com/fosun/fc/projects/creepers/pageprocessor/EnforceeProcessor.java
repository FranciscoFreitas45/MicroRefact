...
@Override
public Site getSite(){
    site = Site.me().setDomain("12333sh.gov.cn").setUserAgent("Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/53.0.2774.3 Safari/537.36").setSleepTime(5000).setAcceptStatCode(Sets.newHashSet(200, 302)).addHeader("Accept-Encoding", "gzip, deflate, sdch, br").setCharset("GBK");
    return site;
}
...
public void main(String[] args){
    ...
    Spider.create(new EnforceeProcessor()).setDownloader(new SimulateLoginDownloader().setParam(param)).addRequest(request).thread(1).runAsync();
}
