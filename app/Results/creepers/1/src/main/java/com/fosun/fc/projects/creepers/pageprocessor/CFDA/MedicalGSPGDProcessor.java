...
public class MedicalGSPGDProcessor implements PageProcessor{
 ...
 public  String URL_PAGE_TEMPLATE;
...
public void process(Page page){
    ...
    if (page.getUrl().toString().equals(param.getOrderUrl(BaseConstant.OrderUrlKey.INDEX_URL))) {
        ...
        if (CommonMethodUtils.isEmpty(trList)) {
            ...
            logger.info("is match:" + linksSelectable.regex("http://www.sda.gov.cn/directory/web/WS01/images/(.*).doc").match());
            ...
            TranslateMethodUtil.addCommonParamMapMedical(list, CreepersConstant.TCreepersMedicalColumn.TYPE.getValue(), BaseConstant.TaskListType.MEDICAL_GSP_LIST.getValue());
...
public void main(String[] args){
    ...
    Spider.create(new MedicalGSPGDProcessor()).setDownloader(new HttpRequestDownloader().setParam(param)).addRequest(request).thread(1).runAsync();
}
