...
public Page download(Request request,Task task){
    ...
    try {
        ...
        while (isReady) {
            ...
            BufferedImage croppedImage = originalImage.getSubimage(location.getX() + gapX + 15, location.getY() + gapY + 1, size.getWidth() - 15, size.getHeight() - 2);
            ...
            } else if (!isElementExsit(webDriver, "//span[contains(.,\"登录名或密码错误！\")]") && !isElementExsit(webDriver, "//span[contains(.,\"验证码输入错误,请重新输入\")]")) {
                isReady = false;
                logger.info("====>>>验证码正确！");
        ...
        wait.until(new ExpectedCondition<Boolean>() {
            ...
            public Boolean apply(WebDriver webDriver) {
                return webDriver.findElement(By.xpath("//*[@id=\"menu\"]/li[contains(.,\"信息服务 >\")]/ul/li[2]/a")).isDisplayed();
            }
        });
