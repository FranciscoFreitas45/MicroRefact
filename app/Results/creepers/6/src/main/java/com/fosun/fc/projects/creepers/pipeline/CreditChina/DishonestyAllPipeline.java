...
public void process(ResultItems resultItems,Task task){
    ...
    try {
        // 获取爬虫解析的数据
        List<Map<String, Object>> shixinAllMapList = resultItems.get(CreepersConstant.TableNamesCreditChina.T_CREEPERS_SHIXIN_ALL.getMapKey());
        if (!CommonMethodUtils.isEmpty(shixinAllMapList)) {
            // map转entity
            List<TCreepersShixinAll> entityList = CommonMethodUtils.mapList(shixinAllMapList, new TCreepersShixinAll());
