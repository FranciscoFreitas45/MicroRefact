...
@Deprecated
public class TaobaoItemCsvExportAction extends CommonCsvExportAction<TempImportDetailView>{
 ...
 private  Map<String,TempImportDetailView> uniqueIsbnDealt;
...
public String getRowDatas(TempImportDetailView record){
    ...
    String data = String.format(dataFmt, title, book.getBookName(), book.getBookPrice(), book.getBookAuthor(), book.getBookPublisher(), book.getIsbn(), pubDate, content, bookIntro, uniqueIsbn, record.getData());
    return data;
}
...
@Override
public Collection<TempImportDetailView> transformList(List<TempImportDetailView> list){
