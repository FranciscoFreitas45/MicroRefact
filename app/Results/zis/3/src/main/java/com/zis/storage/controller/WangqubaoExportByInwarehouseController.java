...
@Override
public boolean isSkip(StorageIoDetail record,Set<String> uniqueIsbnDealt){
...
public List<StorageIoDetail> queryExportData(HttpServletRequest request){
    ...
    if (batchSelectedItemStr != null) {
        ...
        return this.storageService.findStorageIoDetailByRepoIdAndBatchIdInAndIoDetailTypeAndDetailStatusIn(StorageUtil.getRepoId(), Arrays.asList(batchSelectedItem), IoType.IN.getValue(), Arrays.asList(SUCCESS));
    } else {
