...
@Transactional
public JSONReturn modifyTraining(long id,String name,String description,String start,String end,boolean isInsertAttend,String acctName){
...
public List<TrainingListDto> conversionTrainingItem(List<TeTraining> trainingList){
...
@Transactional
public JSONReturn addTraining(String name,String desc,String start,String end,boolean isInsertAttend,String acctName){
    // TODO Auto-generated method stub
    if (!VerifyUtils.verifyDate(start))
        return JSONReturn.buildFailure("开始日期错误!");
    if (!VerifyUtils.verifyDate(end))
    ...
    trainingDao.save(new TeTraining(name, desc, 0, start, end, isInsertAttend, TrainingState.NOT_STARTED, DateTimeUtil.getCurrentTime(), acctName));
...
public JSONReturn evaluationEmployeesTraining(long id,String note,String acctName){
    ...
    // 修改状态为已评分
    log.setState(TrainLogState.EVALUATION);
    log.setNote(note);
...
public JSONReturn findRecord(long trainingId,String acctName){
    ...
    for (TrainingRecordDto dto : dtoList) dto.setStatus(TrainLogState.findStatus(dto.getState()));
