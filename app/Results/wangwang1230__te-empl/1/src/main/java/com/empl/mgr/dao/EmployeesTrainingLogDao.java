...
public void modifyTrainingRecordStop(long trainId,String note){
    ...
    findSession().createQuery(query).setInteger(0, TrainLogState.FINISH).setString(1, note).setLong(2, trainId).setInteger(3, TrainLogState.START).executeUpdate();
}
...
public TeEmployeesTrainingLog findByEmplTraining(long emplId,long trainingId){
    ...
    return (TeEmployeesTrainingLog) findSession().createQuery(query).setLong(0, emplId).setLong(1, trainingId).uniqueResult();
}
