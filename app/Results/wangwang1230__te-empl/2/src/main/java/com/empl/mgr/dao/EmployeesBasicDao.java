...
public List<SelectDto> findEmplByDeptId(long deptId){
    ...
    return findSession().createQuery(query.toString()).setInteger(0, EmployeesState.EMPL_FORMAL).setLong(1, deptId).list();
}
...
@SuppressWarnings("unchecked")
public List<EmployeesInternshipListDto> findEmployeesList(int type,String value,long dept,long position,int page,int emplType){
    // TODO Auto-generated method stub
    StringBuffer query = new StringBuffer();
    ...
    query.append(type == 2 && StringUtils.isNotEmpty(value) ? " and empl.emIdentity like '%" + value + "%' " : "");
    query.append(" order by empl.emId desc ");
    return findSession().createQuery(query.toString()).setFirstResult((page - 1) * PageConstant.PAGE_LIST).setMaxResults(PageConstant.PAGE_LIST).list();
}
...
public String pack(int type){
    String result = "";
    for (int key : type) result += key + ",";
