...
import org.jugbd.mnet.domain.QDiagnosis.diagnosis;
import org.jugbd.mnet.domain.QDiagnosisData.diagnosisData;
import org.jugbd.mnet.domain.QPatient.patient;
import org.jugbd.mnet.domain.QRegister.register;
@Service
@Transactional
...
public String getLikePattern(String searchTerm){
    return "%" + searchTerm.toLowerCase() + "%";
}


public Condition getCondition(String value){
    return Arrays.asList(Condition.values()).stream().filter(condition -> condition.getLabel().toLowerCase().contains(value.toLowerCase())).findFirst().map(condition -> condition).orElse(null);
}


public void setEstimatedDateOfBirth(Patient patient){
    ...
    LocalDate localDate = LocalDate.now().minusDays(day).minusMonths(month).minusYears(year);
...
public Page findPatientBySearchCmd(PatientSearchCmd searchCmd,Pageable pageable){
    ...
    JPAQuery from = new JPAQuery(em).from(patient);
    ...
    if (StringUtils.isNotEmpty(searchCmd.getPhoneNumber()) || StringUtils.isNotEmpty(searchCmd.getRegisterId()) || StringUtils.isNotEmpty(searchCmd.getDiagnosis())) {
        from = from.leftJoin(patient.registers, register);
        if (StringUtils.isNotEmpty(searchCmd.getDiagnosis())) {
            from = from.join(register.diagnosis, diagnosis).join(diagnosis.burns, diagnosisData);
        }
    }
    long count = from.distinct().where(predicate).count();
    List<Patient> patients = applyPagination(from, pageable).orderBy(patient.name.asc()).distinct().list(patient);
