...
public class C_BPartner_Window_AreaSearchProcess extends JavaProcess{
 ...
 private  BPartnerLocationInfoRepository bpartnerLocationInfoRepo;

 private  GeoLocationDocumentService geoLocationDocumentService;

 private  IBPartnerDAO bpartnersRepo;

 private  ICountryDAO countriesRepo;
...
@NonNull
public DocumentFilter createAreaSearchFilter(I_C_Location location){
...
public String doIt(){
    ...
    getResult().setWebuiViewToOpen(ProcessExecutionResult.WebuiViewToOpen.builder().viewId(view.getViewId().toJson()).target(ProcessExecutionResult.ViewOpenTarget.NewBrowserTab).build());
    return MSG_OK;
}


public IView createViewWithFilter(DocumentFilter filter){
    final WindowId windowId = getWindowId();
    return viewsRepo.createView(CreateViewRequest.builder(windowId).setFilters(DocumentFilterList.of(filter)).build());
}


public GeoLocationDocumentQuery createGeoLocationQuery(I_C_Location location){
    ...
    return GeoLocationDocumentQuery.builder().country(IntegerLookupValue.of(countryId, countryName)).address1(location.getAddress1()).city(location.getCity()).postal(location.getPostal()).distanceInKm(distanceInKm).visitorsAddress(visitorsAddress).build();
}
...
public I_C_Location getSelectedLocationOrFirstAvailable(){
