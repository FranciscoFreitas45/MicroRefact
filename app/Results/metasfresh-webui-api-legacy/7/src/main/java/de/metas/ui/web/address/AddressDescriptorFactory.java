...
public class AddressDescriptorFactory {
 ...
 private  CCache<Integer,AddressDescriptor> cache;

 private  String SYSCONFIG_UsePostalLookup;
...
public DocumentEntityDescriptor createAddressEntityDescriptor(){
    final DocumentEntityDescriptor.Builder addressDescriptor = DocumentEntityDescriptor.builder().setDocumentType(DocumentType.Address, // we have only one descriptor for all addresses
    AddressDescriptor.DocumentTypeId).setCaption(Services.get(IMsgBL.class).getTranslatableMsgText("C_Location_ID")).setDataBinding(new AddressDataBindingDescriptorBuilder()).disableDefaultTableCallouts();
    // 
    // Address1 ... Address4 fields
    addressDescriptor.addField(buildFieldDescriptor(IAddressModel.COLUMNNAME_Address1).setValueClass(String.class).setWidgetType(DocumentFieldWidgetType.Text).setDataBinding(new AddressFieldBinding(IAddressModel.COLUMNNAME_Address1, false, I_C_Location::getAddress1, AddressFieldBinding::writeValue_Address1)));
    // 
    addressDescriptor.addField(buildFieldDescriptor(IAddressModel.COLUMNNAME_Address2).setValueClass(String.class).setWidgetType(DocumentFieldWidgetType.Text).setDataBinding(new AddressFieldBinding(IAddressModel.COLUMNNAME_Address2, false, I_C_Location::getAddress2, AddressFieldBinding::writeValue_Address2)));
    // 
    addressDescriptor.addField(buildFieldDescriptor(IAddressModel.COLUMNNAME_Address3).setValueClass(String.class).setWidgetType(DocumentFieldWidgetType.Text).setDataBinding(new AddressFieldBinding(IAddressModel.COLUMNNAME_Address3, false, I_C_Location::getAddress3, AddressFieldBinding::writeValue_Address3)));
    // 
    addressDescriptor.addField(buildFieldDescriptor(IAddressModel.COLUMNNAME_Address4).setValueClass(String.class).setWidgetType(DocumentFieldWidgetType.Text).setDataBinding(new AddressFieldBinding(IAddressModel.COLUMNNAME_Address4, false, I_C_Location::getAddress4, AddressFieldBinding::writeValue_Address4)));
    // 
    // Postal, City, Region and Country fields
    final boolean usePostalLookup = isUsePostalLookup();
    if (usePostalLookup) {
        final AddressPostalLookupDescriptor postalLookup = AddressPostalLookupDescriptor.builder().countryLookup(AddressCountryLookupDescriptor.newInstance()).build();
        addressDescriptor.addField(buildFieldDescriptor(IAddressModel.COLUMNNAME_C_Postal_ID).setValueClass(IntegerLookupValue.class).setWidgetType(DocumentFieldWidgetType.Lookup).setLookupDescriptorProvider(postalLookup).setDataBinding(new AddressFieldBinding(IAddressModel.COLUMNNAME_C_Postal_ID, false, postalLookup::getLookupValueFromLocation, AddressFieldBinding::writeValue_C_Postal_ID)));
    } else {
        // 
        addressDescriptor.addField(buildFieldDescriptor(IAddressModel.COLUMNNAME_Postal).setValueClass(String.class).setWidgetType(DocumentFieldWidgetType.Text).setDataBinding(new AddressFieldBinding(IAddressModel.COLUMNNAME_Postal, false, I_C_Location::getPostal, AddressFieldBinding::writeValue_Postal)));
        // 
        addressDescriptor.addField(buildFieldDescriptor(IAddressModel.COLUMNNAME_City).setValueClass(String.class).setWidgetType(DocumentFieldWidgetType.Text).setDataBinding(new AddressFieldBinding(IAddressModel.COLUMNNAME_City, false, I_C_Location::getCity, AddressFieldBinding::writeValue_City)));
        // 
        // Region and Country fields
        addressDescriptor.addField(buildFieldDescriptor(IAddressModel.COLUMNNAME_C_Region_ID).setValueClass(IntegerLookupValue.class).setWidgetType(DocumentFieldWidgetType.Lookup).setDisplayLogic("@" + IAddressModel.COLUMNNAME_HasRegion + "/N@=Y").setLookupDescriptorProvider(AddressRegionLookupDescriptor.newInstance()).setDataBinding(new AddressFieldBinding(IAddressModel.COLUMNNAME_C_Region_ID, false, AddressFieldBinding::readValue_C_Region_ID, AddressFieldBinding::writeValue_C_Region_ID)));
        // 
        addressDescriptor.addField(buildFieldDescriptor(IAddressModel.COLUMNNAME_HasRegion).setWidgetType(DocumentFieldWidgetType.YesNo).removeCharacteristic(// internal field (not displayed!)
        Characteristic.PublicField).setDataBinding(AddressFieldBinding.internalField(IAddressModel.COLUMNNAME_HasRegion)));
        // 
        addressDescriptor.addField(buildFieldDescriptor(IAddressModel.COLUMNNAME_C_Country_ID).setValueClass(IntegerLookupValue.class).setWidgetType(DocumentFieldWidgetType.Lookup).setMandatoryLogic(true).setLookupDescriptorProvider(AddressCountryLookupDescriptor.newInstance()).setDataBinding(new AddressFieldBinding(IAddressModel.COLUMNNAME_C_Country_ID, false, AddressFieldBinding::readValue_C_Country_ID, AddressFieldBinding::writeValue_C_Country_ID)).addCallout(AddressCallout::onC_Country_ID));
    }
    // 
    // Build it and return
...
public boolean isUsePostalLookup(){
    // don't use postal lookup by default
    final boolean defaultWhenNotFound = false;
...
public DocumentFieldDescriptor.Builder buildFieldDescriptor(String columnName){
    return DocumentFieldDescriptor.builder(columnName).setCaption(Services.get(IMsgBL.class).translatable(columnName)).setLookupDescriptorProvider_None().setReadonlyLogic(false).setDisplayLogic(true).setMandatoryLogic(false).addCharacteristic(Characteristic.PublicField);
}


public AddressLayout createLayout(DocumentEntityDescriptor addressDescriptor){
    final AddressLayout.Builder layout = AddressLayout.builder();
    addressDescriptor.getFields().stream().filter(fieldDescriptor -> fieldDescriptor.hasCharacteristic(Characteristic.PublicField)).map(fieldDescriptor -> createLayoutElement(fieldDescriptor)).forEach(layoutElement -> layout.addElement(layoutElement));
    return layout.build();
}
...
public AddressDescriptor createAddressDescriptor(){
...
public void onC_Country_ID(ICalloutField calloutField){
