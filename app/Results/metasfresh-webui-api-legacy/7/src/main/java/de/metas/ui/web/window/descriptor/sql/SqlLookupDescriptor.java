...
public LookupDescriptorProvider searchInTable(String lookupTableName){
    return builder().setCtxTableName(// tableName
    null).setCtxColumnName(InterfaceWrapperHelper.getKeyColumnName(lookupTableName)).setDisplayType(DisplayType.Search).setReadOnlyAccess().buildProvider();
}
...
public boolean hasParameters(){
    return !getSqlForFetchingExpression().getParameters().isEmpty() || !getPostQueryPredicate().getParameters().isEmpty();
}
...
@Override
public int hashCode(){
    return Objects.hash(tableName, zoomIntoWindowId, sqlForFetchingExpression, sqlForFetchingLookupByIdExpression, entityTypeIndex, postQueryPredicate, // 
    highVolume, numericKey);
}
...
public LookupDescriptorProvider listByAD_Reference_Value_ID(int AD_Reference_Value_ID){
    Check.assumeGreaterThanZero(AD_Reference_Value_ID, "AD_Reference_Value_ID");
    return builder().setCtxTableName(// tableName
    null).setCtxColumnName(null).setDisplayType(DisplayType.List).setAD_Reference_Value_ID(AD_Reference_Value_ID).setReadOnlyAccess().buildProvider();
}
...
public LookupDescriptorProvider searchByAD_Val_Rule_ID(int AD_Reference_Value_ID,int AD_Val_Rule_ID){
    Check.assumeGreaterThanZero(AD_Reference_Value_ID, "AD_Reference_Value_ID");
    return builder().setCtxTableName(// tableName
    null).setCtxColumnName(null).setAD_Reference_Value_ID(AD_Reference_Value_ID).setAD_Val_Rule_ID(AD_Val_Rule_ID).setDisplayType(DisplayType.Search).setReadOnlyAccess().buildProvider();
}
...
public LookupDescriptorProvider productAttributes(){
    return builder().setCtxTableName(// tableName
    null).setCtxColumnName(I_M_AttributeSetInstance.COLUMNNAME_M_AttributeSetInstance_ID).setDisplayType(DisplayType.PAttribute).setReadOnlyAccess().buildProvider();
}
...
public boolean equals(Object obj){
    ...
    return Objects.equals(tableName, other.tableName) && Objects.equals(zoomIntoWindowId, other.zoomIntoWindowId) && Objects.equals(sqlForFetchingExpression, other.sqlForFetchingExpression) && Objects.equals(sqlForFetchingLookupByIdExpression, other.sqlForFetchingLookupByIdExpression) && entityTypeIndex == other.entityTypeIndex && Objects.equals(postQueryPredicate, other.postQueryPredicate) && highVolume == other.highVolume && numericKey == other.numericKey;
}
...
public String toString(){
    return MoreObjects.toStringHelper(this).omitNullValues().add("tableName", tableName).add("zoomIntoWindowId", zoomIntoWindowId.orElse(null)).add("highVolume", highVolume ? highVolume : null).add("sqlForFetching", sqlForFetchingExpression.toOneLineString()).add("postQueryPredicate", postQueryPredicate == null || postQueryPredicate == INamePairPredicate.NULL ? null : postQueryPredicate).toString();
}
