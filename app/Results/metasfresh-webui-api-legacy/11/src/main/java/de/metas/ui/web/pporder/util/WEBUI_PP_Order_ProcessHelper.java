...
public ProcessPreconditionsResolution checkIssueSourceDefaultPreconditionsApplicable(PPOrderLineRow ppOrderLineRow){
...
public List<I_M_Source_HU> retrieveActiveSourceHus(PPOrderLineRow row){
    final I_PP_Order ppOrder = Services.get(IPPOrderDAO.class).getById(row.getOrderId());
    final MatchingSourceHusQuery query = MatchingSourceHusQuery.builder().productId(row.getProductId()).warehouseId(WarehouseId.ofRepoId(ppOrder.getM_Warehouse_ID())).build();
    return SourceHUsService.get().retrieveMatchingSourceHuMarkers(query).stream().filter(huSource -> X_M_HU.HUSTATUS_Active.equals(huSource.getM_HU().getHUStatus())).collect(ImmutableList.toImmutableList());
}
