...
import com.github.haseoo.courier.utilities.Constants.INFORMATION_PROP;
import com.github.haseoo.courier.utilities.Constants.PARCEL_ID_PROP;
@Slf4j
@Service
@Profile("prod")
...
public void sentNotificationToReceiver(ParcelData parcelData){
    log.info(String.format("Sent to receiver %s id %s pin %s", parcelData.getReceiverContactData().getEmailAddress(), parcelData.getId(), String.valueOf(parcelData.getPin())));
...
public void prepareEmailTemplate(ParcelData parcelData,MailModel mail,Map<String,Object> model){
...
public void sentNotificationToSender(ParcelData parcelData){
    log.info(String.format("Sent to sender %s id %s pin %s", parcelData.getSender().getEmailAddress(), parcelData.getId(), Arrays.toString(parcelData.getPin())));
    MailModel mail = new MailModel();
    mail.setFrom(mailFrom);
...
public void sentReturnNotification(ParcelData parcelData){
    log.info(String.format("Sent return to sender %s id %s pin %s", parcelData.getSender().getEmailAddress(), parcelData.getId(), String.valueOf(parcelData.getPin())));
    MailModel mail = new MailModel();
    mail.setFrom(mailFrom);
    mail.setMailTo(parcelData.getSender().getEmailAddress());
