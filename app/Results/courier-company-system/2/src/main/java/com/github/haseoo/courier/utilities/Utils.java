...
import com.github.haseoo.courier.utilities.Constants.EMAIL_REGEX_EXPRESSION;
...
public boolean isParcelMoveable(ParcelData parcelData){
    ParcelStateType currentState = parcelData.getCurrentState().getState();
    return (currentState == ParcelStateType.IN_MAGAZINE || currentState == ParcelStateType.AT_SENDER) && !parcelData.getDateMoved();
}


public Predicate<T> distinctByKey(Function<? super T,?> keyExtractor){
