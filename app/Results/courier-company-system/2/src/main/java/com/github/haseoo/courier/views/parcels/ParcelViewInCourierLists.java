...
public ParcelViewInCourierLists of(ParcelData parcelData){
    return ParcelViewInCourierLists.builder().id(parcelData.getId()).parcelType(ParcelTypeView.of(parcelData.getParcelType())).deliveryAddress(AddressView.of(parcelData.getDeliveryAddress())).senderAddress(AddressView.of(parcelData.getSenderAddress())).sender(ClientView.ofWithoutParcels(parcelData.getSender())).receiverContactData(ReceiverInfoView.of(parcelData.getReceiverContactData())).expectedDeliveryTime(parcelData.getExpectedCourierArrivalDate()).priority(parcelData.getPriority()).parcelFee(parcelData.getParcelFee()).paid(parcelData.getPaid()).dateMoved(parcelData.getDateMoved()).effectivePrice(parcelData.getEffectivePrice()).magazineAddress(AddressView.of(Objects.requireNonNull(parcelData.getParcelStates().stream().filter(pd -> pd.getState() == IN_MAGAZINE).max(new ParcelStateDataComparator()).orElse(null)).getMagazine().getAddress())).currentState(parcelData.getCurrentState().getState()).build();
}
