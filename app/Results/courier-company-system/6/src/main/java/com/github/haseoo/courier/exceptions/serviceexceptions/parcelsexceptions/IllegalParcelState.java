...
import com.github.haseoo.courier.exceptions.ExceptionMessages;
...
public String parcelStateType(Long id,ParcelStateType currentState,ParcelStateType[] acceptableStates){
    return Arrays.stream(acceptableStates).map(state -> String.format(ACCEPTABLE_STATES_LIST_FORMAT, state)).collect(Collectors.joining("", String.format(ILLEGAL_PARCEL_STATE_EXCEPTION_FORMAT, id, currentState), ""));
}
