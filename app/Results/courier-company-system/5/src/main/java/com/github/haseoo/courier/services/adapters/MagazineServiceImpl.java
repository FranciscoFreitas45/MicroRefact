...
import com.github.haseoo.courier.utilities.Constants.POSTAL_CODE_SERVICE_WARN_LOG_FORMAT;
...
public MagazineData add(MagazineAddOperationData magazineAddOperationData){
    postalCodeHelper.validatePostalCode(magazineAddOperationData.getAddress().getCity(), magazineAddOperationData.getAddress().getPostalCode());
...
public List<MagazineModel> findClosestMagazine(AddressData address,List<MagazineModel> list,ClosestMagazineChain magazineChain){
...
public List<MagazineData> getList(){
    return magazineRepository.getList().stream().map(MagazineData::of).collect(Collectors.toList());
}
...
public boolean isPostalCodeInCity(AddressData address){
...
public List<MagazineData> getActiveList(){
    return magazineRepository.getActiveMagazines().stream().map(MagazineData::of).collect(Collectors.toList());
}


@Override
public List<ParcelData> getParcelsToReturn(Long id){
    return getById(id).getParcels().stream().filter(parcelData -> !parcelData.getToReturn()).filter(parcelData -> parcelData.getParcelStates().stream().filter(parcelStateData -> parcelStateData.getState().equals(IN_MAGAZINE)).count() > estimatedDeliveryTimeRepository.getById(Long.valueOf(IDS)).getTimesAtMagazineToReturn()).collect(Collectors.toList());
}
...
public List<ParcelData> getAssignedAtSenderParcels(Long id){
    return getById(id).getParcels().stream().filter(parcelData -> parcelData.getCurrentState().getState().equals(AT_SENDER)).collect(Collectors.toList());
}
