...
public List<UserData> getList(){
    return userRepository.getList().stream().map(UserData::of).collect(Collectors.toList());
}
...
public JwtAuthenticationResponse getJwt(LoginCommandData loginCommandData){
    Authentication authentication = authenticationManager.authenticate(new UsernamePasswordAuthenticationToken(loginCommandData.getUserName(), String.valueOf(loginCommandData.getPassword())));
