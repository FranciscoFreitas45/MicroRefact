...
public CourierData getById(Long id){
    return CourierData.of(courierRepository.getById(id).orElseThrow(() -> new EmployeeNotFoundException(id, EmployeeType.COURIER)));
}
...
public List<CourierData> getList(){
    return courierRepository.getList().stream().map(CourierData::of).collect(Collectors.toList());
}


public void validatePeselExistence(String pesel){
    if (employeeRepository.findActiveByPesel(pesel).stream().anyMatch(employeeModel -> employeeModel instanceof CourierModel)) {
        throw new ActiveCourierExistsException();
    }
}


public boolean peselChanged(EmployeeEditOperationData courierEditOperationData,CourierModel courierModel){
