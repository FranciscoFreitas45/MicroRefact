...
import com.github.haseoo.courier.enums.EmployeeType.LOGISTICIAN;
...
public LogisticianData add(EmployeeAddOperationData addOperationData){
    ...
    return LogisticianData.of(logisticianRepository.saveAndFlush(modelToAdd));
}


@Override
public void consumeAllById(List<Long> ids,Consumer<List<LogisticianModel>> consumer){
    consumer.accept(ids.stream().map(id -> logisticianRepository.getById(id).orElseThrow(() -> new EmployeeNotFoundException(id, LOGISTICIAN))).collect(Collectors.toList()));
}


@Override
public LogisticianData getById(Long id){
    return LogisticianData.of(logisticianRepository.getById(id).orElseThrow(() -> new EmployeeNotFoundException(id, LOGISTICIAN)));
}
...
public List<LogisticianData> getList(){
    return logisticianRepository.getList().stream().map(LogisticianData::of).collect(Collectors.toList());
}


public void validatePeselExistence(String pesel){
    if (employeeRepository.findActiveByPesel(pesel).stream().anyMatch(employeeModel -> employeeModel instanceof LogisticianModel)) {
        throw new ActiveCourierExistsException();
    }
}


public boolean peselChanged(EmployeeEditOperationData operationData,LogisticianModel logisticianModel){
