...
@GetMapping("{id}")
public ResponseEntity<LogisticianView> getById(Long id){
...
public ResponseEntity<List<LogisticianView>> getList(){
    return new ResponseEntity<>(logisticianService.getList().stream().map(LogisticianView::of).collect(Collectors.toList()), OK);
}
