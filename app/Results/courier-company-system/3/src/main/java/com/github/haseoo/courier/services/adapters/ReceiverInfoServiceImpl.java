...
public List<ReceiverInfoData> getList(){
    return receiverInfoRepository.getList().stream().map(ReceiverInfoData::of).collect(Collectors.toList());
}
...
public void consume(ReceiverInfoOperationData receiverInfoOperationData,Consumer<ReceiverInfoModel> consumer){
    ...
    consumer.accept(receiverInfoRepository.receiverInfoExists(ReceiverInfoQueryData.of(receiverInfoOperationData)).orElseGet(() -> receiverInfoRepository.saveAndFlush(modelMapper.map(receiverInfoOperationData, ReceiverInfoModel.class))));
}
