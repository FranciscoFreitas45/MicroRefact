...
public ParcelData moveDate(Long id,char[] pin,LocalDate newDate){
    ParcelModel parcelModel = parcelRepository.getById(id).orElseThrow(() -> new ParcelTypeNotFound(id));
...
public void validatePin(char[] pin,ParcelModel parcelModel){
...
public ParcelStateRecord getFistState(ParcelModel parcelModel){
...
public ParcelModel prepareEditedParcelModel(Long id,ParcelEditData parcelEditData){
...
public void validateNewDate(LocalDate newDate,ParcelModel parcelModel){
...
public void validateMoveable(ParcelModel parcelModel){
...
public void changeParcelState(Payment payment){
    String parcelId = payment.getTransactions().get(0).getItemList().getItems().get(0).getName();
    ParcelModel parcelModel = parcelRepository.getById(Long.valueOf(parcelId.substring(parcelId.indexOf(' ') + 1))).orElseThrow(() -> new ParcelNotFound(Long.valueOf(parcelId.substring(parcelId.indexOf(' ') + 1))));
...
public void setReceiverAsSender(ParcelModel parcelModel){
    parcelModel.setToReturn(true);
    parcelModel.setDateMoved(false);
    ClientModel client = clientRepository.getById(parcelModel.getSender().getId()).orElseThrow(() -> new ClientNotFound(parcelModel.getSender().getId()));
    if (client.getClientType() == ClientType.COMPANY) {
        ClientCompanyModel clientCompany = clientCompanyRepository.getById(client.getId()).orElseThrow(() -> new ClientNotFound(client.getId()));
        ...
        ClientIndividualModel clientIndividual = clientIndividualRepository.getById(client.getId()).orElseThrow(() -> new ClientNotFound(client.getId()));
...
public void checkParcel(ParcelModel parcelModel){
...
public List<ParcelData> getList(){
    return parcelRepository.getList().stream().map(ParcelData::of).collect(Collectors.toList());
}


public void validatePostalCode(AddressOperationData senderAddress,AddressOperationData deliveryAddress){
    postalCodeHelper.validatePostalCode(senderAddress.getCity(), senderAddress.getPostalCode());
    postalCodeHelper.validatePostalCode(deliveryAddress.getCity(), deliveryAddress.getPostalCode());
}
...
public ParcelModel buildParcelModel(ParcelAddData parcelAddData,ParcelTypeModel parcelTypeModel,ClientModel clientModel){
...
public ParcelModel prepareParcelModel(ParcelAddData parcelAddData){
    ParcelTypeModel parcelTypeModel = parcelTypeRepository.getById(parcelAddData.getParcelTypeId()).orElseThrow(() -> new ParcelTypeNotFound(parcelAddData.getParcelTypeId()));
    ClientModel clientModel = clientRepository.getById(parcelAddData.getSenderId()).orElseThrow(() -> new ClientNotFound(parcelAddData.getSenderId()));
