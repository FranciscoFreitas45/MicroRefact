...
public Pager4EasyUI<Complaint> queryName(HttpSession session,String pageNumber,String pageSize,Complaint complaint){
    if (SessionUtil.isLogin(session)) {
...
public Pager4EasyUI<Complaint> queryByPagerComplaintUser(HttpSession session,String pageNumber,String pageSize){
    if (SessionUtil.isOwnerLogin(session)) {
        String roles = "车主";
        if (RoleUtil.checkRoles(roles)) {
...
public Pager4EasyUI<Complaint> queryByPager(HttpSession session,String pageNumber,String pageSize){
    if (SessionUtil.isLogin(session)) {
        String roles = "系统超级管理员,系统普通管理员,公司超级管理员,公司普通管理员,汽车公司接待员,车主";
        if (RoleUtil.checkRoles(roles)) {
            logger.info("分页查看投诉记录");
