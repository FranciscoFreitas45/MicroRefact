...
public String[] perIdsStr2perIds(String perIdsStr,String split){
...
public ControllerResult addPermission(String roleId,String permissionId,HttpSession session){
    // TODO 超级管理员
    if (SessionUtil.isLogin(session)) {
        String roles = "系统超级管理员,系统普通管理员";
        if (RoleUtil.checkRoles(roles)) {
...
public List<Permission> queryPermissions(String id,HttpSession session){
    // TODO 所有管理员
    if (SessionUtil.isLogin(session)) {
        String roles = "系统超级管理员,系统普通管理员";
        if (RoleUtil.checkRoles(roles)) {
...
public ControllerResult removePermission(String roleId,String permissionId,HttpSession session){
    // TODO 超级管理员
    if (SessionUtil.isLogin(session)) {
        String roles = "系统超级管理员,系统普通管理员";
        if (RoleUtil.checkRoles(roles)) {
...
public List<String> removeEmpStr(List addedPerIds){
...
public ControllerResult updatePermission(String roleId,String addedPerIdsStr,String removedPerIdsStr,HttpSession session){
    // TODO 超级管理员
    if (SessionUtil.isLogin(session)) {
        String roles = "系统超级管理员,系统普通管理员";
        if (RoleUtil.checkRoles(roles)) {
...
public Pager4EasyUI queryRecycle(int pageNumber,int pageSize,HttpSession session){
    // TODO 所有管理员
    if (SessionUtil.isLogin(session)) {
