...
public Pager4EasyUI<Supply> queryByPagerDisable(HttpSession session,String pageNumber,String pageSize){
    if (SessionUtil.isLogin(session)) {
...
public Pager4EasyUI<Supply> blurredQuery(HttpSession session,HttpServletRequest request,String pageNumber,String pageSize){
    if (SessionUtil.isLogin(session)) {
        String roles = "系统超级管理员,系统普通管理员,公司超级管理员,公司普通管理员";
        if (RoleUtil.checkRoles(roles)) {
            logger.info("供应商模糊查询");
...
public Pager4EasyUI<Supply> queryByPager(HttpSession session,String pageNumber,String pageSize){
    if (SessionUtil.isLogin(session)) {
        String roles = "系统超级管理员,系统普通管理员,公司超级管理员,公司普通管理员";
        if (RoleUtil.checkRoles(roles)) {
            logger.info("供应商分页查询");
