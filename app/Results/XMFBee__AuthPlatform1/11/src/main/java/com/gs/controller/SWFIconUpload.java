...
public boolean fileSave(List<MultipartFile> sourceFiles,String savePath,HttpSession session){
...
public Object uploadIcon(DefaultMultipartHttpServletRequest req,HttpSession session){
    ...
    if (fileSave(files, savePath, session)) {
        if (user.getUserIcon() == null || user.getUserIcon().equals("")) {
            // 设置头像
            int i = userService.updIcon(user.getUserId(), savePath + user.getUserId() + ".jpg");
            if (i > 0) {
