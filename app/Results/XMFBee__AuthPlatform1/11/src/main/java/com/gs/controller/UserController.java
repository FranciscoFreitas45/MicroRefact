...
import com.gs.DTO.IndustrySMS;
import com.gs.DTO.Mail;
...
public ControllerResult emailConfirm(HttpServletRequest req,HttpSession session,String email,int code,String pwd){
...
public ControllerResult updatePwd(String oldPwd,String newPwd,String conPwd,HttpSession session,HttpServletRequest request){
...
public ControllerResult rtEmailCode(HttpServletRequest req,HttpSession session){
...
public ControllerResult userLogin(User user1,HttpSession session,HttpServletRequest req,String checkCode){
    ...
    if (checkCode != null && checkCode.equals(codeSession)) {
        ...
        try {
            ...
            if (subject.hasRole(Constants.role_systemSuperAdmin) || subject.hasRole(Constants.role_systemOrdinaryAdmin) || subject.hasRole(Constants.role_companySuperAdmin) || subject.hasRole(Constants.role_companyOrdinaryAdmin) || subject.hasRole(Constants.role_companyReceptionist) || subject.hasRole(Constants.role_companyTotalTC) || subject.hasRole(Constants.role_companyTechnician) || subject.hasRole(Constants.role_companyApprentice) || subject.hasRole(Constants.role_companySales) || subject.hasRole(Constants.role_companyFinancial) || subject.hasRole(Constants.role_companyProcurement) || subject.hasRole(Constants.role_companyLibraryTube) || subject.hasRole(Constants.role_companyHR)) {
                logger.info("登录成功");
...
public ControllerResult phoneConfirm(HttpServletRequest req,HttpSession session,String phone,int code,String pwd){
