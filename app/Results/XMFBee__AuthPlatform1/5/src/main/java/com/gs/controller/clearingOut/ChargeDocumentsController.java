...
import com.gs.DTO.MaintainRecord;
import com.gs.DTO.Checkin;
import com.gs.DTO.ExcelExport;
...
public ControllerResult updateDate(HttpSession session,String chargeBillId,String maintainRecordId){
    if (SessionUtil.isLogin(session)) {
...
public Pager4EasyUI<ChargeBill> queryByOwner(HttpSession session,String pageNumber,String pageSize){
    if (SessionUtil.isOwnerLogin(session)) {
        String roles = "车主";
        if (RoleUtil.checkRoles(roles)) {
...
public Pager4EasyUI<ChargeBill> queryByPagerDisable(HttpSession session,String pageNumber,String pageSize){
    if (SessionUtil.isLogin(session)) {
    ...
    if (SessionUtil.isLogin(session)) {
        ...
        if (RoleUtil.checkRoles(roles)) {
            ...
            try {
                ...
                String title = "收费单据";
                String[] rowsName = new String[] { "收费单据编号", "车主姓名", "车主手机", "汽车品牌", "汽车车型", "汽车颜色", "汽车车牌", "车牌号码", "维修保养记录提车时间", "维修保养记录描述", "付款方式", "总金额", "实际付款", "收费时间", "收费单据创建时间", "收费单据描述", "收费单据状态" };
...
public Pager4EasyUI<ChargeBill> blurredQuery(HttpSession session,HttpServletRequest request,String pageNumber,String pageSize){
    if (SessionUtil.isLogin(session)) {
        String roles = "系统超级管理员,系统普通管理员,公司超级管理员,公司普通管理员,汽车公司接待员";
        if (RoleUtil.checkRoles(roles)) {
...
public Pager4EasyUI<ChargeBill> queryByPager(HttpSession session,String pageNumber,String pageSize){
    if (SessionUtil.isLogin(session)) {
        String roles = "系统超级管理员,系统普通管理员,公司超级管理员,公司普通管理员,汽车公司接待员";
        if (RoleUtil.checkRoles(roles)) {
