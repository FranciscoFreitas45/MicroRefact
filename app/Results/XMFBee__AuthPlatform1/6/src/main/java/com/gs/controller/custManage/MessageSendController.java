...
public ControllerResult batchInsert(HttpSession session,String userIds,String userPhone){
...
public Pager4EasyUI<MessageSend> blurredQuery(HttpSession session,String pageNumber,String pageSize,MessageSend messageSend,User user){
    if (SessionUtil.isLogin(session)) {
        String roles = "公司超级管理员,公司普通管理员,汽车公司接待员";
        if (RoleUtil.checkRoles(roles)) {
...
public Pager4EasyUI<MessageSend> queryByPager(HttpSession session,String pageNumber,String pageSize){
    if (SessionUtil.isLogin(session)) {
        String roles = "公司超级管理员,公司普通管理员,汽车公司接待员";
        if (RoleUtil.checkRoles(roles)) {
...
public MessageSend queryById(HttpSession session,String messageId){
    if (SessionUtil.isLogin(session)) {
        String roles = "公司超级管理员,公司普通管理员,汽车公司接待员";
        if (RoleUtil.checkRoles(roles)) {
