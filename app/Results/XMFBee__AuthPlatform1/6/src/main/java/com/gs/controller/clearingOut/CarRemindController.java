...
public ControllerResult remind(HttpSession session,HttpServletRequest req,String ids,String phones,String userName){
    if (SessionUtil.isLogin(session)) {
        ...
        if (RoleUtil.checkRoles(roles)) {
            ...
            if (ids != null && ids != "" && phones != null && phones != "") {
                ...
                // 根据维修保养记录id查出所有的用户, 拿到用户的email;
                List<User> users = userService.queryEmail(ids);
                String emails = "";
                for (User u : users) {
                    // 设置邮箱
