...
public Pager4EasyUI<Company> queryByPagerDisable(HttpSession session,String pageNumber,String pageSize){
    if (SessionUtil.isLogin(session)) {
...
public Map afterSubForm(MultipartFile file,String userId,HttpServletRequest request){
    ...
    if (file != null) {
        ...
        if (fileSave(file, savePath, userId, session)) {
            // 设置头像
            companyService.updLogo(userId, savePath + userId + ".jpg");
...
public Pager4EasyUI<Company> queryAll(HttpSession session,String pageNumber,String pageSize){
    if (SessionUtil.isLogin(session)) {
        String roles = "系统超级管理员,系统普通管理员,公司超级管理员,公司普通管理员";
        if (RoleUtil.checkRoles(roles)) {
...
public Pager4EasyUI<Company> blurredQuery(HttpSession session,HttpServletRequest request,String pageNumber,String pageSize){
    if (SessionUtil.isLogin(session)) {
        String roles = "系统超级管理员,系统普通管理员,公司超级管理员,公司普通管理员";
        if (RoleUtil.checkRoles(roles)) {
            logger.info("公司模糊查询");
...
public ModelAndView queryById(HttpServletRequest request,String companyId){
