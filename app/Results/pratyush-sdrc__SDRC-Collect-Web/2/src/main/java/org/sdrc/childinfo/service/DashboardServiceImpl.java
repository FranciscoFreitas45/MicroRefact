...
import org.sdrc.DTO.UtData;
import org.sdrc.DTO.UtAreaEn;
import org.sdrc.DTO.ValueObject;
import org.sdrc.DTO.CounterCount;
import org.sdrc.DTO.UtIndicatorEn;
@Service
        ...
        for (int i = 0; i < listData.size(); i++) {
            ...
            for (Object[] dataObject : listData) {
                ...
                if (utData.getUnit_NId() == 2) {
                    ...
                    lineChat.setIsPositive(percentChange != null ? (Double) (list.get(listData.size() - 1).getValue()) > (Double) (list.get(listData.size() - 2).getValue()) ? isPositveIndicator(indicatorId) == 1 ? true : false : isPositveIndicator(indicatorId) == 0 ? false : true : false);
                } else {
                    ...
                    lineChat.setIsPositive(percentChange != null ? percentChange.doubleValue() >= 0 ? isPositveIndicator(indicatorId) == 1 ? true : false : isPositveIndicator(indicatorId) == 1 ? false : true : false);
                }
            }
...
public String getDecadesvalue(double x){
    ...
    return String.valueOf(t1) + "," + String.valueOf(t2) + "," + String.valueOf(Integer.parseInt(valueofY) - 1);
}
