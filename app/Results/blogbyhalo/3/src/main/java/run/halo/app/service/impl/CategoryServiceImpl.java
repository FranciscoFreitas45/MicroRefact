...
public CategoryServiceImpl(CategoryRepository categoryRepository, PostCategoryService postCategoryService, OptionService optionService, AuthenticationService authenticationService, AuthorizationService authorizationService) {
...
public boolean doCategoryHasEncrypt(Map<Integer,Category> idToCategoryMap,Integer categoryId){
    if (categoryId == 0) {
        return false;
    }
...
public void doEncryptPost(Category category){
    CategoryVO topLevelCategory = createTopLevelCategory();
...
public void concreteTree(CategoryVO parentCategory,List<Category> categories,Boolean fillPassword){
    ...
    children.forEach(category -> {
        ...
        fullPath.append(URL_SEPARATOR).append(optionService.getCategoriesPrefix()).append(URL_SEPARATOR).append(child.getSlug()).append(optionService.getPathSuffix());
...
public void doDecryptPost(Category category){
    List<Category> allCategoryList = super.listAll();
    Map<Integer, Category> idToCategoryMap = allCategoryList.stream().collect(Collectors.toMap(Category::getId, Function.identity()));
...
@NonNull
public CategoryVO createTopLevelCategory(){
...
public void doFilterEncryptCategory(List<CategoryVO> categoryList){
...
public void refreshPostStatus(List<Integer> affectedPostIdList){
    ...
    for (Integer postId : affectedPostIdList) {
        ...
        if (StrUtil.isNotBlank(post.getPassword())) {
            ...
            postCategoryService.listByPostId(postId).stream().map(PostCategory::getCategoryId).filter(this::categoryHasEncrypt).findAny().ifPresent(id -> post.setStatus(PostStatus.INTIMATE));
        }
...
public void collectAllChildByCategoryId(List<Category> collectorList,List<CategoryVO> childrenList,Integer categoryId,Boolean doNotCollectEncryptedCategory){
...
public void removeCategoryAndPostCategoryBy(Integer categoryId){
    ...
    List<Integer> affectedPostIdList = postCategoryService.removeByCategoryId(categoryId).stream().map(PostCategory::getPostId).collect(Collectors.toList());
    refreshPostStatus(affectedPostIdList);
}


public void collectAllChild(List<Category> collectorList,List<CategoryVO> childrenList,Boolean doNotCollectEncryptedCategory){
    if (CollectionUtil.isEmpty(childrenList)) {
        return;
    }
    for (CategoryVO categoryVO : childrenList) {
