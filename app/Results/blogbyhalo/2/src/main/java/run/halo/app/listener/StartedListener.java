...
public void printStartInfo(){
    ...
    if (documentationEnabled) {
        log.debug(AnsiOutput.toString(AnsiColor.BRIGHT_BLUE, "Halo api documentation was enabled at  ", blogUrl, "/swagger-ui.html"));
    }
...
public void migrate(){
    ...
    Flyway flyway = Flyway.configure().locations("classpath:/migration").baselineVersion("1").baselineOnMigrate(true).dataSource(url, username, password).load();
    flyway.repair();
    flyway.migrate();
