...
@DisableOnCondition
public BaseResponse<String> updatePassword(PasswordParam passwordParam,User user){
...
@DisableOnCondition
public MultiFactorAuthVO updateMFAuth(MultiFactorAuthParam multiFactorAuthParam,User user){
...
@DisableOnCondition
public UserDTO updateProfile(UserParam userParam,User user){
...
@DisableOnCondition
public MultiFactorAuthVO generateMFAQrImage(MultiFactorAuthParam multiFactorAuthParam,User user){
    if (MFAType.NONE == user.getMfaType()) {
        if (MFAType.TFA_TOTP == multiFactorAuthParam.getMfaType()) {
            ...
            String qrImageBase64 = "data:image/png;base64," + Base64.encode(QrCodeUtil.generatePng(optAuthUrl, 128, 128));
