package run.halo.app.theme;
 import run.halo.app.theme.ThemeUpdater.backup;
import run.halo.app.theme.ThemeUpdater.restore;
import run.halo.app.utils.GitUtils.commitAutomatically;
import run.halo.app.utils.GitUtils.logCommit;
import run.halo.app.utils.GitUtils.removeRemoteIfExists;
...
public ThemeProperty update(String themeId){
    // get theme property
    final var oldThemeProperty = themeRepository.fetchThemePropertyByThemeId(themeId).orElseThrow(() -> new NotFoundException("主题 " + themeId + " 不存在或以删除！").setErrorData(themeId));
    // get update config
