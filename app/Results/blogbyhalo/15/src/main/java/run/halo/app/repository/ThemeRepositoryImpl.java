package run.halo.app.repository;
 import run.halo.app.model.properties.PrimaryProperties.THEME;
import run.halo.app.model.support.HaloConst.DEFAULT_THEME_ID;
import run.halo.app.utils.FileUtils.copyFolder;
import run.halo.app.utils.FileUtils.deleteFolderQuietly;
import run.halo.app.utils.VersionUtil.compareVersion;
...
import run.halo.app.DTO.OptionRepository;
@Repository
@Slf4j
public class ThemeRepositoryImpl implements ThemeRepository,ApplicationListener<OptionUpdatedEvent>{
...
@NonNull
public ThemeProperty getThemeByThemeId(String themeId){
    return fetchThemeByThemeId(themeId).orElseThrow(() -> new ThemeNotFoundException("Failed to find theme with id: " + themeId));
}
...
public Optional<ThemeProperty> fetchThemePropertyByThemeId(String themeId){
    ...
    return ThemePropertyScanner.INSTANCE.scan(getThemeRootPath(), null).stream().filter(property -> Objects.equals(themeId, property.getId())).findFirst();
}


@NonNull
public Optional<ThemeProperty> fetchThemeByThemeId(String themeId){
    return ThemePropertyScanner.INSTANCE.scan(getThemeRootPath(), null).stream().filter(property -> Objects.equals(themeId, property.getId())).findFirst();
}
    ...
    if (themeProperty == null) {
        ...
        synchronized (this) {
            if (this.currentTheme == null) {
                ...
                String currentThemeId = this.optionRepository.findByKey(THEME.getValue()).map(Option::getValue).orElse(DEFAULT_THEME_ID);
                // fetch current theme
                this.currentTheme = this.fetchThemeByThemeId(currentThemeId).orElseGet(() -> {
                    ...
                    throw new ThemeNotFoundException("Default theme: " + DEFAULT_THEME_ID + " was not found!");
                });
            }
        }
...
public boolean checkThemePropertyCompatibility(ThemeProperty themeProperty){
    ...
    return StringUtils.isNotEmpty(themeProperty.getRequire()) && !compareVersion(HaloConst.HALO_VERSION, themeProperty.getRequire());
}
