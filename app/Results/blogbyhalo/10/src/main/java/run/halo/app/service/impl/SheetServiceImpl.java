...
public SheetServiceImpl(SheetRepository sheetRepository, ApplicationEventPublisher eventPublisher, SheetCommentService sheetCommentService, SheetMetaService sheetMetaService, ThemeService themeService, OptionService optionService) {
...
@NonNull
public SheetDetailVO convertTo(Sheet sheet,List<SheetMeta> metas){
...
public String buildFullPath(Sheet sheet){
    ...
    if (permalinkType.equals(SheetPermalinkType.SECONDARY)) {
        fullPath.append(URL_SEPARATOR).append(optionService.getSheetPrefix()).append(URL_SEPARATOR).append(sheet.getSlug()).append(optionService.getPathSuffix());
    } else if (permalinkType.equals(SheetPermalinkType.ROOT)) {
        fullPath.append(URL_SEPARATOR).append(sheet.getSlug()).append(optionService.getPathSuffix());
    }
...
public Page<SheetListVO> convertToListVo(Page<Sheet> sheetPage){
    ...
    Map<Integer, Long> sheetCommentCountMap = sheetCommentService.countByStatusAndPostIds(CommentStatus.PUBLISHED, sheetIds);
...
@Override
public void slugMustNotExist(Sheet sheet){
    Assert.notNull(sheet, "Sheet must not be null");
    // Get slug count
    boolean exist;
