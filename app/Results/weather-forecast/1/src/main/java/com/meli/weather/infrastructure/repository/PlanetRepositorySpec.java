package com.meli.weather.infrastructure.repository;
 import java.lang;
import java.util;
import java.io;
import java.net;
import groovy.lang;
import groovy.util;
public class PlanetRepositorySpec {


public com.meli.weather.infrastructure.model.Planet getPlanetModel(){
    return (com.meli.weather.infrastructure.model.Planet) null;
}


public com.meli.weather.infrastructure.model.Planet getVulcanoModel(){
    return (com.meli.weather.infrastructure.model.Planet) null;
}


public com.meli.weather.domain.Planet getBetasoideDomain(){
    return (com.meli.weather.domain.Planet) null;
}


public com.meli.weather.domain.Planet getPlanetDomain(java.lang.Integer speed,java.lang.Integer distance){
    return (com.meli.weather.domain.Planet) null;
}


@groovy.transform.Generated()
@groovy.transform.Internal()
public void setMetaClass(groovy.lang.MetaClass mc){
}


public com.meli.weather.infrastructure.model.Planet getBetasoideModel(){
    return (com.meli.weather.infrastructure.model.Planet) null;
}


public com.meli.weather.domain.repository.PlanetRepository getPlanetRepository(){
    return (com.meli.weather.domain.repository.PlanetRepository) null;
}


public void setPlanetRepository(com.meli.weather.domain.repository.PlanetRepository value){
}


public com.meli.weather.domain.Planet getVulcanoDomain(){
    return (com.meli.weather.domain.Planet) null;
}


public com.meli.weather.infrastructure.model.Planet getFerengiModel(){
    return (com.meli.weather.infrastructure.model.Planet) null;
}


@groovy.transform.Generated()
@groovy.transform.Internal()
public groovy.lang.MetaClass getMetaClass(){
    return (groovy.lang.MetaClass) null;
}


public com.meli.weather.domain.Planet getFerengiDomain(){
    return (com.meli.weather.domain.Planet) null;
}


}