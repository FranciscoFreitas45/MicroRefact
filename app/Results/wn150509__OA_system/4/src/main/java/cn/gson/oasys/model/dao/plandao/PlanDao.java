...
public Page<Plan> findByUserOrderByStatusIdDesc(long userid,Pageable pa)
;

@Query("from Plan p where p.user.userId=?1  order by p.statusId Asc")
public Page<Plan> findByUserOrderByStatusIdAsc(long userid,Pageable pa)
;
...
@Modifying
public Integer updatesome(long typeId,long statusId,Date startTime,Date endTime,String title,String label,String planContent,String planSummary,long pid)
;

@Query("from Plan p where (p.label like %?1% or p.title like %?1% or DATE_format(p.createTime,'%Y-%m-%d') like %?1% or " + "p.typeId in (select t.typeId from SystemTypeList t where t.typeName like %?1% ) or " + "p.statusId in (select s.statusId from SystemStatusList s where s.statusName like %?1%)) and p.user.userId=?2")
public Page<Plan> findBybasekey(String baseKey,long userid,Pageable pa)
;

@Query("from Plan p where p.user.userId=?1  order by p.typeId Desc")
public Page<Plan> findByUserOrderByTypeIdDesc(long userid,Pageable pa)
;

@Query("from Plan p where p.user.userId=?1 order by p.createTime Asc")
public Page<Plan> findByUserOrderByCreateTimeAsc(long userid,Pageable pa)
;

@Query("from Plan p where p.user.userId=?1 order by p.createTime Desc ")
public Page<Plan> findByUserOrderByCreateTimeDesc(long userid,Pageable pa)
;
...
public Plan findlatest(Date start,Date end,long id,long typeid)
;

@Query("from Plan p where p.user.userId=?1  order by p.typeId Asc")
public Page<Plan> findByUserOrderByTypeIdAsc(long userid,Pageable pa)
;
