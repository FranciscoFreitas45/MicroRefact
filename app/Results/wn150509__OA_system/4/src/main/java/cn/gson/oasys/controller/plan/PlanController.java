...
@Autowired
 private FileServices fServices;
...
public void sortpaging(Model model,HttpSession session,int page,String baseKey,String type,String status,String time,String icon){
...
public String test(Model model,HttpSession session,int page,String baseKey,String type,String status,String time,String icon){
...
public String test23(HttpServletRequest request,Model model,HttpSession session,String pid,String comment,int page,String baseKey){
...
public String testdd(Model model,HttpSession session,int page,String baseKey,String type,String status,String time,String icon){
...
public String testMess(MultipartFile file,HttpServletRequest req,Plan plan2,BindingResult br){
    ...
    } else // 校验通过，下面写自己的逻辑业务
    {
        ...
        if (pid == -1) {
            if (!file.isEmpty()) {
                ...
                attid = null;
            plan = new Plan(typeid, statusid, attid, start, end, new Date(), plan2.getTitle(), plan2.getLabel(), plan2.getPlanContent(), plan2.getPlanSummary(), null, user);
            planDao.save(plan);
        }
        if (pid > 0) {
            ...
            planservice.updateplan(typeid, statusid, start, end, plan2.getTitle(), plan2.getLabel(), plan2.getPlanContent(), plan2.getPlanSummary(), pid);
        }
...
public void typestatus(Model model){
