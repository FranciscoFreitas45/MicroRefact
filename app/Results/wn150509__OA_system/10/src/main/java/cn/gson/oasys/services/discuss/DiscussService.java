...
public void discussHandle(Model model,Long num,Long userId,int page,int size,Long selectType,Long selectSort){
    ...
    if (!StringUtils.isEmpty(selectType)) {
        ...
        // 根据讨论id找到所有的回复表
        replyPage = replyDao.findByDiscuss(discuss, pa);
    }
    ...
    if (replyCols.size() > 0) {
        // 用数组来结束所有回复表的id
        Long[] replyLong = new Long[replyCols.size()];
...
public Pageable setPageable(int page,String type,String time,String visitnum,List<Order> orders){
    int size = 10;
