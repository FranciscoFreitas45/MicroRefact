...
public ResponseEntity<RestResult> list(HttpServletRequest request,String id,String cate,String q,Integer p,Integer ps){
    if (StringUtils.isNotBlank(id)) {
        return new ResponseEntity<>(new RestResult(RestResultType.OK, quickReplyRepository.findOne(id)), HttpStatus.OK);
    }
    Page<QuickReply> replyList = quickReplyRepository.getByOrgiAndCate(getOrgi(request), cate, q, new PageRequest(p == null ? 1 : p, ps == null ? 20 : ps));
...
public ResponseEntity<RestResult> put(HttpServletRequest request,QuickReply quickReply){
