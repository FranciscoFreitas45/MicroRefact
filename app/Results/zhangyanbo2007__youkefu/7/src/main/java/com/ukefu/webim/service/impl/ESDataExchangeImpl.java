...
public UKDataBean getIObjectByPK(String type,String id){
    ...
    if (!StringUtils.isBlank(type)) {
        GetResponse getResponse = UKDataContext.getTemplet().getClient().prepareGet(UKDataContext.CALLOUT_INDEX, type, id).execute().actionGet();
...
public IndexRequestBuilder saveBulk(UKDataBean dataBean){
    ...
    if (!StringUtils.isBlank(dataBean.getType())) {
        indexRequestBuilder = UKDataContext.getTemplet().getClient().prepareIndex(UKDataContext.CALLOUT_INDEX, dataBean.getType(), dataBean.getId()).setSource(processValues(dataBean));
    } else {
        indexRequestBuilder = UKDataContext.getTemplet().getClient().prepareIndex(UKDataContext.CALLOUT_INDEX, dataBean.getTable().getTablename(), dataBean.getId()).setSource(processValues(dataBean));
    }
    return indexRequestBuilder;
}


public void deleteIObject(UKDataBean dataBean){
...
public void deleteByCon(QueryBuilder query,String type){
    BulkRequestBuilder bulkRequest = UKDataContext.getTemplet().getClient().prepareBulk();
    SearchResponse response = UKDataContext.getTemplet().getClient().prepareSearch(UKDataContext.CALLOUT_INDEX).setTypes(type).setSearchType(SearchType.DFS_QUERY_THEN_FETCH).setQuery(query).setFrom(0).setSize(10000).setExplain(true).execute().actionGet();
...
public Map<String,Object> processValues(UKDataBean dataBean){
...
public void saveIObject(UKDataBean dataBean,boolean refresh){
    if (dataBean.getId() == null) {
        dataBean.setId((String) dataBean.getValues().get("id"));
    }
...
public PageImpl<EkmDataBean> findAllPageAggResultEkm(QueryBuilder query,Pageable page,boolean loadRef,String types){
    ...
    SearchResponse response = searchBuilder.setQuery(query).execute().actionGet();
    List<String> users = new ArrayList<String>(), organs = new ArrayList<String>(), dimenids = new ArrayList<String>();
            ...
            if (agg != null) {
                if (agg.getBuckets() != null && agg.getBuckets().size() > 0) {
                    for (Terms.Bucket entry : agg.getBuckets()) {
                        ...
                        if (loadRef == true) {
                            if ("creater".equals(agg.getName())) {
                                // 根据知识作者ID
                                users.add(entry.getKeyAsString());
                            }
                            if ("organ".equals(agg.getName())) {
                                // 根据知识所属的部门ID
                                organs.add(entry.getKeyAsString());
                            }
                            if ("dimenid".equals(agg.getName())) {
                                // 根据知识所属的维度ID
...
public void updateIObject(UKDataBean dataBean){
    if (dataBean.getId() == null) {
        dataBean.setId((String) dataBean.getValues().get("id"));
    }
    ...
    UKDataContext.getTemplet().getClient().prepareUpdate(UKDataContext.CALLOUT_INDEX, dataBean.getTable().getTablename(), dataBean.getId()).setDoc(processValues(dataBean)).execute().actionGet();
}
...
public void deleteById(String type,String id){
    if (!StringUtils.isBlank(type) && !StringUtils.isBlank(id)) {
        UKDataContext.getTemplet().getClient().prepareDelete(UKDataContext.CALLOUT_INDEX, type, id).execute().actionGet();
    }
}
