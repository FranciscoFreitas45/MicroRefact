...
public void postHandle(HttpServletRequest arg0,HttpServletResponse response,Object arg2,ModelAndView view){
    User user = (User) arg0.getSession().getAttribute(UKDataContext.USER_SESSION_NAME);
    // 进入信息采集模式
    String infoace = (String) arg0.getSession().getAttribute(UKDataContext.UKEFU_SYSTEM_INFOACQ);
    ...
    if (view != null) {
        ...
        view.addObject("uKeFuDic", UKeFuDic.getInstance());
        // 处理系统 需要隐藏号码的字段， 启动的时候加载
        view.addObject("uKeFuSecField", CacheHelper.getSystemCacheBean().getCacheObject(UKDataContext.UKEFU_SYSTEM_SECFIELD, UKDataContext.SYSTEM_ORGI));
