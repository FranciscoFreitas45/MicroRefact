...
@Autowired
 ...
 public  int PAGE_SIZE_BG;

 public  int PAGE_SIZE_TW;

 public  int PAGE_SIZE_FV;

 public  int PAGE_SIZE_HA;
        ...
        if (tpList.size() > 0) {
            for (TableProperties tp : tpList) {
                if (!StringUtils.isBlank(request.getParameter("condition")) && tp.getName().equals(request.getParameter("nasearch"))) {
                    if (request.getParameter("condition").equals("scope")) {
                        // 范围
                    ...
                    } else if (request.getParameter("condition").equals("equal")) {
                        // 等于
                    ...
                    } else if (request.getParameter("condition").equals("not")) {
                        // 不等于
                        RangeQueryBuilder rangeQu = null;
                        if (UKTools.getDateFormat(request) == true) {
                            try {
