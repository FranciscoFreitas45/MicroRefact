...
public class QualityResource extends Resource{
 ...
 private  QualityFormFilter qcFormFilter;
...
@Override
public void begin(){
    if (!StringUtils.isBlank(this.jobDetail.getFilterid())) {
        // 筛选表单id是否为空，作为数据的筛选依据
        ...
        if (this.isRecovery() && !StringUtils.isBlank(this.jobDetail.getExectype()) && (this.jobDetail.getExectype().equals("filterid") || this.jobDetail.getExectype().equals("filterskill") || this.jobDetail.getExectype().equals("taskid") || this.jobDetail.getExectype().equals("taskskill"))) {
