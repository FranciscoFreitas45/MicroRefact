...
@Autowired
 private  EkmKnowbaseOrganRepository ekmKnowbaseOrganRes;

@Autowired
 private  UserRoleRepository userRoleRes;

@Autowired
 private  EkmKnowbaseRoleRepository ekmKnowbaseRoleRes;
...
public Page<EkmKnowledgeMaster> processQueryItem(BoolQueryBuilder boolQueryBuilder,Pageable page){
    // 过滤掉已过期的知识
...
@SuppressWarnings("deprecation")
public Page<EkmKnowledgeMaster> processQuery(BoolQueryBuilder boolQueryBuilder,Pageable page){
    // 过滤掉已过期的知识
    QueryBuilder beginFilter = QueryBuilders.boolQuery().should(QueryBuilders.missingQuery("begintime")).should(QueryBuilders.rangeQuery("begintime").to(new Date().getTime()));
    QueryBuilder endFilter = QueryBuilders.boolQuery().should(QueryBuilders.missingQuery("endtime")).should(QueryBuilders.rangeQuery("endtime").from(new Date().getTime()));
...
@Override
public Page<EkmKnowledgeMaster> findByKnowledge(BoolQueryBuilder boolQueryBuilder,boolean datastatus,List<String> ktList,String orgi,User user,Pageable pageable){
