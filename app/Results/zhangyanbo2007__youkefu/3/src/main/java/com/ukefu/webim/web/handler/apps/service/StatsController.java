...
public void statcomentexp(ModelMap map,HttpServletRequest request,HttpServletResponse response,String agent,String skill,String begin,String end){
    Map<String, Object> mapR = UKTools.getRequestParam(request);
    mapR.put("orgi", super.getOrgi(request));
    ReportData reportData = new CubeService("coment.xml", path, dataSource, mapR).execute("SELECT [comment].[满意度].members on columns " + ", NonEmptyCrossJoin(Order([time].[日期].members , [time].[日期].CurrentMember.name , DESC) " + ", NonEmptyCrossJoin([skill].[技能组].members,[agent].[坐席].members)) on rows  FROM [满意度]");
...
@Menu(type = "service", subtype = "statcoment", admin = true)
public ModelAndView statcoment(ModelMap map,HttpServletRequest request,String agent,String skill,String begin,String end){
    Map<String, Object> mapR = UKTools.getRequestParam(request);
    mapR.put("orgi", super.getOrgi(request));
    ReportData reportData = new CubeService("coment.xml", path, dataSource, mapR).execute("SELECT [comment].[满意度].members on columns " + ", NonEmptyCrossJoin(Order([time].[日期].members , [time].[日期].CurrentMember.name , DESC) " + ", NonEmptyCrossJoin([skill].[技能组].members,[agent].[坐席].members)) on rows  FROM [满意度]");
    List<SysDic> dicList = UKeFuDic.getInstance().getDic(UKDataContext.UKEFU_SYSTEM_COMMENT_DIC);
...
public void statagentexp(ModelMap map,HttpServletRequest request,HttpServletResponse response,String agent,String skill,String begin,String end){
    Map<String, Object> mapR = UKTools.getRequestParam(request);
    mapR.put("orgi", super.getOrgi(request));
    ReportData reportData = new CubeService("consult.xml", path, dataSource, mapR).execute("SELECT {[Measures].[咨询数量],[Measures].[平均等待时长（秒）],[Measures].[平均咨询时长（秒）]} on columns " + ", NonEmptyCrossJoin(Order([time].[日期].members , [time].[日期].CurrentMember.name , DESC) " + ", NonEmptyCrossJoin([skill].[技能组].members,[agent].[坐席].members)) on rows  FROM [咨询]");
    response.setHeader("content-disposition", "attachment;filename=UCKeFu-Report-" + new SimpleDateFormat("yyyy-MM-dd").format(new Date()) + ".xls");
...
@Menu(type = "service", subtype = "statagent", admin = true)
public ModelAndView statagent(ModelMap map,HttpServletRequest request,String agent,String skill,String begin,String end){
    Map<String, Object> mapR = UKTools.getRequestParam(request);
    mapR.put("orgi", super.getOrgi(request));
    ReportData reportData = new CubeService("consult.xml", path, dataSource, mapR).execute("SELECT {[Measures].[咨询数量],[Measures].[平均等待时长（秒）],[Measures].[平均咨询时长（秒）]} on columns " + ", NonEmptyCrossJoin(Order([time].[日期].members , [time].[日期].CurrentMember.name , DESC) " + ", NonEmptyCrossJoin([skill].[技能组].members,[agent].[坐席].members)) on rows  FROM [咨询]");
    map.addAttribute("reportData", reportData);
