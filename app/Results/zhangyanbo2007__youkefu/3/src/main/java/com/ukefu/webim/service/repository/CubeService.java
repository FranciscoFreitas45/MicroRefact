...
public class CubeService {
 ...
 private  String SCHEMA_DATA_PATH;
...
public CubeService(String xml, String path, DataSourceService dataSource, Map<String, Object> requestValues) throws IOException, TemplateException {
    ...
    // 使用系统默认编码
    FileUtils.write(schemaFile, UKTools.getTemplet(writer.toString(), requestValues), "UTF-8");
    ...
    if (isContentStr) {
        // 使用系统默认编码
        FileUtils.write(schemaFile, UKTools.getTemplet(xml, requestValues), "UTF-8");
    } else {
        StringWriter writer = new StringWriter();
        IOUtils.copy(CubeService.class.getClassLoader().getResourceAsStream(SCHEMA_DATA_PATH + xml), writer, "UTF-8");
        // 使用系统默认编码
...
public Level getParentLevel(Level level,int rowno,int rows){
...
public void iterator(Map<String,Map> value,Level level,String leveltype){
...
public int getDepth(Level level){
    int depth = 0;
...
public void getRowData(Axis[] axes,int axis,int[] pos,Result result,List<List<ValueData>> dataList,int rowno,Position position,CubeReportData cubeData,List<ColumnProperties> cols){
    if (axis < 0) {
...
public void addFirstTitle(List<FirstTitle> firstTitleList,int index,FirstTitle title){
    boolean found = false;
...
public void processSum(Level level,List<List<ValueData>> valueDataList,Level root){
...
public void sumRowspanColspan(Level level){
    if (level.getChilderen() != null) {
        for (int i = 0; level.getChilderen() != null && i < level.getChilderen().size(); i++) {
...
public Level createTitle(Axis axis,int index,List<ColumnProperties> cols){
    ...
    for (Position pos : posList) {
        ...
        for (int i = 0; i < pos.size(); i++) {
            ...
            } else if (member.getLevel() instanceof RolapLevel && cubeLevel.getName().equals("MeasuresLevel")) {
                // 指标列
                if (level.getFirstTitle() == null) {
                    level.setFirstTitle(firstTitle);
                    FirstTitle first = new FirstTitle();
...
public void processTitle(Level level,Level root){
    for (int i = 0; level.getChilderen() != null && i < level.getChilderen().size(); i++) {
        Level child = level.getChilderen().get(i);
...
public CubeReportData execute(String mdx,List<ColumnProperties> cols){
