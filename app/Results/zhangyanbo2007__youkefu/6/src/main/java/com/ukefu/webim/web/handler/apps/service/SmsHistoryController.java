...
public ModelAndView index(ModelMap map,HttpServletRequest request,String tptype,String name,String phonenumber,String begin,String end){
...
@Menu(type = "callcenter", subtype = "sms")
public void expall(ModelMap map,HttpServletResponse response,HttpServletRequest request,String tptype,String name,String phonenumber,String begin,String end){
    final String orgi = super.getOrgi(request);
    Page<SmsResult> smsResultList = smsRecorRes.findAll(new Specification<SmsResult>() {
    ...
    response.setHeader("content-disposition", "attachment;filename=UCKeFu-SMS-History-" + new SimpleDateFormat("yyyy-MM-dd").format(new Date()) + ".xls");
    ExcelExporterProcess excelProcess = new ExcelExporterProcess(values, table, response.getOutputStream());
    excelProcess.process();
    return;
}
