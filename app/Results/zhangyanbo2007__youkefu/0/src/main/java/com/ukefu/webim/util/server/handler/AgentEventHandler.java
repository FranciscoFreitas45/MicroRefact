    ...
    if (agentUser != null && user != null && user.equals(agentUser.getAgentno())) {
        ...
        if (agentUserTask != null) {
            if (agentUserTask.getLastgetmessage() != null && agentUserTask.getLastmessage() != null) {
                ...
                // 坐席上次回复消息的间隔
                data.setAgentreplyinterval((int) ((System.currentTimeMillis() - agentUserTask.getLastgetmessage().getTime()) / 1000));
                // 坐席回复消息花费时间
                data.setAgentreplytime((int) ((System.currentTimeMillis() - agentUserTask.getLastmessage().getTime()) / 1000));
            }
            // 总咨询记录数量
            agentUserTask.setAgentreplys(agentUserTask.getAgentreplys() + 1);
            // 总时长
            agentUserTask.setAgentreplyinterval(agentUserTask.getAgentreplyinterval() + data.getAgentreplyinterval());
