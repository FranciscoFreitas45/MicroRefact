...
public RiverDropForm findForm(Long dropId,Long formId){
    ...
    try {
        dropForm = (RiverDropForm) em.createQuery(query).setParameter("dropId", dropId).setParameter("formId", formId).getSingleResult();
...
public boolean isRead(RiverDrop riverDrop,Account account){
    String sql = "SELECT * FROM `river_droplets_read` " + "WHERE `rivers_droplets_id` = :riverDropId " + "AND account_id = :accountId";
...
public void delete(RiverDrop t){
    String sql = String.format("DELETE FROM `river_droplets_read` WHERE `rivers_droplets_id` = %d", t.getId());
