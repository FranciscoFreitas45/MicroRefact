...
import com.metservice.kanban.DTO.KanbanJournalItem;
...
public void insertJournalEntries(CategoryDataset dataset,KanbanProject project,CategoryPlot plot,LocalDate startDate,LocalDate endDate){
    for (KanbanJournalItem journalItem : project.getJournal()) {
        // check if the item is visible on the chart
        if (!journalItem.getDate().toLocalDate().isBefore(startDate) && !journalItem.getDate().toLocalDate().isAfter(endDate)) {
            ...
            if (columnId < dataset.getColumnCount()) {
                ...
                CategoryPointerAnnotation annotation = new CategoryPointerAnnotation(text, dataset.getColumnKey(columnId), yValue, 30);
                annotation.setBaseRadius(30);
                annotation.setLabelOffset(7);
                plot.addAnnotation(annotation);
            } else {
                LOGGER.warn("Cannot display journal entry {} for day {}, not sure why", journalItem.getText(), journalItem.getDate());
            }
        }
    }
}
