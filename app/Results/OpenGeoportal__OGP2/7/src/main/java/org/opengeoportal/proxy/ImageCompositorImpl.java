...
public File writeImage(BufferedImage compositeImage){
...
public void rescale(byte[] comps,float scaleFactor){
...
public void setDimensions(Integer height,Integer width){
    // should I just throw an exception if there is a negative value?
    height = Math.abs(height);
    width = Math.abs(width);
    // number of bytes
    Long estImgSize = height.longValue() * width.longValue() * 4;
...
public Graphics2D getCompositeImage(){
    if (compositeImageGraphicsObj == null) {
...
public BufferedImage rescaleAlpha(BufferedImage indexed,float alphaFactor){
...
public void addImageToComposite(File imgFile,int opacity){
    // now we have everything we need to create a request
...
public Graphics2D createCompositeImage(int width,int height){
...
public File getOutputFile(File imageDirectory){
    File outputFile;
    do {
