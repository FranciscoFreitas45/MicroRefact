...
public void init(){
    ...
    SSLContext sc = null;
    try {
        sc = SSLContexts.custom().useTLS().setSecureRandom(new java.security.SecureRandom()).build();
 ...
 */
    ...
    client = HttpClients.custom().setConnectionManager(connectionManager).setDefaultCredentialsProvider(getCredentialsProvider()).build();
    initCalled = true;
    ...
    for (ProxyConfig config : proxyList) {
        ...
        for (ServerMapping server : serverList) {
            ...
            try {
                ...
                credsProvider.setCredentials(new AuthScope(domain, port), new UsernamePasswordCredentials(username, password));
