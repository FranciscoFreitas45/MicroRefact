...
@Value("${export.xsllink}")
 private  String xsllink;

@Value("${mt.use.ec.mt}")
 public  String useECMT;

@Value("${mt.servicewsdl}")
 private  String mtServiceWsdl;

@Value("${microsoft.translation.client.id}")
...
@ResponseBody
public SimpleResult deactivatetranslations(HttpServletRequest request,Locale locale){
...
public String adaptTranslation(String id,boolean active,HttpServletRequest request,Locale locale){
    Form form;
    try {
        form = sessionService.getForm(request, null, false, false);
    } catch (NoFormLoadedException ne) {
        logger.error(ne.getLocalizedMessage(), ne);
        return resources.getMessage("error.NoFormLoadedNew", null, "You have to load a survey before using this page!", locale);
    } catch (Exception e) {
...
public ModelAndView replace(String shortname,HttpServletRequest request,HttpServletResponse response,Locale locale){
    Form form;
...
public ModelAndView addtranslations(String shortname,HttpServletRequest request,Locale locale){
    Form form;
    form = sessionService.getForm(request, shortname, false, false);
    User u = sessionService.getCurrentUser(request);
...
public ModelAndView downloadtranslation(String shortname,String id,String format,HttpServletRequest request,HttpServletResponse response,Locale locale){
    Form form;
    form = sessionService.getForm(request, shortname, false, false);
                ...
                if (allTranslationsById.size() == 1) {
                    ...
                    if (translations.getComplete()) {
                        for (Entry<Integer, Translations> entry : allTranslationsById.entrySet()) {
                            if (!entry.getKey().equals(translations.getId()) && entry.getValue().getComplete()) {
                                completeFound = true;
                                break;
                            }
                        }
...
public ModelAndView importtranslation2(String shortname,HttpServletRequest request,HttpServletResponse response,Locale locale){
    try {
...
@ResponseBody
public SimpleResult activatetranslations(HttpServletRequest request,Locale locale){
    String id = request.getParameter("id");
    SimpleResult result = new SimpleResult();
    User u = sessionService.getCurrentUser(request);
    Translations t = translationService.getTranslations(Integer.parseInt(id));
...
public boolean isUseECMT(){
    return useECMT != null && useECMT.equalsIgnoreCase("true");
}


@PostMapping(value = "/savetranslations")
public ModelAndView savetranslations(String shortname,HttpServletRequest request,Locale locale){
    try {
        boolean dirty = false;
        ...
        for (Translations t : changedtranslations) {
            ...
            activityService.logTranslations(227, t.getLanguage().getCode(), oldInfos.get(t.getId()), t.getInfo(), sessionService.getCurrentUser(request).getId(), surveyByID.getUniqueId());
        }
        if (dirty) {
...
public ModelAndView translations(String shortname,HttpServletRequest request,Locale locale){
    User u = sessionService.getCurrentUser(request);
    initMTCheck();
    Form form;
    ...
    if (request.getParameter(Constants.ERROR) != null && request.getParameter(Constants.ERROR).equalsIgnoreCase("RequestTranslation")) {
        String message = resources.getMessage("error.RequestTranslation", null, "Request for translation failed", locale);
        result.addObject(Constants.ERROR, message);
    }
    if (request.getParameter(Constants.ERROR) != null && request.getParameter(Constants.ERROR).equalsIgnoreCase("LanguageNotRecognized")) {
...
@ResponseBody
public String importtranslation(String shortname,HttpServletRequest request,HttpServletResponse response,Locale locale){
