...
public class TranslationsHelper {
 ...
 private  int maxEmptyOdsLines;


public java.io.File getODS(Survey survey,Translations translations,FileService fileService){
    try {
...
public Translations importXML(InputStream inputStream,SurveyService surveyService){
    try {
        ...
        try {
            ...
            result.getTranslations().add(new Translation(Survey.LOGOTEXT, logoText, lang, surveyId, result));
        } catch (Exception e) {
        // ignore
        }
        try {
        ...
        for (int i = 0; i < translation.getElementsByTagName("Element").getLength(); i++) {
            ...
            if (type.contains("GalleryQuestion")) {
                ...
                for (int j = 0; j < children.getElementsByTagName("File").getLength(); j++) {
                    org.w3c.dom.Element child = (org.w3c.dom.Element) children.getElementsByTagName("File").item(j);
                    key = Tools.repairXML(child.getAttribute("key"));
...
public java.io.File getXLS(Survey survey,Translations translations,FileService fileService){
    try {
        HashMap<String, String> translationsByKey = new HashMap<>();
        for (Translation translation : translations.getTranslations()) {
            if (translation.getLabel() != null) {
...
public HashMap<String,String> getTypeSuffixMapping(){
...
public String getLabel(com.ec.survey.model.survey.base.File file,String suffix,Map<String,String> translationByKey){
    try {
        String label = "";
...
public void addTextCell(Row row,int col,String text){
...
public String serializeXml(org.w3c.dom.Node element){
...
public List<KeyValue> getLongDescriptions(Survey survey,MessageSource resources,Locale locale){
    ...
    for (Element element : survey.getElements()) {
        result.add(new KeyValue(element.getUniqueId() + Constants.SHORTNAME, resources.getMessage("label.Identifier", null, "Identifier", locale)));
        if (element instanceof Section) {
        ...
        if (element instanceof NumberQuestion) {
            ...
            if (((NumberQuestion) element).isSlider()) {
                result.add(new KeyValue(element.getUniqueId() + NumberQuestion.MINLABEL, resources.getMessage("label.MinLabel", null, "Min Label", locale)));
                result.add(new KeyValue(element.getUniqueId() + NumberQuestion.MAXLABEL, resources.getMessage("label.MaxLabel", null, "Max Label", locale)));
            }
        }
        if (element instanceof ChoiceQuestion) {
            ChoiceQuestion choice = (ChoiceQuestion) element;
        ...
        if (element instanceof GalleryQuestion) {
            GalleryQuestion gallery = (GalleryQuestion) element;
            for (com.ec.survey.model.survey.base.File child : gallery.getFiles()) {
                result.add(new KeyValue(child.getUid(), resources.getMessage("label.DescriptiveText", null, "Descriptive Text", locale)));
            }
        }
    }
    return result;
...
public String getText(NodeList elements,String name){
...
public Translations importXLS(InputStream inputStream,List<String> invalidKeys,SurveyService surveyService,ServletContext servletContext){
    try {
        Translations result = new Translations();
...
public Translations importODS(InputStream inputStream,List<String> invalidKeys,SurveyService surveyService,ServletContext servletContext){
    try {
        Translations result = new Translations();
...
public java.io.File getXML(Survey survey,Translations translations,String xsllink,FileService fileService){
    try {
...
public List<KeyValue> getShortDescriptions(Survey survey){
    List<KeyValue> result = new ArrayList<>();
...
public void synchronizePivot(Survey survey,Translations translations){
    Map<String, Translation> translationsByKey = translations.getTranslationsByKey();
    if (translationsByKey.containsKey(Survey.TITLE) && notNullOrEmpty(translationsByKey.get(Survey.TITLE).getLabel()))
        survey.setTitle(translationsByKey.get(Survey.TITLE).getLabel());
    if (translationsByKey.containsKey(Survey.LOGOTEXT) && notNullOrEmpty(translationsByKey.get(Survey.LOGOTEXT).getLabel()))
        survey.setLogoText(translationsByKey.get(Survey.LOGOTEXT).getLabel());
    if (translationsByKey.containsKey(Survey.INTRODUCTION) && notNullOrEmpty(translationsByKey.get(Survey.INTRODUCTION).getLabel()))
        survey.setIntroduction(translationsByKey.get(Survey.INTRODUCTION).getLabel());
    if (translationsByKey.containsKey(Survey.ESCAPEPAGE) && notNullOrEmpty(translationsByKey.get(Survey.ESCAPEPAGE).getLabel()))
        survey.setEscapePage(translationsByKey.get(Survey.ESCAPEPAGE).getLabel());
    if (translationsByKey.containsKey(Survey.ESCAPELINK) && notNullOrEmpty(translationsByKey.get(Survey.ESCAPELINK).getLabel()))
        survey.setEscapeLink(translationsByKey.get(Survey.ESCAPELINK).getLabel());
    if (translationsByKey.containsKey(Survey.CONFIRMATIONPAGE) && notNullOrEmpty(translationsByKey.get(Survey.CONFIRMATIONPAGE).getLabel()))
        survey.setConfirmationPage(translationsByKey.get(Survey.CONFIRMATIONPAGE).getLabel());
    if (translationsByKey.containsKey(Survey.CONFIRMATIONLINK) && notNullOrEmpty(translationsByKey.get(Survey.CONFIRMATIONLINK).getLabel()))
    ...
    for (Element element : survey.getElements()) {
        if (translationsByKey.containsKey(element.getId().toString()) && notNullOrEmpty(translationsByKey.get(element.getId().toString()).getLabel()))
            element.setTitle(translationsByKey.get(element.getId().toString()).getLabel());
        if (translationsByKey.containsKey(element.getUniqueId()) && notNullOrEmpty(translationsByKey.get(element.getUniqueId()).getLabel()))
            element.setTitle(translationsByKey.get(element.getUniqueId()).getLabel());
        if (element instanceof Question) {
            Question question = (Question) element;
            if (translationsByKey.containsKey(element.getId().toString() + "help") && notNullOrEmpty(translationsByKey.get(element.getId().toString() + "help").getLabel()))
                question.setHelp(translationsByKey.get(element.getId().toString() + "help").getLabel());
            if (translationsByKey.containsKey(element.getUniqueId() + "help") && notNullOrEmpty(translationsByKey.get(element.getUniqueId() + "help").getLabel()))
                question.setHelp(translationsByKey.get(element.getUniqueId() + "help").getLabel());
        }
        if (element instanceof Section) {
            Section section = (Section) element;
            if (translationsByKey.containsKey(element.getId().toString() + Section.TABTITLE) && notNullOrEmpty(translationsByKey.get(element.getId().toString() + Section.TABTITLE).getLabel()))
                section.setTabTitle(translationsByKey.get(element.getId().toString() + Section.TABTITLE).getLabel());
            if (translationsByKey.containsKey(element.getUniqueId() + Section.TABTITLE) && notNullOrEmpty(translationsByKey.get(element.getUniqueId() + Section.TABTITLE).getLabel()))
                section.setTabTitle(translationsByKey.get(element.getUniqueId() + Section.TABTITLE).getLabel());
        }
        if (element instanceof NumberQuestion) {
            NumberQuestion number = (NumberQuestion) element;
            if (translationsByKey.containsKey(element.getId().toString() + NumberQuestion.UNIT) && notNullOrEmpty(translationsByKey.get(element.getId().toString() + NumberQuestion.UNIT).getLabel()))
                number.setUnit(translationsByKey.get(element.getId().toString() + NumberQuestion.UNIT).getLabel());
            if (translationsByKey.containsKey(element.getUniqueId() + NumberQuestion.UNIT) && notNullOrEmpty(translationsByKey.get(element.getUniqueId() + NumberQuestion.UNIT).getLabel()))
        ...
        if (element instanceof Confirmation) {
            Confirmation confirmation = (Confirmation) element;
            if (translationsByKey.containsKey(element.getId().toString() + Confirmation.TEXT) && notNullOrEmpty(translationsByKey.get(element.getId().toString() + Confirmation.TEXT).getLabel()))
                confirmation.setConfirmationtext(translationsByKey.get(element.getId().toString() + Confirmation.TEXT).getLabel());
            if (translationsByKey.containsKey(element.getUniqueId() + Confirmation.TEXT) && notNullOrEmpty(translationsByKey.get(element.getUniqueId() + Confirmation.TEXT).getLabel()))
                confirmation.setConfirmationtext(translationsByKey.get(element.getUniqueId() + Confirmation.TEXT).getLabel());
            if (translationsByKey.containsKey(element.getId().toString() + Confirmation.LABEL) && notNullOrEmpty(translationsByKey.get(element.getId().toString() + Confirmation.LABEL).getLabel()))
                confirmation.setConfirmationlabel(translationsByKey.get(element.getId().toString() + Confirmation.LABEL).getLabel());
            if (translationsByKey.containsKey(element.getUniqueId() + Confirmation.LABEL) && notNullOrEmpty(translationsByKey.get(element.getUniqueId() + Confirmation.LABEL).getLabel()))
                confirmation.setConfirmationlabel(translationsByKey.get(element.getUniqueId() + Confirmation.LABEL).getLabel());
        }
        if (element instanceof ChoiceQuestion) {
            ChoiceQuestion choice = (ChoiceQuestion) element;
            for (PossibleAnswer answer : choice.getPossibleAnswers()) {
                if (translationsByKey.containsKey(answer.getId().toString()) && notNullOrEmpty(translationsByKey.get(answer.getId().toString()).getLabel()))
                    answer.setTitle(translationsByKey.get(answer.getId().toString()).getLabel());
                if (translationsByKey.containsKey(answer.getUniqueId()) && notNullOrEmpty(translationsByKey.get(answer.getUniqueId()).getLabel()))
                    answer.setTitle(translationsByKey.get(answer.getUniqueId()).getLabel());
            }
        }
        if (element instanceof Matrix) {
            Matrix matrix = (Matrix) element;
            if (translationsByKey.containsKey(element.getId().toString() + MatrixOrTable.FIRSTCELL) && notNullOrEmpty(translationsByKey.get(element.getId().toString() + MatrixOrTable.FIRSTCELL).getLabel()))
                matrix.setFirstCellText(translationsByKey.get(element.getId().toString() + MatrixOrTable.FIRSTCELL).getLabel());
            if (translationsByKey.containsKey(element.getUniqueId() + MatrixOrTable.FIRSTCELL) && notNullOrEmpty(translationsByKey.get(element.getUniqueId() + MatrixOrTable.FIRSTCELL).getLabel()))
                matrix.setFirstCellText(translationsByKey.get(element.getUniqueId() + MatrixOrTable.FIRSTCELL).getLabel());
            for (Element child : matrix.getChildElements()) {
                if (child instanceof Text) {
                    if (translationsByKey.containsKey(child.getId().toString()) && notNullOrEmpty(translationsByKey.get(child.getId().toString()).getLabel()))
                        child.setTitle(translationsByKey.get(child.getId().toString()).getLabel());
                    if (translationsByKey.containsKey(child.getUniqueId()) && notNullOrEmpty(translationsByKey.get(child.getUniqueId()).getLabel()))
        ...
        if (element instanceof Table) {
            Table table = (Table) element;
            if (translationsByKey.containsKey(element.getId().toString() + MatrixOrTable.FIRSTCELL) && notNullOrEmpty(translationsByKey.get(element.getId().toString() + MatrixOrTable.FIRSTCELL).getLabel()))
                table.setFirstCellText(translationsByKey.get(element.getId().toString() + MatrixOrTable.FIRSTCELL).getLabel());
            if (translationsByKey.containsKey(element.getUniqueId() + MatrixOrTable.FIRSTCELL) && notNullOrEmpty(translationsByKey.get(element.getUniqueId() + MatrixOrTable.FIRSTCELL).getLabel()))
                table.setFirstCellText(translationsByKey.get(element.getUniqueId() + MatrixOrTable.FIRSTCELL).getLabel());
            for (Element child : table.getChildElements()) {
                if (child instanceof Text) {
                    if (translationsByKey.containsKey(child.getId().toString()) && notNullOrEmpty(translationsByKey.get(child.getId().toString()).getLabel()))
                        child.setTitle(translationsByKey.get(child.getId().toString()).getLabel());
        ...
        if (element instanceof GalleryQuestion) {
            GalleryQuestion gallery = (GalleryQuestion) element;
            for (com.ec.survey.model.survey.base.File child : gallery.getFiles()) {
                if (translationsByKey.containsKey(child.getUid()) && notNullOrEmpty(translationsByKey.get(child.getUid()).getLabel()))
...
public org.w3c.dom.Element getFileNode(com.ec.survey.model.survey.base.File element,Document doc,HashMap<String,String> translationByKey){
...
public org.w3c.dom.Element getElementNode(Element element,Document doc,HashMap<String,String> translationByKey){
...
public String getODSText(org.odftoolkit.simple.table.Table sheet,int row,int column,String key,List<String> invalidKeys,ServletContext servletContext){
...
public Translations removeIDs(Translations result){
...
public Translations getTranslations(Survey survey,boolean complete){
    ...
    if (complete || notNullOrEmpty(survey.getLogoText())) {
        translations.getTranslations().add(new Translation(Survey.LOGOTEXT, survey.getLogoText() != null ? survey.getLogoText() : "", survey.getLanguage().getCode(), survey.getId(), translations));
    }
    ...
    for (Element element : survey.getElements()) {
        if (element.getUniqueId() != null) {
            ...
            if (element instanceof NumberQuestion) {
                ...
                if (number.isSlider()) {
                    translations.getTranslations().add(new Translation(number.getUniqueId() + NumberQuestion.MINLABEL, number.getMinLabel() != null ? number.getMinLabel() : "", survey.getLanguage().getCode(), survey.getId(), translations));
                    translations.getTranslations().add(new Translation(number.getUniqueId() + NumberQuestion.MAXLABEL, number.getMaxLabel() != null ? number.getMaxLabel() : "", survey.getLanguage().getCode(), survey.getId(), translations));
                }
            }
            if (element instanceof ChoiceQuestion) {
                ChoiceQuestion choice = (ChoiceQuestion) element;
            ...
            if (element instanceof GalleryQuestion) {
                GalleryQuestion gallery = (GalleryQuestion) element;
                for (com.ec.survey.model.survey.base.File child : gallery.getFiles()) {
                    translations.getTranslations().add(new Translation(child.getUid(), child.getDescription() != null ? child.getDescription() : "", survey.getLanguage().getCode(), survey.getId(), translations));
                }
            }
        } else {
...
public void synchronize(Translations existingTranslations,Translations translations){
...
public boolean isComplete(Translations translations,Survey survey)// elements. Check also all elements are
...
// similar helper functions above
{
    try {
        ...
        if ((!translationMap.containsKey(Survey.TITLE) || translationMap.get(Survey.TITLE) == null || translationMap.get(Survey.TITLE).trim().length() == 0))
            return false;
        if (survey.getLogoText() != null && survey.getLogoText().trim().length() > 0 && (!translationMap.containsKey(Survey.LOGOTEXT) || translationMap.get(Survey.LOGOTEXT).trim().length() == 0))
            return false;
        if (survey.getIntroduction() != null && survey.getIntroduction().trim().length() > 0 && (!translationMap.containsKey(Survey.INTRODUCTION) || translationMap.get(Survey.INTRODUCTION).trim().length() == 0))
            return false;
        if (survey.getEscapePage() != null && survey.getEscapePage().trim().length() > 0 && (!translationMap.containsKey(Survey.ESCAPEPAGE) || translationMap.get(Survey.ESCAPEPAGE).trim().length() == 0))
            return false;
        if (survey.getEscapeLink() != null && survey.getEscapeLink().trim().length() > 0 && (!translationMap.containsKey(Survey.ESCAPELINK) || translationMap.get(Survey.ESCAPELINK).trim().length() == 0))
            return false;
        if (survey.getConfirmationPage() != null && survey.getConfirmationPage().trim().length() > 0 && (!translationMap.containsKey(Survey.CONFIRMATIONPAGE) || translationMap.get(Survey.CONFIRMATIONPAGE).trim().length() == 0))
            return false;
        if (survey.getConfirmationLink() != null && survey.getConfirmationLink().trim().length() > 0 && (!translationMap.containsKey(Survey.CONFIRMATIONLINK) || translationMap.get(Survey.CONFIRMATIONLINK).trim().length() == 0))
            return false;
        for (Element element : survey.getElementsRecursive()) {
            if (!(element instanceof EmptyElement) && element.getTitle() != null) {
                ...
                if (element instanceof Question) {
                    ...
                    if (question instanceof FreeTextQuestion && getLabel(question, "", translationMap).length() == 0) {
                        return false;
                    }
                }
                if (element instanceof MatrixOrTable) {
                    MatrixOrTable matrix = (MatrixOrTable) element;
                    if (matrix.getFirstCellText() != null && matrix.getFirstCellText().trim().length() > 0 && getLabel(matrix, MatrixOrTable.FIRSTCELL, translationMap).length() == 0) {
                        return false;
                    }
                    for (Element child : matrix.getChildElements()) {
