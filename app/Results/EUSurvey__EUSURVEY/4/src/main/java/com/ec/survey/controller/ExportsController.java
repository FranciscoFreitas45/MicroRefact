...
import com.ec.survey.DTO.SessionInfo;
import com.ec.survey.DTO.MailTask;
@Controller
...
public ModelAndView recreateExport(String exportIdList,HttpServletRequest request,Locale locale){
    ...
    for (String exportId : exportIds) {
        ...
        if (export != null) {
            if (!(sessionService.checkUser(export.getUserId(), request) || sessionService.getCurrentUser(request).getGlobalPrivileges().get(GlobalPrivilege.FormManagement).equals(2))) {
                return new ModelAndView(Constants.VIEW_ERROR_GENERIC, Constants.MESSAGE, resources.getMessage("message.AccessDenied", null, "Access denied", locale));
            }
            exports.add(export);
        }
    }
...
@ResponseBody
public String checkNew(String uid,HttpServletRequest request,HttpServletResponse response){
...
@ResponseBody
public String deleteExport(int exportId,HttpServletRequest request){
    try {
...
@ResponseBody
public String checkNewMails(HttpServletRequest request,HttpServletResponse response,Locale locale){
...
@ResponseBody
public String startExport(String exportName,String allAnswers,String showShortnames,String type,String format,String group,HttpServletRequest request,HttpServletResponse response,Locale locale){
    try {
...
@ResponseBody
public List<Export> exportsjson(HttpServletRequest request){
    int itemsPerPage = -1;
    int page = -1;
    String sortKey = "date";
    String sortOrder = "DESC";
    ...
    if (user.getGlobalPrivileges().get(GlobalPrivilege.FormManagement).equals(2)) {
        exports = exportService.getExports(-1, sortKey, sortOrder.equalsIgnoreCase("asc"), page, itemsPerPage, true, true, false, true);
    } else {
        exports = exportService.getExports(user.getId(), sortKey, sortOrder.equalsIgnoreCase("asc"), page, itemsPerPage, true, true, false, true);
    }
    return exports;
}
...
@ResponseBody
public ResponseEntity<byte[]> downloadExport(int exportId,HttpServletRequest request,HttpServletResponse response){
