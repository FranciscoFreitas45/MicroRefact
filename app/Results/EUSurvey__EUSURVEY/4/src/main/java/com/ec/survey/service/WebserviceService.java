...
import com.ec.survey.DTO.ServiceRequest;
@Service("webserviceService")
@Configurable
public class WebserviceService extends BasicService{


public void internalSave(WebserviceTask task){
...
public void increaseServiceRequest(Integer userId){
    ...
    if (disablewebservicelimit != null && disablewebservicelimit.equalsIgnoreCase("true"))
        return;
    Session session = sessionFactory.getCurrentSession();
...
@Transactional
public void setDone(WebserviceTask task){
    boolean saved = false;
    int counter = 1;
    while (!saved) {
        try {
...
public Date internalSetStarted(int taskid){
    Session session;
    session = sessionFactory.getCurrentSession();
...
public void internalSetDone(WebserviceTask task){
    Session session;
    session = sessionFactory.getCurrentSession();
...
@Transactional
public void setError(int task,String error){
    boolean saved = false;
    int counter = 1;
    while (!saved) {
        try {
...
@Transactional
public Date setStarted(int taskid){
    int counter = 1;
    while (true) {
        try {
