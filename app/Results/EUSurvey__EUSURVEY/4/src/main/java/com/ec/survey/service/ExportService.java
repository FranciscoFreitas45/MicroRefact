...
@Value("${export.deleteexportstimeout}")
 private  String deleteexportstimeout;
...
public boolean startExport(Form form,Export export,boolean immediate,MessageSource resources,Locale locale,String uid,String exportFilePath,boolean skipcheckworkerserver){
    try {
        ...
        if (exportFilePath == null)
            exportFilePath = getExportFilePath(export, uid);
        ExportCreator exportCreator = null;
        switch(export.getFormat()) {
            case doc:
                exportCreator = (DocExportCreator) context.getBean("docExportCreator");
                break;
            case xls:
                exportCreator = (XlsExportCreator) context.getBean("xlsExportCreator");
                break;
            case odt:
            case ods:
                exportCreator = (OdfExportCreator) context.getBean("odfExportCreator");
                break;
            case csv:
                exportCreator = (CsvExportCreator) context.getBean("csvExportCreator");
                break;
            case xml:
                exportCreator = (XmlExportCreator) context.getBean("xmlExportCreator");
                break;
            case pdf:
                exportCreator = (PdfExportCreator) context.getBean("pdfExportCreator");
                break;
            case zip:
                exportCreator = (ZipExportCreator) context.getBean("zipExportCreator");
                break;
            case eus:
                exportCreator = (EusExportCreator) context.getBean("eusExportCreator");
                break;
            default:
                throw new MessageException("Export format not supported");
        }
        if (user == null) {
...
public void deleteExportFiles(Export export){
...
public void determineValidState(Export export,boolean evict){
    try {
        if (export != null) {
                        ...
                        if (element instanceof ChoiceQuestion || element instanceof Text) {
                            ...
                            for (String svalId : values) {
                                ...
                                if (oldUniqueIdsById.containsKey(valId) && newIdsByUniqueId.containsKey(oldUniqueIdsById.get(valId))) {
                                    ...
                                    // this should not happen but we keep the original id if we cannot find a new one
                                    newValues.append(valId);
                                }
                            }
                        }
                    }
...
public String getExportPathWithSuffix(String exportFilePath,String ext,int fileCounter){
