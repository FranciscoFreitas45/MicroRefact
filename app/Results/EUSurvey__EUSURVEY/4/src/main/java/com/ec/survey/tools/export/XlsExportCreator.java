...
import com.ec.survey.DTO.ECFGlobalResult;
import com.ec.survey.DTO.ECFGlobalCompetencyResult;
import com.ec.survey.DTO.Publication;
...
import com.ec.survey.DTO.ECFProfileCompetencyResult;
import com.ec.survey.DTO.ECFProfileResult;
import com.ec.survey.DTO.NumberQuestion;
import com.ec.survey.DTO.RatingQuestion;
import com.ec.survey.DTO.ECFOrganizationalCompetencyResult;
import com.ec.survey.DTO.ECFOrganizationalResult;
...
public class XlsExportCreator extends ExportCreator{
 ...
 private CellStyle questionTitleStyle;
...
public void checkColumnInsertHeader(Export export){
...
public void exportOneECFGlobalResultSheet(ECFGlobalResult globalResult){
    String exportSheetName = resources.getMessage("label.ECF.Results", null, "Individual Assessment Results and gaps", locale);
    ...
    boolean displayGap = (globalResult.getTotalResults().getTotalGaps() != null && globalResult.getTotalResults().getTotalScores().size() == globalResult.getTotalResults().getTotalGaps().size());
    ...
    for (ECFGlobalCompetencyResult competencyResult : globalResult.getIndividualResults()) {
        ...
        displayGap = displayGap && (competencyResult.getCompetencyScoreGaps() != null && competencyResult.getCompetencyScoreGaps().size() == competencyResult.getCompetencyScores().size());
...
public void enableLineBreaksInCell(Cell cell){
    if (cswrap == null) {
...
public void CreateTableForAnswer(String title,CellStyle boldstyle){
    // TODO: bold
...
public void exportContent(Publication publication,boolean sync){
    ...
    if (export == null || export.isForArchiving() == null || !export.isForArchiving()) {
        answersets = reportingService.getAnswerSets(survey, filter, null, false, true, publication == null || publication.getShowUploadedDocuments(), false, false, export != null && export.getShowShortnames());
    }
    List<Question> questions = form.getSurvey().getQuestions();
    if (answersets != null) {
        for (List<String> row : answersets) {
            parseAnswerSet(null, row, publication, filter, filesByAnswer, export, questions, uploadedFilesByCodeAndQuestionUID, uploadQuestionNicenames, null, null, explanationFilesOfSurvey, explanationFilesToExport);
        }
    } else {
        ...
        try {
            ...
            while (results.next()) {
                ...
                if (lastAnswerSet == answer.getAnswerSetId()) {
                    ...
                    if (lastAnswerSet > 0) {
                        session.flush();
                        parseAnswerSet(answerSet, null, publication, filter, filesByAnswer, export, questions, uploadedFilesByCodeAndQuestionUID, uploadQuestionNicenames, explanations, discussions, explanationFilesOfSurvey, explanationFilesToExport);
                    }
    ...
    if (fileCounter > 0 || !uploadedFiles.isEmpty() || explanationFilesToExport.hasFiles()) {
        ...
        try {
            ...
            for (String code : uploadedFilesByCodeAndQuestionUID.keySet()) {
                for (String questionNiceName : uploadedFilesByCodeAndQuestionUID.get(code).keySet()) {
                    for (File file : uploadedFilesByCodeAndQuestionUID.get(code).get(questionNiceName)) {
                        ...
                        if (f.exists()) {
                            os.putArchiveEntry(new ZipArchiveEntry(code + Constants.PATH_DELIMITER + questionNiceName + Constants.PATH_DELIMITER + file.getName()));
            ...
            explanationFilesToExport.applyFunctionOnEachFile((answerSetId, questionUid, explanationFile) -> {
                ...
                if (file.exists()) {
                    os.putArchiveEntry(new ZipArchiveEntry(answerSetId + Constants.PATH_DELIMITER + questionUid + Constants.PATH_DELIMITER + explanationFile.getName()));
...
public void exportECFProfileResults(){
    ...
    String exportSheetName = resources.getMessage("label.ECF.Results2", null, "Profile assessment results and gaps", locale);
...
public Cell checkColumnsParseAnswerSet(){
...
public void parseAnswerSet(AnswerSet answerSet,List<String> answerrow,Publication publication,ResultFilter filter,Map<Integer,List<File>> filesByAnswer,Export export,List<Question> questions,Map<String,Map<String,List<File>>> uploadedFilesByContributionIDAndQuestionUID,Map<String,String> uploadQuestionNicenames,Map<Integer,Map<String,String>> explanations,Map<Integer,Map<String,String>> discussions,FilesByTypes<Integer,String> explanationFilesOfSurvey,FilesByTypes<String,String> explanationFilesToExport){
    CreationHelper createHelper = wb.getCreationHelper();
    // Excel older than 2007 has a limit on the number of rows
    if (// 65000 == 0)
    rowIndex > 0 && rowIndex % 20000 == 0) {
        fileCounter++;
        wb.write(outputStream);
        outputStream.close();
        wb.close();
    ...
    // the first item is the answerset code, the second one the id
    int answerrowcounter = 2;
    for (Question question : questions) {
        if ((publication != null || filter.exported(question.getId().toString())) && (publication == null || publication.isAllQuestions() || publication.isSelected(question.getId())) && question.isUsedInResults())
                        ...
                        if (sfiles != null && sfiles.length() > 0) {
                            ...
                            for (String sfile : files) {
                                if (sfile.length() > 0) {
                                    ...
                                    linkValue = answerrow.get(0) + Constants.PATH_DELIMITER + uploadQuestionNicenames.get(question.getUniqueId());
                                }
                            }
                        }
                    } else {
                        ...
                        for (Answer answer : answers) {
                            if (filesByAnswer.containsKey(answer.getId())) {
                                for (File file : filesByAnswer.get(answer.getId())) {
                                    ...
                                    cellValue.append((cellValue.length() > 0) ? ";" : "").append(file.getName());
                                    linkValue = answerSet.getUniqueCode() + Constants.PATH_DELIMITER + uploadQuestionNicenames.get(question.getUniqueId());
                                }
                            }
                        }
                    }
        ...
        if (question.isDelphiElement() && filter.explanationExported(question.getId().toString())) {
            ...
            if (answerSet == null) {
                ...
                explanation = ExportCreatorHelper.retrieveExplanationWithFilesFromReportingAnswer(answerrow.get(answerrowcounter++), answerSetUid, questionUid, explanationFilesToExport);
            } else {
                answerSetUid = answerSet.getUniqueCode();
                explanation = ExportCreatorHelper.retrieveExplanationWithFilesFromAnswerSetAndExistingFiles(answerSet, explanations, questionUid, explanationFilesOfSurvey, explanationFilesToExport);
            }
...
public void exportECFOrganizationalResults(){
    ...
    String exportSheetName = resources.getMessage("label.ECF.Results3", null, "Average and maximum score for all profiles", locale);
    safeName = WorkbookUtil.createSafeSheetName(exportSheetName);
    sheet = wb.createSheet(safeName);
    sheets.add(sheet);
    // TABLE HEADER
...
public void drawChart(double percent,CreationHelper helper,Drawing drawing){
...
public int insertHeader(List<Sheet> sheets,Publication publication,ResultFilter filter,Export export){
...
public void linkCell(Cell cell,String linkValue,CreationHelper createHelper){
...
public int initWorkbook(Sheet sheet,Export export){
    sheet.setColumnWidth(0, 5000);
            ...
            if (question instanceof ChoiceQuestion) {
                ...
                if (number.showStatisticsForNumberQuestion()) {
                    ...
                    for (String answer : number.getAllPossibleAnswers()) {
                        ...
                        row.createCell(2).setCellValue(statistics.getRequestedRecords().get(number.getAnswerWithPrefix(answer)));
                        Cell pcell = row.createCell(3);
                        pcell.setCellValue(percent / 100);
                        pcell.setCellStyle(percentStyle);
                    }
