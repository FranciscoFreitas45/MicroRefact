...
@Autowired
 ...
 private org.odftoolkit.simple.table.Table sheet;
...
public int InitSpreadsheet(org.odftoolkit.simple.table.Table sheet){
    int rowIndex = 0;
...
public org.odftoolkit.simple.table.Table CreateTableForAnswer(TextDocument document,String title){
...
public void ExportContent(Publication publication,boolean sync){
    ...
    List<List<String>> answersets = reportingService.getAnswerSets(survey, filter, null, false, true, publication == null || publication.getShowUploadedDocuments(), false, false, export != null && export.getShowShortnames());
    if (answersets != null) {
        for (List<String> row : answersets) {
            parseAnswerSet(null, row, publication, filter, filesByAnswer, export, uploadedFilesByCodeAndQuestionUID, uploadQuestionNicenames, null, null, explanationFilesOfSurvey, explanationFilesToExport);
        }
    } else {
        ...
        while (results.next()) {
            ...
            if (lastAnswerSet == answer.getAnswerSetId()) {
                ...
                if (lastAnswerSet > 0) {
                    parseAnswerSet(answerSet, null, publication, filter, filesByAnswer, export, uploadedFilesByCodeAndQuestionUID, uploadQuestionNicenames, explanations, discussions, explanationFilesOfSurvey, explanationFilesToExport);
                    session.flush();
                }
...
public int CreateChart(SpreadsheetDocument spreadsheet,String title,String[] labels,double[] stats,int xPos){
...
public void drawImage(double percent){
...
public void initHeader(Publication publication,ResultFilter filter,Export export){
    String cellValue;
...
public void CreateTableForAnswerStat(String title){
    cell = sheet.getCellByPosition(0, rowIndex);
...
public void parseAnswerSet(AnswerSet answerSet,List<String> answerrow,Publication publication,ResultFilter filter,Map<Integer,List<File>> filesByAnswer,Export export,Map<String,Map<String,List<File>>> uploadedFilesByContributionIDAndQuestionUID,Map<String,String> uploadQuestionNicenames,Map<Integer,Map<String,String>> explanations,Map<Integer,Map<String,String>> discussions,FilesByTypes<Integer,String> explanationFilesOfSurvey,FilesByTypes<String,String> explanationFilesToExport){
    rowIndex++;
    columnIndex = 0;
                        ...
                        if (publication == null || publication.getShowUploadedDocuments()) {
                            ...
                            if (answerSet == null) {
                                ...
                                if (sfiles != null && sfiles.length() > 0) {
                                    ...
                                    p.appendHyperlink(cellValue.toString(), new URI("../" + answerrow.get(0) + Constants.PATH_DELIMITER + uploadQuestionNicenames.get(question.getUniqueId())));
                                }
                            } else {
                                ...
                                Paragraph p = cell.addParagraph("");
                                p.appendHyperlink(cellValue.toString(), new URI("../" + answerSet.getUniqueCode() + Constants.PATH_DELIMITER + uploadQuestionNicenames.get(question.getUniqueId())));
                            }
                        }
                    } else if (question instanceof Table) {
                        Table table = (Table) question;
        ...
        if (question.isDelphiElement() && filter.explanationExported(question.getId().toString())) {
            ...
            if (!files.isEmpty()) {
                ...
                while (fileIterator.hasNext()) {
                    ...
                    p.appendHyperlink(file.getNameForExport(), new URI("../" + answerSetUid + Constants.PATH_DELIMITER + questionUid + Constants.PATH_DELIMITER + file.getNameForExport()));
...
public void drawChart(double percent,Row row){
...
public void ExportStatisticsODS(){
    SpreadsheetDocument spreadsheet = SpreadsheetDocument.newSpreadsheetDocument();
    sheet = spreadsheet.getSheetByIndex(0);
            ...
            if (question instanceof ChoiceQuestion) {
                ...
                if (numberQuestion.showStatisticsForNumberQuestion()) {
                    ...
                    for (String answer : numberQuestion.getAllPossibleAnswers()) {
                        ...
                        cell = sheet.getCellByPosition(2, rowIndex);
                        cell.setDoubleValue((double) statistics.getRequestedRecords().get(numberQuestion.getAnswerWithPrefix(answer)));
                        cell.setDisplayText(statistics.getRequestedRecords().get(numberQuestion.getAnswerWithPrefix(answer)).toString());
                        cell = sheet.getCellByPosition(3, rowIndex);
                        cell.setPercentageValue(percent);
                        cell.setDisplayText(df.format(percent) + "%");
                    }
            ...
            if (question instanceof ChoiceQuestion) {
                ...
                if (numberQuestion.showStatisticsForNumberQuestion()) {
                    ...
                    for (String answer : numberQuestion.getAllPossibleAnswers()) {
                        ...
                        row.getCellByIndex(2).setDoubleValue((double) statistics.getRequestedRecords().get(numberQuestion.getAnswerWithPrefix(answer)));
                        row.getCellByIndex(2).setStringValue(Integer.toString(statistics.getRequestedRecords().get(numberQuestion.getAnswerWithPrefix(answer))));
                        row.getCellByIndex(3).setPercentageValue(percent);
                        row.getCellByIndex(3).setStringValue(df.format(percent) + "%");
                    }
                    Row row = table.appendRow();
