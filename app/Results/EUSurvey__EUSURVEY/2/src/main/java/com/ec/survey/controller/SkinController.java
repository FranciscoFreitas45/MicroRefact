...
public ModelAndView skins(HttpServletRequest request,Locale locale){
    ...
    if (request.getParameter("used") != null && request.getParameter("used").equalsIgnoreCase("true")) {
        result.addObject(Constants.MESSAGE, resources.getMessage("error.SkinInUse", null, "This skin is being used in a survey and can therefore not be deleted.", locale));
    }
    return result;
}
...
public ModelAndView download(String id,Locale locale,HttpServletRequest request,HttpServletResponse response){
    User user = sessionService.getCurrentUser(request);
    ...
    if (skinId > 0) {
        ...
        if (skin.getIsPublic() || skin.getOwner().getId().equals(user.getId()) || user.getGlobalPrivileges().get(GlobalPrivilege.FormManagement) == 2) {
            ...
            return new ModelAndView(Constants.VIEW_ERROR_GENERIC, Constants.MESSAGE, resources.getMessage("error.SkinDownloadUnauthorized", null, "You are not authorized to download this skin", locale));
        }
    }
    return null;
}
...
public ModelAndView deleteSkin(String id,Locale locale,HttpServletRequest request){
    User user = sessionService.getCurrentUser(request);
...
public ModelAndView copySkin(String id,Locale locale,HttpServletRequest request){
    User user = sessionService.getCurrentUser(request);
...
public ModelAndView editSkin(String id,Locale locale,HttpServletRequest request){
    User user = sessionService.getCurrentUser(request);
    Survey demoSurvey = surveyService.getSurvey("SkinDemo", true, false, false, false, null, true, false);
    Form form = new Form(resources);
    form.setSurvey(demoSurvey);
...
public ModelAndView saveSkin(Skin skin,BindingResult bindingresult,HttpServletRequest request,Locale locale){
    User user = sessionService.getCurrentUser(request);
    ...
    if (skin.getId() != null && skin.getId() > 0) {
        ...
        if (!existingSkin.getOwner().getId().equals(user.getId()) && user.getGlobalPrivileges().get(GlobalPrivilege.FormManagement) < 2) {
            return new ModelAndView(Constants.VIEW_ERROR_GENERIC, Constants.MESSAGE, resources.getMessage("error.SkinUnauthorized", null, "You are not authorized to edit this skin.", locale));
        }
        if (!existingSkin.getName().equalsIgnoreCase(skin.getName()) && skinService.nameAlreadyExists(skin.getName(), user.getId())) {
            Survey demoSurvey = surveyService.getSurvey("SkinDemo", true, false, false, false, null, true, false);
            Form form = new Form(resources);
            form.setSurvey(demoSurvey);
            ModelAndView model = new ModelAndView("settings/skin");
            model.addObject("skin", skin);
            model.addObject("form", form);
            model.addObject(Constants.MESSAGE, resources.getMessage("error.NameAlreadyUsed", null, "This name already exists. Please choose a unique one.", locale));
            return model;
        }
    ...
    if (skin.getName().trim().length() == 0) {
        ...
        model.addObject("skin", skin);
        model.addObject("form", form);
        model.addObject(Constants.MESSAGE, resources.getMessage("error.ChoseName", null, "Please choose a unique name.", locale));
        return model;
    }
