...
public Object getCss(boolean forPDF){
    ...
    if (backgroundColor != null && backgroundColor.length() > 0) {
        ...
        if (!backgroundColor.startsWith("#") && !backgroundColor.equals("transparent"))
            s.append("#");
    ...
    if (forPDF) {
        ...
        if (fontFamilyPDF != null && fontFamilyPDF.length() > 0)
            s.append("font-family: ").append(fontFamilyPDF).append("; \n");
        if (fontStylePDF != null && fontStylePDF.length() > 0)
            s.append("font-style: ").append(fontStylePDF).append("; \n");
    } else {
        if (fontFamily != null && fontFamily.length() > 0)
            s.append("font-family: ").append(fontFamily).append("; \n");
        if (fontStyle != null && fontStyle.length() > 0)
            s.append("font-style: ").append(fontStyle).append("; \n");
    }
    if (fontSize != null && fontSize.length() > 0)
        s.append("font-size: ").append(fontSize).append("; \n");
    if (fontWeight != null && fontWeight.length() > 0)
        s.append("font-weight: ").append(fontWeight).append("; \n");
    s.append("} ");
    s.append("\n\n");
    return s.toString();
}
