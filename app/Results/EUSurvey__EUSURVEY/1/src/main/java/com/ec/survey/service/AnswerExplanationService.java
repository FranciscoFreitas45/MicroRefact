...
public void createUpdateOrDeleteExplanations(AnswerSet answerSet){
    ...
    for (Question question : survey.getQuestions()) {
        ...
        if (question.getIsDelphiQuestion()) {
            ...
            if (explanationData.text != null && explanationData.text.length() > 0 && !explanationData.text.equalsIgnoreCase(explanation.getText()) && !answers.isEmpty()) {
...
@Transactional
public void deleteCommentsForDeletedAnswers(AnswerSet answerSet){
    final Session session = sessionFactory.getCurrentSession();
    for (Question question : answerSet.getSurvey().getQuestions()) {
        if (question.getIsDelphiQuestion()) {
            ...
            final Query replyDeletionQuery = session.createQuery("DELETE FROM AnswerComment " + "WHERE answerSetId = :answerSetId AND questionUid = :questionUid AND parent IS NOT NULL").setInteger("answerSetId", answerSet.getId()).setString("questionUid", question.getUniqueId());
            replyDeletionQuery.executeUpdate();
            final Query commentDeletionQuery = session.createQuery("DELETE FROM AnswerComment " + "WHERE answerSetId = :answerSetId AND questionUid = :questionUid AND parent IS NULL").setInteger("answerSetId", answerSet.getId()).setString("questionUid", question.getUniqueId());
...
@Transactional
public void deleteExplanationFilesOfSurvey(int surveyId){
    final Session session = sessionFactory.getCurrentSession();
    final Query explanationFilesRetrievalQuery = session.createSQLQuery("SELECT f.FILE_UID " + "FROM FILES f " + "JOIN ANSWERS_EXPLANATIONS_FILES aef ON aef.files_FILE_ID = f.FILE_ID " + "JOIN ANSWERS_EXPLANATIONS ae ON ae.ANSWER_EXPLANATION_ID = aef.ANSWERS_EXPLANATIONS_ANSWER_EXPLANATION_ID " + "JOIN ANSWERS_SET ans ON ans.ANSWER_SET_ID = ae.ANSWER_SET_ID " + "WHERE ans.SURVEY_ID = :id").setInteger("id", surveyId);
    final List<String> fileUids = explanationFilesRetrievalQuery.list();
    final Query explanationFilesDeletionQuery12 = session.createSQLQuery("DELETE aef.* " + "FROM ANSWERS_EXPLANATIONS_FILES aef " + "JOIN ANSWERS_EXPLANATIONS ae ON ae.ANSWER_EXPLANATION_ID = aef.ANSWERS_EXPLANATIONS_ANSWER_EXPLANATION_ID " + "JOIN ANSWERS_SET ans ON ans.ANSWER_SET_ID = ae.ANSWER_SET_ID " + "WHERE ans.SURVEY_ID = :id").setInteger("id", surveyId);
    explanationFilesDeletionQuery12.executeUpdate();
    if (fileUids.isEmpty())
        return;
    final Query explanationFilesDeletionQuery2 = session.createSQLQuery("DELETE f.* " + "FROM FILES f " + "WHERE f.FILE_UID IN :ids").setParameterList("ids", fileUids);
    explanationFilesDeletionQuery2.executeUpdate();
}


@Transactional
public void deleteExplanationsOfSurvey(int surveyId){
    final Session session = sessionFactory.getCurrentSession();
    final Query query = session.createSQLQuery("DELETE ae.* " + "FROM ANSWERS_EXPLANATIONS ae " + "JOIN ANSWERS_SET an ON ae.ANSWER_SET_ID = an.ANSWER_SET_ID " + "WHERE an.SURVEY_ID = :id").setInteger("id", surveyId);
    query.executeUpdate();
}
...
@Transactional
public void removeFileFromExplanation(int answerSetId,String questionUid,String fileUid){
    final Session session = sessionFactory.getCurrentSession();
...
public List<AnswerComment> loadComments(int answerSetId,String questionUid){
    final Session session = sessionFactory.getCurrentSession();
    Query query = session.createQuery("FROM AnswerComment WHERE answerSetId = :answerSetId and questionUid = :questionUid ORDER BY id");
...
public void deleteExplanationByAnswerSet(AnswerSet answerSet){
    ...
    final Session session = sessionFactory.getCurrentSession();
    final Query query = session.createQuery("DELETE FROM AnswerExplanation WHERE answerSetId = :answerId").setInteger("answerId", answerSetId);
    query.executeUpdate();
}
...
@Transactional
public void deleteCommentsOfSurvey(int surveyId){
    final Session session = sessionFactory.getCurrentSession();
    final Query replyDeletionQuery = session.createSQLQuery("DELETE ac.* " + "FROM ANSWERS_COMMENTS ac " + "JOIN ANSWERS_SET an ON an.ANSWER_SET_ID = ac.ANSWER_SET_ID " + "WHERE ac.PARENT IS NOT NULL AND an.SURVEY_ID = :id").setInteger("id", surveyId);
    replyDeletionQuery.executeUpdate();
    final Query commentDeletionQuery = session.createSQLQuery("DELETE ac.* " + "FROM ANSWERS_COMMENTS ac " + "JOIN ANSWERS_SET an ON an.ANSWER_SET_ID = ac.ANSWER_SET_ID " + "WHERE ac.PARENT IS NULL AND an.SURVEY_ID = :id").setInteger("id", surveyId);
    commentDeletionQuery.executeUpdate();
}
...
public boolean hasCommentChildren(int id){
    final Session session = sessionFactory.getCurrentSession();
    final Query query = session.createSQLQuery("SELECT COUNT(*) FROM ANSWERS_COMMENTS WHERE PARENT = :parent").setInteger("parent", id);
    return ((BigInteger) query.uniqueResult()).intValue() > 0;
}
