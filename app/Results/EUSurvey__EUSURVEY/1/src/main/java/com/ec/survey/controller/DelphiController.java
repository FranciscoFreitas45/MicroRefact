...
import com.ec.survey.DTO.NumberQuestionStatistics;
...
@Resource(name = "mailService")
 ...
 private  Map<String,Map<String,String>> uniqueCodeToUser;

 private  List<String> stopWords;


public List<List<DelphiContribution>> groupDelphiContributions(DelphiContributions contributions){
...
public void loadComments(DelphiTableEntry tableEntry,int originalAnswerSetId,Integer currentAnswerSetId,String questionUid,String surveyUid){
...
public ResponseEntity<AbstractDelphiGraphData> handleDelphiFreetextQuestion(Survey survey,Question question,StatisticsCreator creator){
...
public DelphiTable handleDelphiTableMatrix(Matrix question,DelphiTableOrderBy orderBy,int limit,int offset,Integer answerSetId){
    ...
    for (List<DelphiContribution> entry : groupDelphiContributions(contributions)) {
        ...
        List<DelphiTableAnswer> sortedAnswers = answers.stream().sorted(Comparator.comparingInt(Pair::getKey)).map(Pair::getValue).collect(Collectors.toList());
...
public ResponseEntity<AbstractDelphiGraphData> handleDelphiGraphRatingQuestion(Survey survey,RatingQuestion question,Statistics statistics,StatisticsCreator creator,Map<Integer,Map<Integer,Integer>> numberOfAnswersMapRatingQuestion){
...
public ResponseEntity<String> delphiEditComment(String id,HttpServletRequest request){
    try {
        ...
        if (comment == null || !comment.getUniqueCode().equals(uniqueCode) || comment.getText().equals(AnswerExplanationService.DELETED_DELPHI_COMMENT_WITH_REPLIES_TEXT) || text.equals(AnswerExplanationService.DELETED_DELPHI_COMMENT_WITH_REPLIES_TEXT)) {
            return new ResponseEntity<>(null, HttpStatus.BAD_REQUEST);
        }
        if (formManager) {
            ...
            activityService.log(801, "id: " + comment.getId() + "; text: " + comment.getText(), "id: " + comment.getId() + "; text: " + text, userid, surveyID);
        }
...
public DelphiTable handleDelphiTableRawValueQuestion(Question question,DelphiTableOrderBy orderBy,int limit,int offset,Integer answerSetId){
    DelphiTable result = new DelphiTable();
    result.setOffset(offset);
    // get survey
    Survey survey = question.getSurvey();
...
public ResponseEntity<AbstractDelphiGraphData> delphiGraph(HttpServletRequest request){
    try {
        ...
        boolean privileged = resultsview && (survey.getOwner().getId().equals(user.getId()) || (user.getGlobalPrivileges().get(GlobalPrivilege.FormManagement) == 2) || (user.getLocalPrivileges().get(LocalPrivilege.AccessResults) > 0));
        AnswerSet answerSet = answerService.get(request.getParameter("uniquecode"));
        ...
        Question question = (Question) element;
        if (!privileged && !survey.getIsDelphiShowAnswersAndStatisticsInstantly() && !answerSetContainsAnswerForQuestion(answerSet, question)) {
            return new ResponseEntity<>(null, HttpStatus.NO_CONTENT);
        }
        ...
        creator.getAnswers4Statistics(survey, question, numberOfAnswersMap, numberOfAnswersMapMatrix, numberOfAnswersMapGallery, multipleChoiceSelectionsByAnswerset, numberOfAnswersMapRatingQuestion, numberOfAnswersMapNumberQuestion);
...
public DelphiTable handleDelphiTableRatingQuestion(RatingQuestion question,DelphiTableOrderBy orderBy,int limit,int offset,Integer answerSetId){
    DelphiTable result = new DelphiTable();
    result.setOffset(offset);
    Map<String, Integer> questionPositions = new HashMap<>();
    Map<String, String> questionTitles = new HashMap<>();
...
public ResponseEntity<String> delphiDeleteComment(String id,HttpServletRequest request){
    try {
        final int idParsed = Integer.parseInt(id);
        final String uniqueCode = request.getParameter("uniqueCode");
        final boolean formManager = request.getParameter("formManager") != null;
        ...
        if (formManager) {
            final int userid = sessionService.getCurrentUser(request).getId();
            final String surveyID = surveyService.getSurveyForQuestion(comment.getQuestionUid()).getUniqueId();
            activityService.log(802, "id: " + comment.getId() + "; text: " + comment.getText(), null, userid, surveyID);
        }
...
public ResponseEntity<DelphiUpdateResult> delphiCreateOrUpdateAnswerAndExplanation(HttpServletRequest request,Locale locale){
    try {
        ...
        final Map<Element, String> validation = SurveyHelper.validateAnswerSet(answerSet, answerService, invisibleElements, resources, locale, null, request, true, user, fileService, elements);
...
public ResponseEntity<AbstractDelphiGraphData> handleDelphiNumberQuestion(Survey survey,NumberQuestion question,NumberQuestionStatistics numberQuestionStatistics){
...
public DelphiTable handleDelphiTableTable(Table question,DelphiTableOrderBy orderBy,int limit,int offset,Integer answerSetId){
    DelphiTable result = new DelphiTable();
    result.setOffset(offset);
    DelphiContributions contributions = answerExplanationService.getDelphiContributions(question, orderBy, limit, offset);
    result.setTotal(contributions.getTotal());
...
public ResponseEntity<AbstractDelphiGraphData> handleDelphiGraphChoiceQuestion(Survey survey,ChoiceQuestion question,Statistics statistics,StatisticsCreator creator,Map<Integer,Integer> numberOfAnswersMap,Map<Integer,Map<String,Set<String>>> multipleChoiceSelectionsByAnswerset){
...
public ResponseEntity<String> sendDelphiLink(HttpServletRequest request,Locale locale){
    try {
        ...
        String body = "Dear EUSurvey user,<br /><br />Your contribution to the survey '<b>" + answerSet.getSurvey().cleanTitle() + "</b>' has been saved. To open the contribution again, please follow this link:<br /><br />";
        ...
        try {
            ...
            mailService.SendHtmlMail(email, sender, sender, resources.getMessage("message.mail.linkSubject", null, new Locale("EN")), text, null);
        } catch (Exception e) {
...
public DelphiTable handleDelphiTableRankingQuestion(RankingQuestion question,DelphiTableOrderBy orderBy,int limit,int offset,Integer answerSetId){
    DelphiTable result = new DelphiTable();
    result.setOffset(offset);
    // get survey
    Survey survey = question.getSurvey();
...
public DelphiTable handleDelphiTableChoiceQuestion(ChoiceQuestion question,DelphiTableOrderBy orderBy,int limit,int offset,Integer answerSetId){
    DelphiTable result = new DelphiTable();
    result.setOffset(offset);
    ...
    Survey survey = question.getSurvey();
    for (List<DelphiContribution> entry : groupDelphiContributions(contributions)) {
        List<String> values = entry.stream().map(DelphiContribution::getAnswerUid).collect(Collectors.toList());
        DelphiTableEntry tableEntry = new DelphiTableEntry();
        DelphiContribution firstValue = entry.get(0);
        tableEntry.setAnswerSetId(firstValue.getAnswerSetId());
        tableEntry.setAnswerSetUniqueCode(firstValue.getAnswerSetUniqueCode());
...
public ResponseEntity<DelphiTable> delphiTable(HttpServletRequest request){
    try {
        ...
        Question question = (Question) element;
        if (!question.getIsDelphiQuestion() || (!survey.getIsDelphiShowAnswersAndStatisticsInstantly() && !answerSetContainsAnswerForQuestion(answerSet, question))) {
            return new ResponseEntity<>(null, HttpStatus.NO_CONTENT);
        }
...
public ResponseEntity<AbstractDelphiGraphData> handleDelphiGraphMatrix(Survey survey,Matrix question,Statistics statistics,StatisticsCreator creator,Map<Integer,Map<Integer,Integer>> numberOfAnswersMapMatrix){
    DelphiGraphDataMulti result = new DelphiGraphDataMulti();
...
public void loadFiles(DelphiTableEntry tableEntry,int answerSetId,String questionUid){
    try {
