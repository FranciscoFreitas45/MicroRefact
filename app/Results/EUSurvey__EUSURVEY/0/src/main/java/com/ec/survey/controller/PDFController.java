...
public ModelAndView answer(String code,HttpServletRequest request,Locale locale,HttpServletResponse response){
...
@ResponseBody
public String answerready(String code,HttpServletRequest request,Locale locale,HttpServletResponse response){
    try {
...
public ModelAndView pubsurvey(String id,HttpServletRequest request,Locale locale,HttpServletResponse response){
    try {
...
@ResponseBody
public String surveyexists(String id,HttpServletRequest request,Locale locale,HttpServletResponse response){
    try {
...
public ModelAndView survey(String id,HttpServletRequest request,Locale locale,HttpServletResponse response){
    try {
        String language = locale.getLanguage();
        Survey survey = surveyService.getSurvey(Integer.parseInt(id), false, true);
        if (survey == null) {
        ...
        if (!survey.getIsDraft()) {
            ...
            if (!u.getId().equals(survey.getOwner().getId()) && u.getGlobalPrivileges().get(GlobalPrivilege.FormManagement) < 2 && u.getLocalPrivileges().get(LocalPrivilege.AccessDraft) < 1 && u.getLocalPrivileges().get(LocalPrivilege.FormManagement) < 1) {
                throw new ForbiddenURLException();
            }
        }
        SendFile(survey, request, response, language);
...
public void SendFile(Survey survey,HttpServletRequest request,HttpServletResponse response,String language){
    String lang = request.getParameter("lang");
...
@ResponseBody
public String answerexists(String code,HttpServletRequest request,Locale locale,HttpServletResponse response){
    try {
        java.io.File target = new java.io.File(String.format("%sanswer%s.pdf", tempFileDir, code));
        if (target.exists() && target.length() > 0) {
            return "exists";
...
@ResponseBody
public String surveyready(String id,HttpServletRequest request,Locale locale,HttpServletResponse response){
    try {
        String lang = request.getParameter("lang");
        Survey survey = surveyService.getSurvey(Integer.parseInt(id), true);
        java.io.File folder = fileService.getSurveyExportsFolder(survey.getUniqueId());
