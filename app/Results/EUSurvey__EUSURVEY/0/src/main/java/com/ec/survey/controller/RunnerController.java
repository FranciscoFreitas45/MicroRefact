...
import com.ec.survey.DTO.EcasUser;
import com.ec.survey.DTO.Upload;
@Controller
...
public ModelAndView processDraftSubmit(String mode,HttpServletRequest request,Locale locale,HttpServletResponse response,Device device){
    try {
        ...
        if (!mode.equalsIgnoreCase("preview")) {
            ...
            if (draftid != null) {
                ...
                if (draft != null) {
                    SurveyHelper.parseAndMergeAnswerSet(request, survey, uniqueCode, draft.getAnswerSet(), lang, user, fileService);
                    // this also sets the
...
public void upload(String id,String uniqueCode,HttpServletRequest request,HttpServletResponse response){
    ...
    try {
        ...
        boolean error = false;
        if (!directory.exists() && !directory.mkdirs() && !directory.exists() && !directory.mkdirs() && !directory.exists() && !directory.mkdirs()) {
...
@ResponseBody
public String delete(String id,String uniqueCode,String surveyUID,HttpServletRequest request){
    try {
        ...
        if (isDelphi) {
            final com.ec.survey.model.survey.base.File fileInDatabase = answerExplanationService.getExplanationFile(survey.getUniqueId(), uniqueCode, questionUid, fileName);
            if (fileInDatabase != null) {
                final AnswerSet answerSet = answerService.get(uniqueCode);
                answerExplanationService.removeFileFromExplanation(answerSet.getId(), questionUid, fileInDatabase.getUid());
                fileService.deleteFileFromDiskAndDatabase(survey.getUniqueId(), fileInDatabase);
            }
            parentDirectory = fileService.deleteUploadedExplanationFileAndReturnParentDirectoryIfSuccessful(survey.getUniqueId(), uniqueCode, questionUid, fileName);
        } else {
...
public ModelAndView invitedPOST(String group,String unique,HttpServletRequest request,Locale locale,Device device){
    try {
        String participationGroupId = request.getParameter("participationGroup");
        String invitationId = request.getParameter("invitation");
        String uniqueCode = request.getParameter(Constants.UNIQUECODE);
        ...
        if (!survey.getIsDelphi() && invitation.getAnswers() > 0) {
            ...
            model.addObject(Constants.MESSAGE, resources.getMessage("error.InvitationUsed", null, "The invitation was already used.", locale));
            ...
            if (aws != null) {
                ...
                model.addObject("messageComplement", resources.getMessage("message.SubmittedOn", args, "Submitted on: " + ConversionTools.getFullString(answerDate), locale));
            }
            return model;
        }
        ...
        if (!validation.isEmpty()) {
            ...
            model.addObject("submit", true);
            model.addObject(Constants.UNIQUECODE, uniqueCode);
            model.addObject(Constants.MESSAGE, resources.getMessage("error.CheckValidation", null, "Please check for validation errors.", locale));
            model.addObject("invitation", invitation.getId());
            model.addObject("participationGroup", participationGroup.getId());
...
public ModelAndView loadSurvey(Survey survey,HttpServletRequest request,Locale locale,String action,boolean passwordauthenticated,boolean readonlyMode){
    if (survey != null) {
        String draftid = request.getParameter("draftid");
        if (draftid != null) {
            try {
        ...
        if (draftid != null && draftid.trim().length() > 0) {
            try {
                Draft draft = answerService.getDraft(draftid);
                if (draft == null) {
                    model.addObject(Constants.MESSAGE, resources.getMessage("error.DraftIDInvalid", null, "The given draft ID is not valid!", locale));
                } else {
    ...
    ModelAndView model = new ModelAndView(Constants.VIEW_ERROR_GENERIC);
    model.addObject(Constants.MESSAGE, resources.getMessage("error.WrongURL", null, "The url you entered was not correct.", locale));
    return model;
}
...
@ResponseBody
public String sendDraftLinkByMail(HttpServletRequest request,HttpServletResponse response){
    Map<String, String[]> parameters = Ucs2Utf8.requestToHashMap(request);
    String email = parameters.get(Constants.EMAIL)[0];
    ...
    if (!skipChecks && !checkCaptcha(request)) {
        ...
        try {
            ...
            mailService.SendHtmlMail(email, sender, sender, resources.getMessage("message.mail.linkDraftSubject", null, new Locale("EN")), text, null);
        } catch (Exception e) {
...
public ModelAndView DraftSubmit(String draftid,HttpServletRequest request,Locale locale,HttpServletResponse response,Device device){
    Draft draft = answerService.getDraft(draftid);
    if (draft == null) {
...
public ModelAndView preparesurvey(String id,HttpServletRequest request,Locale locale){
...
public ModelAndView getEscapePageModel(Survey survey,HttpServletRequest request,Device device){
...
public ModelAndView contactform(String uidorshortname,HttpServletRequest request,HttpServletResponse response,Locale locale,Device device){
...
public ModelAndView runner(String uidorshortname,HttpServletRequest request,HttpServletResponse response,Locale locale,Device device){
            ...
            if (survey.getEcasSecurity()) {
                try {
                    ...
                    if (readonlyMode || (user != null && user.getType().equalsIgnoreCase(User.ECAS) && ecasauthenticated)) {
                        ...
                        if (contributionsCount > 0 && (survey.getAllowedContributionsPerUser() == 1 || survey.getAllowedContributionsPerUser() <= contributionsCount)) {
                            if (survey.getIsDelphi()) {
                            ...
                            if (survey.getAllowedContributionsPerUser() == 1) {
                                modelReturn.addObject(Constants.MESSAGE, resources.getMessage("error.UserAlreadySubmitted", null, "This account has already been used to submit a contribution. Multiple submission is prohibited.", locale));
                            } else {
                                modelReturn.addObject(Constants.MESSAGE, resources.getMessage("error.UserAlreadySubmitted2", null, "You have submitted the maximum number of contributions this survey allows. Further contributions are not allowed.", locale));
                            }
                            return modelReturn;
                        }
...
public String contactformPOST(String uidorshortname,HttpServletRequest request,ModelMap model){
    Survey survey = surveyService.getSurvey(uidorshortname, false, true, false, false, null, true, true);
    if (survey == null) {
        survey = surveyService.getSurvey(uidorshortname, true, true, false, false, null, true, true);
    }
...
public ModelAndView getMaxAnswersReachedPageModel(Survey survey,HttpServletRequest request,Device device){
...
@ResponseBody
public List<Element> element(String id,HttpServletRequest request,HttpServletResponse response){
...
public ModelAndView invited(String group,String unique,HttpServletRequest request,Locale locale,Integer draftSurveyId,Device device){
    boolean readonlyMode = false;
    String p = request.getParameter("readonly");
    ...
    try {
        ...
        if (invitation != null && (invitation.getDeactivated() == null || !invitation.getDeactivated()) && participationGroup != null && invitation.getParticipationGroupId().equals(participationGroup.getId())) {
            if (!participationGroup.getActive()) {
                ModelAndView model = new ModelAndView(Constants.VIEW_ERROR_GENERIC);
                model.addObject(Constants.MESSAGE, resources.getMessage("error.InvitationDeactivated", null, "The access for this guest-list has not yet been activated. Please try again later.", locale));
                model.addObject("noMenu", true);
                model.addObject("runnermode", true);
                return model;
            }
            ...
            if (survey != null) {
                ...
                if (lang != null) {
                    if (f.translationIsValid(lang)) {
                        List<Translations> translations = translationService.getTranslationsForSurvey(survey.getId(), true);
                        for (Translations trans : translations) {
                            if (trans.getLanguage().getCode().equalsIgnoreCase(lang) && (!trans.getComplete() || !trans.getActive())) {
                ...
                if (draftid != null && draftid.trim().length() > 0) {
                    try {
                        ...
                        f.getAnswerSets().add(draft.getAnswerSet());
                        f.setWcagCompliance(draft.getAnswerSet().getWcagMode() != null && draft.getAnswerSet().getWcagMode());
                        SurveyHelper.recreateUploadedFiles(draft.getAnswerSet(), f.getSurvey(), fileService, answerExplanationService);
                        uniqueCode = draft.getAnswerSet().getUniqueCode();
                        if (lang == null) {
                            lang = draft.getAnswerSet().getLanguageCode();
                            Survey translated = SurveyHelper.createTranslatedSurvey(f.getSurvey().getId(), lang, surveyService, translationService, true);
                ...
                if (draft != null) {
                    ...
                    ModelAndView model = new ModelAndView(Constants.VIEW_ERROR_GENERIC);
                    model.addObject(Constants.MESSAGE, resources.getMessage("error.SurveyNotActive", null, "The survey is not active at the moment. Please try again later.", locale));
                    model.addObject("noMenu", true);
                    return model;
                }
            }
...
public ModelAndView runnerTokenPost(String shortname,String token,HttpServletRequest request,Locale locale,Device device){
...
public ModelAndView runnerToken(String shortname,String token,HttpServletRequest request,Locale locale,Device device){
    Survey survey = null;
    try {
...
public String getFiles(java.io.File directory){
...
public String validateDeleteParameters(int questionId,String uniqueCode,String fileName,String surveyUID){
    ...
    boolean validDirectoryPath = validator.isValidDirectoryPath("check directory path in RunnerController.delete method", canonicalPath, basePath, false);
...
public ModelAndView processSubmit(String uidorshortname,HttpServletRequest request,HttpServletResponse response,Locale locale,Model modelMap,Device device){
    boolean hibernateOptimisticLockingFailureExceptionCatched = false;
    String email = null;
    try {
        if (request.getParameter("redirectFromCheckPassword") != null && request.getParameter("redirectFromCheckPassword").equalsIgnoreCase("true") && request.getParameter("password") != null && request.getParameter(Constants.SHORTNAME) != null) {
            Survey survey = surveyService.getSurvey(uidorshortname, false, true, false, false, null, true, true);
            ...
            if (survey != null && survey.getSecurity().startsWith("secured")) {
                if (survey.getPassword() != null && survey.getPassword().trim().length() > 0 && survey.getPassword().equals(password)) {
                    // authenticated
                ...
                if (invitation != null) {
                    ...
                    if (participationGroup != null && participationGroup.getSurveyUid().equals(draftsurvey.getUniqueId())) {
                        return new ModelAndView("redirect:/runner/invited/" + participationGroup.getId() + Constants.PATH_DELIMITER + password);
                    }
                }
                String draftid = request.getParameter("draftid");
                ...
                model.addObject(Constants.ERROR, resources.getMessage("error.PasswordInvalid", null, "You did not provide a valid password!", locale));
                return model;
            }
            return new ModelAndView("redirect:/errors/500.html");
        }
        ...
        if (origsurvey.getSecurity().startsWith("secured")) {
            if (!validCodesService.checkValid(uniqueCode, origsurvey.getUniqueId())) {
                ModelAndView model = new ModelAndView(Constants.VIEW_ERROR_GENERIC);
                model.addObject(Constants.MESSAGE, resources.getMessage("error.NoInvitation", null, "You are not authorized to submit to this survey without a proper invitation.", locale));
                return model;
            }
        } else {
        ...
        try {
            ...
            model.addObject("submit", true);
            model.addObject(Constants.UNIQUECODE, uniqueCode);
            model.addObject(Constants.MESSAGE, resources.getMessage("error.LoginExists", null, "This login already exists. Please choose a unique login.", locale));
            return model;
        ...
        if (origsurvey.getSecurity().startsWith("secured") && origsurvey.getEcasSecurity() && origsurvey.getConfirmationPageLink()) {
            request.getSession().invalidate();
        }
...
@ResponseBody
public String createquizpdf(String code,HttpServletRequest request,HttpServletResponse response){
    try {
        if (!checkCaptcha(request)) {
            return "errorcaptcha";
        }
