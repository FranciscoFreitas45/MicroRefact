...
import com.ec.survey.DTO.SchemaService;
...
public void run(){
    ...
    if (StringUtils.isNotEmpty(enabled) && enabled.equalsIgnoreCase("true")) {
        ...
        if (StringUtils.isNotEmpty(interval) && interval.length() >= 2 && (interval.endsWith("y") || interval.endsWith("w") || interval.endsWith("d"))) {
            ...
            if (answerSetsToAnonymise.size() >= 1) {
                Date newLastAnswerSetAnonymisedDate = answerSetsToAnonymise.get(answerSetsToAnonymise.size() - 1).getDate();
