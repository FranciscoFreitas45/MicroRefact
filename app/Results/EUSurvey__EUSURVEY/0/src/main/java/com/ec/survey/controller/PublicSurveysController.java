...
public ModelAndView publicsurveys(HttpServletRequest request,Model model,Locale locale){
    ...
    if (filteralias == null)
        filteralias = (String) request.getSession().getAttribute("publicsurveysfilteralias");
    String filterowner = request.getParameter("filterowner");
    if (filterowner == null)
        filterowner = (String) request.getSession().getAttribute("publicsurveysfilterowner");
    String filterrequestdatefrom = request.getParameter("filterrequestdatefrom");
    if (filterrequestdatefrom == null)
        filterrequestdatefrom = (String) request.getSession().getAttribute("publicsurveysfilterrequestdatefrom");
    String filterrequestdateto = request.getParameter("filterrequestdateto");
    if (filterrequestdateto == null)
        filterrequestdateto = (String) request.getSession().getAttribute("publicsurveysfilterrequestdateto");
    ...
    if (error != null)
        m.addObject(Constants.ERROR, resources.getMessage("error.OperationFailed", null, "", locale));
