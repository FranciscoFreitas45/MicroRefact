...
public boolean userIsResultReadAuthorized(Survey survey,HttpServletRequest request){
...
public boolean userIsAnswerer(AnswerSet answerSet,User user){
    if (answerSet != null && answerSet.getResponderEmail() != null && user.getEmail() != null && (answerSet.getResponderEmail().equalsIgnoreCase(user.getEmail()) || answerSet.getResponderEmail().equalsIgnoreCase(Tools.md5hash(user.getEmail())))) {
        // User's email is the same
        return true;
    }
    if (answerSet != null && answerSet.getSurvey() != null && answerSet.getSurvey().getOwner() != null && answerSet.getSurvey().getOwner().getId().equals(user.getId())) {
        // User is same ID
        return true;
    }
    return false;
...
public Form checkSurvey(Survey survey,User user,HttpServletRequest request){
    if (survey == null)
        return null;
...
public void upgradePrivileges(Survey survey,User user,HttpServletRequest request){
    if (user == null)
