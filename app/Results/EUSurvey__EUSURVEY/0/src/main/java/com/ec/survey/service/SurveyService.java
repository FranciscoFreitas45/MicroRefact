...
@Value("${publicsurveynotification}")
...
public void updateAnswerUids(int id,String uid,boolean pa){
    boolean saved = false;
    int counter = 1;
    while (!saved) {
        try {
...
@Transactional
public void SaveAnswerSets(Set<AnswerSet> answerSets,String fileDir,String draftid){
...
public void deleteSurveyData(int id,boolean deleteAnswers,boolean deleteAccesses,String uid,boolean deleteLogs){
    Session session = sessionFactory.getCurrentSession();
    if (deleteAnswers) {
        Query query0 = session.createSQLQuery("SELECT fi.FILE_UID from FILES fi JOIN ANSWERS_FILES f ON fi.FILE_ID = f.files_FILE_ID JOIN ANSWERS a ON f.ANSWERS_ANSWER_ID = a.ANSWER_ID JOIN ANSWERS_SET an ON a.AS_ID = an.ANSWER_SET_ID WHERE an.SURVEY_ID = :id");
        ...
        Query query1 = session.createSQLQuery("DELETE f.* from ANSWERS_FILES f JOIN ANSWERS a ON f.ANSWERS_ANSWER_ID = a.ANSWER_ID JOIN ANSWERS_SET an ON a.AS_ID = an.ANSWER_SET_ID WHERE an.SURVEY_ID = :id");
...
public void internalUpdateAnswerUids(int id,String uid,boolean pa){
    Session session = sessionFactory.getCurrentSession();
    Query query;
    if (pa) {
...
public Map<String,String> importDraftSurvey(Survey copy,ImportResult result,User user){
...
public void synchronizeSurvey(Survey survey,String languageCode,boolean setSurvey){
...
@Transactional
public void freeze(String surveyId,String mailText){
...
@Transactional
public Survey clearChanges(String shortname,int userId){
    Session session = sessionFactory.getCurrentSession();
...
public void computeTrustScore(Survey survey){
    Session session = sessionFactory.getCurrentSession();
    int score = 0;
...
@Transactional
public void unfreeze(String surveyId){
    Session session = sessionFactory.getCurrentSession();
    Survey survey = getSurvey(Integer.parseInt(surveyId));
    if (survey == null) {
        throw new MessageException("survey does not exist");
...
public void synchronizeTranslation(Survey survey,int userId){
    ...
    if (translations != null) {
        ...
        if (originalTranslations != null) {
            ...
            for (Translation translation : translations.getTranslations()) {
                if (!originalTranslationsMap.containsKey(translation.getKey()) && !translation.getKey().endsWith("help") && !translation.getKey().endsWith("UNIT") && !translation.getKey().endsWith("TABTITLE")) {
            ...
            for (Translation originalTranslation : originalTranslations.getTranslations()) {
                if (!newTranslationsMap.containsKey(originalTranslation.getKey()) && !originalTranslation.getKey().endsWith("help") && !originalTranslation.getKey().endsWith("UNIT") && !originalTranslation.getKey().endsWith("TABTITLE") && !deletedElements.contains(originalTranslation.getKey())) {
...
public void sendListFormMail(Survey survey){
    String body = "A request for publishing the survey '" + survey.getShortname() + "' in the list of all public surveys has been made.<br /><br />" + "To see all requests, please follow this link:<br /><a href=\"" + host + "administration/publicsurveys\">" + host + "administration/publicsurveys</a><br /><br />" + "Your EUSurvey Team";
    InputStream inputStream = servletContext.getResourceAsStream("/WEB-INF/Content/mailtemplateeusurvey.html");
...
public void saveAnswerSet(AnswerSet answerSet,String fileDir,String draftid){
    boolean saved = false;
    int counter = 1;
    while (!saved) {
        try {
...
public void UpdatePossibleAnswers(Survey survey){
    for (Element element : survey.getElements()) {
...
public void deleteNoTransaction(int id,boolean deleteLogs,boolean deleteFileMappings){
    Session session = sessionFactory.getCurrentSession();
...
public void deleteFileMappings(List<Integer> surveyIDs){
    Session session = sessionFactory.getCurrentSession();
    String sql = "delete from ELEMENTS_FILES" + " where ELEMENTS_FILES.ELEMENTS_ID" + " in (select ELEMENTS.ID from ELEMENTS" + " where ELEMENTS.ID" + " in (select SURVEYS_ELEMENTS.elements_ID" + " from SURVEYS_ELEMENTS" + " where SURVEYS_ELEMENTS.SURVEYS_SURVEY_ID = :id));";
...
public void sendOPCApplyChangesMail(Survey survey,int userId){
    if (survey.getIsOPC() && opcnotify != null && opcnotify.length() > 0) {
        User user = userId > 0 ? administrationService.getUser(userId) : null;
        String body = "The survey <a href=\"" + host + survey.getShortname() + "/management/overview\">" + survey.getShortname() + "</a> has been changed by user " + (user != null ? user.getName() : "SYSTEM") + " at " + new Date() + ".";
        InputStream inputStream = servletContext.getResourceAsStream("/WEB-INF/Content/mailtemplateeusurvey.html");
        String text = IOUtils.toString(inputStream, "UTF-8").replace("[CONTENT]", body).replace("[HOST]", host);
...
public Map<String,String> copyFiles(Survey survey,Map<String,String> convertedUIDs,boolean allowMissingFiles,Map<String,String> missingFiles,String originalSurveyUID){
    ...
    if (survey.getBackgroundDocuments() != null) {
        ...
        for (String label : labels) {
            ...
            if (convertedUIDs.containsKey(olduid)) {
                newuid = convertedUIDs.get(olduid);
                survey.getBackgroundDocuments().put(label, servletContext.getContextPath() + "/files/" + survey.getUniqueId() + Constants.PATH_DELIMITER + newuid);
                result.put(olduid, newuid);
            } else {
                try {
                    java.io.File source = fileService.getSurveyFile(originalSurveyUID, olduid);
            ...
            if (image.getUrl() != null && !image.getUrl().contains("photo_scenery.png")) {
                ...
                if (fileUID.length() > 0) {
                    ...
                    if (convertedUIDs.containsKey(fileUID)) {
                        ...
                        image.setUrl(servletContext.getContextPath() + "/files/" + survey.getUniqueId() + Constants.PATH_DELIMITER + newuid);
...
public void importPublishedSurvey(Survey copyActive,ImportResult result,User user,Map<String,String> oldToNewUniqueIds,Map<Integer,Integer> oldSourceIdsById){
...
@Transactional
public boolean changeOwner(String surveyuid,String owner,int currentuserid){
    int ownerid = 0;
    User newowner = null;
    try {
        ...
        for (ResultFilter publishedSurveyFilter : publishedSurveyFilters) {
            ...
            if (export == null) {
                ...
                if (publishedSurveyFilter.getUserId() != null) {
                    sessionService.internalSetLastResultFilter(newPublishedSurveyFilter, publishedSurveyFilter.getUserId(), newPublishedSurveyFilter.getSurveyId());
                }
            }
        }
    }
...
@SuppressWarnings("unchecked")
public List<Object[]> loadSurveysfromDatabase(String sql,HashMap<String,Object> parameters,SqlPagination sqlPagination){
    Session session = sessionFactory.getCurrentSession();
    SQLQuery query = session.createSQLQuery(sql);
...
public void checkAndRecreateMissingElements(Survey survey,ResultFilter filter){
    ...
    for (Object o : res) {
        ...
        if (!surveyelementsbyuid.containsKey(questionUID) && !missingelementuids.containsKey(questionUID)) {
            ...
            if (missingquestion == null) {
                ...
                if (parent instanceof Matrix) {
                    ...
                    } else if (!surveyelementsbyuid.containsKey(parentMatrix.getUniqueId()) && missingelementuids.containsKey(parentMatrix.getUniqueId()) && !parentMatrix.containsChild(missingquestion.getId())) {
                        // check if matrix contains question
                        parentMatrix.getMissingQuestions().add(missingquestion);
                    }
        ...
        if (possibleAnswerUID != null && possibleAnswerUID.length() > 0 && !possibleAnswerUID.contains("#") && !surveyelementsbyuid.containsKey(possibleAnswerUID) && !missingelementuids.containsKey(possibleAnswerUID)) {
...
public void reportAbuse(Survey survey,String type,String text,String email){
    ...
    if (count < max) {
        ...
        mailtext = mailtext.replace("[ALIAS]", survey.getShortname()).replace("[LINK]", link).replace("[TITLE]", survey.getTitle()).replace("[ID]", survey.getId().toString()).replace("[EMAIL]", email).replace("[TYPE]", type).replace("[TEXT]", text).replace("[DATE]", abuse.getCreated().toString()).replace("[COUNT]", Integer.toString(count + 1));
        InputStream inputStream = servletContext.getResourceAsStream("/WEB-INF/Content/mailtemplateeusurvey.html");
...
public Survey importOldPublishedSurvey(Survey survey,User user,Map<String,String> oldToNewUniqueIds){
...
public void AdaptIDs(Survey oldSurvey,Survey newSurvey,boolean isDraft){
    Session session = sessionFactory.getCurrentSession();
    if (isDraft) {
...
@SuppressWarnings("unchecked")
public List<Object> GetAllRankingQuestionAnswers(String surveyUid,Set<String> rankingQuestionUids){
    Session session = sessionFactory.getCurrentSession();
    Query query = session.createSQLQuery("SELECT DISTINCT QUESTION_UID, VALUE FROM ANSWERS a INNER JOIN ANSWERS_SET ans ON ans.ANSWER_SET_ID = a.AS_ID JOIN SURVEYS s ON  ans.SURVEY_ID =  s.SURVEY_ID WHERE s.ISDRAFT = FALSE AND s.SURVEY_UID = :surveyUid AND a.QUESTION_UID IN (" + StringUtils.collectionToCommaDelimitedString(rankingQuestionUids) + ")").setString("surveyUid", surveyUid);
...
public String translateIPMKey(String key,Map<Integer,Element> elementsBySourceId,Map<String,String> oldToNewUniqueIds,ImportResult result){
...
public void sendNotificationEmail(Survey survey,AnswerSet answerSet,String email){
...
@Transactional
public int importSurvey(ImportResult result,User user,boolean isRestore){
...
public void copyTranslations(List<Translations> translations,Survey survey,Map<String,String> oldToNewUniqueIds,ImportResult result,boolean newTitle,Map<String,String> convertedFileUIDs){
    ...
    for (Translations tOriginal : translations) {
        ...
        if (result != null && result.isFromIPM()) {
            for (List<Integer> ids : result.getAdditionalElements().values()) for (Integer id : ids) {
                Element upload = elementsBySourceId.get(id);
...
@Transactional
public Survey activate(Survey survey,boolean deactivateAutoPublishing,int userId){
...
public String translateKey(String key,Map<Integer,Element> elementsBySourceId,Map<String,String> oldToNewUniqueIds,Map<String,String> convertedFileUIDs){
    if (key == null)
        return key;
    if (key.equalsIgnoreCase(Survey.TITLE))
        return key;
...
public boolean checkTranslations(List<Translations> first,List<Translations> second,Map<String,Translation> currentKeys){
    ...
    for (Translations firstTranslations : first) {
        ...
        for (Translation firstFilteredTranslation : firstTranslations.getFilteredTranslations()) {
            ...
            if (!secondTranslationsMap.containsKey(firstFilteredTranslation.getKey()) || !secondTranslationsMap.get(firstFilteredTranslation.getKey()).equals(firstFilteredTranslation.getLabel())) {
                // A label is not the same or a key is not present
                // Check if the key is supposed to be present in the current survey?
                if (firstFilteredTranslation.getLabel() != null && firstFilteredTranslation.getLabel().trim().length() > 0 && currentKeys.containsKey(firstFilteredTranslation.getKey()) && !firstFilteredTranslation.getKey().startsWith("ESCAPE")) {
                    return true;
                }
            }
        }
...
@SuppressWarnings("unchecked")
public List<Object> GetAllQuestionsAndPossibleAnswers(String surveyUid){
    Session session = sessionFactory.getCurrentSession();
    Query query = session.createSQLQuery("SELECT DISTINCT QUESTION_UID, PA_UID FROM ANSWERS a INNER JOIN ANSWERS_SET ans ON ans.ANSWER_SET_ID = a.AS_ID JOIN SURVEYS s ON  ans.SURVEY_ID =  s.SURVEY_ID WHERE s.ISDRAFT = FALSE AND s.SURVEY_UID = :surveyUid").setString("surveyUid", surveyUid);
    return (List<Object>) query.list();
}
...
public java.io.File exportSurvey(String shortname,SurveyService surveyService,boolean answers){
    return SurveyExportHelper.exportSurvey(shortname, surveyService, answers, translationService, answerService, fileDir, sessionService, fileService, sessionFactory.getCurrentSession(), host, answerExplanationService);
}


public void importSurveyData(ImportResult result,boolean draft,Survey survey,Map<String,String> oldToNewUniqueIds,Integer surveyid,Map<String,String> convertedFileUIDs){
...
public Survey editSave(Survey oldsurvey,HttpServletRequest request){
    ...
    Survey survey = SurveyHelper.parseSurvey(request, this, fileService, servletContext, activityService.isEnabled(217), activityService.isEnabled(220), fileIDsByUID);
...
@Transactional
public Survey publish(Survey draftSurvey,int pnumberOfAnswerSets,int pnumberOfAnswerSetsPublished,boolean deactivateAutoPublishing,int userId,boolean resetSourceIds,boolean resetSurvey){
    Session session = sessionFactory.getCurrentSession();
    ...
    Survey publishedSurvey = draftSurvey.copy(this, draftSurvey.getOwner(), fileDir, true, pnumberOfAnswerSets, pnumberOfAnswerSetsPublished, true, resetSourceIds, true, null, null);
    // this means it is not a draft
    publishedSurvey.setIsDraft(false);
    if (deactivateAutoPublishing) {
        publishedSurvey.setAutomaticPublishing(false);
    }
    ...
    for (Translations draftTranslations : draftTranslationsList) {
        if (!draftTranslations.getLanguage().getCode().equalsIgnoreCase(draftSurvey.getLanguage().getCode())) {
            ...
            for (Translation draftTranslation : draftTranslations.getTranslations()) {
                ...
                if (draftTranslation.getKey().equalsIgnoreCase(Survey.TITLE) || draftTranslation.getKey().equalsIgnoreCase(Survey.LOGOTEXT) || draftTranslation.getKey().equalsIgnoreCase(Survey.INTRODUCTION) || draftTranslation.getKey().equalsIgnoreCase(Survey.ESCAPEPAGE) || draftTranslation.getKey().equalsIgnoreCase(Survey.ESCAPELINK) || draftTranslation.getKey().equalsIgnoreCase(Survey.CONFIRMATIONPAGE) || draftTranslation.getKey().equalsIgnoreCase(Survey.CONFIRMATIONLINK) || draftTranslation.getKey().equalsIgnoreCase(Survey.HELP) || draftTranslation.getKey().equalsIgnoreCase(Survey.QUIZWELCOMEMESSAGE) || draftTranslation.getKey().equalsIgnoreCase(Survey.QUIZRESULTSMESSAGE)) {
