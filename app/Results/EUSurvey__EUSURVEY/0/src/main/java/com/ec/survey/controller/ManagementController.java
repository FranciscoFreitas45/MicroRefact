...
import com.ec.survey.DTO.Template;
@Controller
...
@Value("${opc.users}")
 public  String opcusers;

@Value("${opc.department:@null}")
 public  String opcdepartments;

@Value("${opc.template}")
 public  String opctemplatesurvey;

@Value("${ecf.template}")
 public  String ecfTemplateSurvey;


public ModelAndView updateSurvey(Form form,HttpServletRequest request,boolean creation,Locale locale){
    ...
    if (creation) {
        ...
        uploadedSurvey.setIsDelphi(request.getParameter("delphi") != null && request.getParameter("delphi").equalsIgnoreCase("true"));
        uploadedSurvey.setIsECF(request.getParameter("ecf") != null && request.getParameter("ecf").equalsIgnoreCase("true"));
        ...
        if (!survey.getOwner().getId().equals(u.getId()) && u.getGlobalPrivileges().get(GlobalPrivilege.FormManagement) < 2) {
        ...
        if (!u.getId().equals(survey.getOwner().getId()) && u.getGlobalPrivileges().get(GlobalPrivilege.FormManagement) < 2 && u.getLocalPrivileges().get(LocalPrivilege.FormManagement) < 2) {
    ...
    if (survey.getLanguage() == null || uploadedSurvey.getLanguage() == null || !Tools.isEqual(survey.getLanguage().getCode(), uploadedSurvey.getLanguage().getCode()))
        hasPendingChanges = true;
    ...
    if (!Tools.isEqual(survey.getIsUseMaxNumberContribution(), uploadedSurvey.getIsUseMaxNumberContribution()))
        hasPendingChanges = true;
    if (!Tools.isEqual(survey.getIsUseMaxNumberContributionLink(), uploadedSurvey.getIsUseMaxNumberContributionLink()))
        hasPendingChanges = true;
    ...
    if (!creation) {
        ...
        for (Entry<String, String> entry : oldLinks.entrySet()) {
            ...
            if (!survey.getUsefulLinks().containsKey(label)) {
                hasPendingChanges = true;
                String[] oldnew = { label.contains("#") ? label.substring(label.indexOf('#') + 1) : label + ":" + entry.getValue(), null };
        ...
        for (Entry<String, String[]> entry : parameterMap.entrySet()) {
            ...
            } else if (key.equalsIgnoreCase("newskin") && !survey.getIsOPC() && entry.getValue()[0].trim().length() > 0) {
...
@ResponseBody
public ECFGlobalResult ecfGlobalResultsJSON(String shortname,HttpServletRequest request){
    // PARAMS
    ...
    Survey survey = (filter != null) ? surveyService.getSurvey(filter.getSurveyId(), true) : surveyService.getSurvey(shortname, false, true, false, false, null, true, false);
...
public void upload(HttpServletRequest request,HttpServletResponse response,boolean isImage){
    ...
    try {
        ...
        if (!u.getId().equals(form.getSurvey().getOwner().getId()) && u.getGlobalPrivileges().get(GlobalPrivilege.FormManagement) < 2 && u.getLocalPrivileges().get(LocalPrivilege.FormManagement) < 2) {
            throw new ForbiddenURLException();
        }
        String filenamelc = filename.toLowerCase();
        if (isImage && !filenamelc.endsWith("png") && !filenamelc.endsWith("jpg") && !filenamelc.endsWith("gif") && !filenamelc.endsWith("bmp")) {
            error = true;
...
public ModelAndView testPOST(String shortname,HttpServletRequest request,Locale locale){
    ...
    Map<Element, String> validation = SurveyHelper.validateAnswerSet(answerSet, answerService, invisibleElements, resources, locale, request.getParameter("draftid"), request, false, user, fileService);
    ...
    survey = surveyService.getSurvey(survey.getId(), lang);
    if (survey.getIsQuiz()) {
        ModelAndView result = new ModelAndView("management/testQuizResult", Constants.UNIQUECODE, answerSet.getUniqueCode());
...
public ModelAndView preparestatisticsquiz(String id,String exportId,Locale locale,HttpServletRequest request){
    Export export = exportService.getExport(Integer.parseInt(exportId), false);
    if (export == null || export.getState() != ExportState.Pending || !export.getSurvey().getId().equals(Integer.parseInt(id)))
        return null;
    ...
    ModelAndView results = results(survey, new HashMap<>(), null, survey.getIsDraft(), export.isAllAnswers(), filter, true);
...
@ResponseBody
public ECFIndividualResult ecfResultJSON(String shortname,HttpServletRequest request){
...
public void uploadImage(String shortname,HttpServletRequest request,HttpServletResponse response){
    upload(request, response, true);
}


public ModelAndView results(Survey survey,Map<String,String[]> parameters,HttpServletRequest request,Boolean draft,Boolean pallanswers,ResultFilter resultFilter,boolean forPDF){
    ...
    boolean deletecolumn = request != null && request.getParameter("operation") != null && request.getParameter("operation").equalsIgnoreCase("deletecolumn");
    ...
    if (active) {
        ...
        } else if (request != null && !user.getId().equals(survey.getOwner().getId()) && user.getGlobalPrivileges().get(GlobalPrivilege.FormManagement) < 2 && user.getLocalPrivileges().get(LocalPrivilege.AccessResults) < 1) {
            active = false;
            allanswers = false;
            survey = surveyService.getSurvey(shortname, true, false, false, false, null, true, true);
        }
    ...
    if (multidelete) {
        if (!user.getId().equals(survey.getOwner().getId()) && user.getGlobalPrivileges().get(GlobalPrivilege.FormManagement) < 2 && user.getLocalPrivileges().get(LocalPrivilege.AccessResults) < 2) {
            throw new ForbiddenURLException();
        }
    ...
    if (!resultType.equalsIgnoreCase("content") && !resultType.equalsIgnoreCase("charts") && !resultType.equalsIgnoreCase("statistics") && !resultType.equalsIgnoreCase("ecf") && !resultType.equalsIgnoreCase("ecf2") && !resultType.equalsIgnoreCase("ecf3")) {
        resultType = "content";
    }
...
public String getNumberingLabel(int value){
    switch(value) {
        case 0:
            return "disabled";
        case 1:
...
public ModelAndView test(String shortname,HttpServletRequest request,Locale locale){
    User u = sessionService.getCurrentUser(request);
    ...
    if (!u.getId().equals(form.getSurvey().getOwner().getId()) && u.getGlobalPrivileges().get(GlobalPrivilege.FormManagement) < 2 && u.getLocalPrivileges().get(LocalPrivilege.AccessResults) < 2 && u.getLocalPrivileges().get(LocalPrivilege.AccessDraft) < 1 && u.getLocalPrivileges().get(LocalPrivilege.FormManagement) < 2) {
        throw new ForbiddenURLException();
    }
...
@ResponseBody
public Map<String,String> statisticsDelphiJSON(String shortname,HttpServletRequest request){
    try {
        ResultFilter filter = sessionService.getLastResultFilter(request);
...
public ModelAndView edit(String shortname,HttpServletRequest request,Locale locale){
    Form form;
...
public ModelAndView prepareECFGlobalResults(String id,String exportId,Locale locale,HttpServletRequest request){
    Export export = exportService.getExport(Integer.parseInt(exportId), false);
    if (export == null) {
...
@ResponseBody
public Statistics statisticsJSON(String shortname,HttpServletRequest request){
    try {
        ...
        if (statisticsrequestid == null) {
            ...
            if (useworkerserver.equalsIgnoreCase("true") && isworkerserver.equalsIgnoreCase("false")) {
                ...
                try {
                    ...
                    if (result.toString().equals("OK")) {
                        ...
                        logger.error("calling worker server for statisticsrequest " + statisticsRequest.getId() + " returned" + result);
                        return null;
                    }
            ...
            return answerService.getStatisticsOrStartCreator(survey, filter, false, active.equalsIgnoreCase("true") && allanswers.equalsIgnoreCase("true"), true);
        } else {
...
public void ensurePropertiesDependingOnSurveyType(Survey survey,boolean creation){
    if (survey.getIsDelphi()) {
        // should always be activated for delphi surveys
        survey.setChangeContribution(true);
        // should always be deactivated for delphi surveys
...
@ResponseBody
public Map<String,String> statisticsDelphiMedianJSON(String shortname,HttpServletRequest request){
    try {
        ResultFilter filter = sessionService.getLastResultFilter(request);
        if (filter == null || filter.getSurveyId() == 0) {
            return Collections.emptyMap();
...
public ModelAndView confirmation(HttpServletRequest request,HttpServletResponse response,Locale locale,Model modelMap){
...
public ModelAndView overviewPOST(String shortname,HttpServletRequest request,Locale locale){
...
public ModelAndView clearchanges(String shortname,HttpServletRequest request,Locale locale){
    Form form;
    form = sessionService.getForm(request, shortname, false, false);
    User u = sessionService.getCurrentUser(request);
    if (!u.getId().equals(form.getSurvey().getOwner().getId()) && u.getGlobalPrivileges().get(GlobalPrivilege.FormManagement) < 2 && u.getLocalPrivileges().get(LocalPrivilege.FormManagement) < 2) {
...
public void importSurvey(HttpServletRequest request,HttpServletResponse response,Locale locale){
    ...
    try {
        ...
        if (filename.toLowerCase().endsWith("zip")) {
            result = SurveyExportHelper.importIPMSurvey(file, surveyService, sessionService.getCurrentUser(request), fileService, servletContext, u.getEmail());
...
public ModelAndView preparecharts(String id,String exportId,Locale locale){
    try {
        Export export = exportService.getExport(Integer.parseInt(exportId), false);
        if (export == null || export.getState() != ExportState.Pending || !export.getSurvey().getId().equals(Integer.parseInt(id)))
            return null;
        Survey survey = surveyService.getSurvey(Integer.parseInt(id), false, true);
        surveyService.initializeSurvey(survey);
        ModelAndView results = results(survey, new HashMap<>(), null, survey.getIsDraft(), export.isAllAnswers(), export.getResultFilter(), true);
        Publication publication = new Publication();
...
public ModelAndView activate(String shortname,HttpServletRequest request,Locale locale){
    Form form;
    form = sessionService.getForm(request, shortname, false, false);
...
public boolean invalid(String term){
...
public boolean checkConclusiveSurveyType(Survey survey){
    ...
    if (isNormal && !isQuiz && !isOPC && !isDelphi)
        return true;
    if (!isNormal && isQuiz && !isOPC && !isDelphi)
        return true;
    if (!isNormal && !isQuiz && isOPC && !isDelphi)
...
public ModelAndView recalculateScore(String shortname,String id,HttpServletRequest request,Locale locale){
    Survey survey = null;
    User u = sessionService.getCurrentUser(request);
    survey = surveyService.getSurvey(Integer.parseInt(id));
    sessionService.upgradePrivileges(survey, u, request);
    if (!u.getId().equals(survey.getOwner().getId()) && u.getGlobalPrivileges().get(GlobalPrivilege.FormManagement) < 2 && u.getLocalPrivileges().get(LocalPrivilege.AccessResults) < 1 && u.getLocalPrivileges().get(LocalPrivilege.AccessDraft) < 2) {
        throw new ForbiddenURLException();
    }
...
public ModelAndView propertiesPost(Form form,BindingResult bindingresult,HttpServletRequest request,Locale locale){
...
@ResponseBody
public String deleteDownloadFile(HttpServletRequest request,HttpServletResponse response){
    try {
        String uid = request.getParameter("uid");
...
public ModelAndView access(String shortname,HttpServletRequest request,Locale locale){
    User u = sessionService.getCurrentUser(request);
    Form form;
...
public ModelAndView createNewSurvey(HttpServletRequest request,HttpServletResponse response,Locale locale){
    ...
    if (request.getParameter("uuid") != null && request.getParameter("uuid").length() > 0) {
        ...
        try {
            ...
            if (result != null && result.getSurvey() != null) {
                ...
                if (request.getParameter("origin") != null && request.getParameter("origin").startsWith(contextpath)) {
                    return new ModelAndView("redirect:" + request.getParameter("origin").substring(contextpath.length()) + "?imported=" + Tools.escapeHTML(parameters.get(Constants.SHORTNAME)[0]) + "&invalidCodeFound=" + result.isInvalidCodeFound());
                } else {
                    return new ModelAndView("redirect:/forms?imported=" + Tools.escapeHTML(parameters.get(Constants.SHORTNAME)[0]) + "&invalidCodeFound=" + result.isInvalidCodeFound());
                }
            } else {
                String message = resources.getMessage("message.FileCouldNotBeImported", null, "The file could not be imported.", locale);
        ...
        if (original != null) {
            if (!original.getOwner().getId().equals(u.getId()) && u.getGlobalPrivileges().get(GlobalPrivilege.FormManagement) < 2) {
            ...
            if (!u.getId().equals(original.getOwner().getId()) && u.getGlobalPrivileges().get(GlobalPrivilege.FormManagement) < 2 && u.getLocalPrivileges().get(LocalPrivilege.FormManagement) < 1) {
                throw new ForbiddenURLException();
            }
            Map<String, String[]> parameterMap = Ucs2Utf8.requestToHashMap(request);
            ...
            try {
                ...
                // leave blank for old surveys that have no uniqueIds
                oldToNewUniqueIds.put("", "");
                // recreate unique ids
                ...
                copy.setIsDelphi(request.getParameter("delphi") != null && request.getParameter("delphi").equalsIgnoreCase("true"));
                copy.setIsECF(request.getParameter("ecf") != null && request.getParameter("ecf").equalsIgnoreCase("true"));
...
@ResponseBody
public boolean pendingChangesForPublishing(String shortname,HttpServletRequest request,HttpServletResponse response){
...
@ResponseBody
public ECFOrganizationalResult ecfOrganizationalResultsJSON(String shortname,HttpServletRequest request){
    ResultFilter filter = sessionService.getLastResultFilter(request);
    Survey survey = (filter != null) ? surveyService.getSurvey(filter.getSurveyId(), true) : surveyService.getSurvey(shortname, false, true, false, false, null, true, false);
    this.sessionService.userIsResultReadAuthorized(survey, request);
    try {
...
public ModelAndView editPOST(String shortname,HttpServletRequest request,Locale locale){
    Form form;
    form = sessionService.getForm(request, shortname, false, false);
    User u = sessionService.getCurrentUser(request);
    if (!u.getId().equals(form.getSurvey().getOwner().getId()) && u.getGlobalPrivileges().get(GlobalPrivilege.FormManagement) < 2 && u.getLocalPrivileges().get(LocalPrivilege.FormManagement) < 2) {
...
public ModelAndView unpublish(String shortname,HttpServletRequest request,Locale locale){
    Form form;
    form = sessionService.getForm(request, shortname, false, false);
    User u = sessionService.getCurrentUser(request);
    if (!u.getId().equals(form.getSurvey().getOwner().getId()) && u.getGlobalPrivileges().get(GlobalPrivilege.FormManagement) < 2 && u.getLocalPrivileges().get(LocalPrivilege.FormManagement) < 2) {
...
public ModelAndView prepareECFIndividualResults(String id,String exportId,Locale locale,HttpServletRequest request){
    return null;
}
...
public ModelAndView exportSurvey(String answers,String shortname,HttpServletRequest request,HttpServletResponse response,Locale locale){
    Form form = sessionService.getForm(request, shortname, false, false);
    ...
    User u = sessionService.getCurrentUser(request);
    if (!u.getId().equals(survey.getOwner().getId()) && u.getGlobalPrivileges().get(GlobalPrivilege.FormManagement) < 2 && (u.getLocalPrivileges().get(LocalPrivilege.FormManagement) < 1 || answers != null && answers.equalsIgnoreCase("true"))) {
        throw new ForbiddenURLException();
    }
...
@ResponseBody
public List<String> resultsJSON(String shortname,HttpServletRequest request){
    try {
        User user = sessionService.getCurrentUser(request);
        ...
        List<List<String>> answersets = reportingService.getAnswerSets(survey, filter, sqlPagination, addlinks, false, showuploadedfiles, false, false, true);
        if (answersets != null) {
                ...
                if (visibleQuestions.contains(question) || survey.getMissingElements().contains(question)) {
                            ...
                            for (Answer answer : answerSet.getAnswers(question.getId(), question.getUniqueId())) {
                                for (File file : answer.getFiles()) {
                                    if (isOwner || user == null || user.getFormPrivilege() > 1 || user.getLocalPrivilegeValue("AccessResults") > 1 || (survey.getIsDraft() && user.getLocalPrivilegeValue("AccessDraft") > 0)) {
                                        s.append("<a target='blank' href='").append(contextpath).append("/files/").append(survey.getUniqueId()).append(Constants.PATH_DELIMITER).append(file.getUid()).append("'>").append(file.getName()).append("</a><br />");
                                    } else {
                    ...
                    if (survey.getIsDelphi() && question.getIsDelphiQuestion()) {
                        if (filter.getVisibleExplanations().contains(question.getId().toString())) {
                            try {
                                final String explanation = answerExplanationService.getFormattedExplanationWithFiles(answerSet.getId(), question.getUniqueId(), survey.getUniqueId(), true);
...
public ModelAndView preparestatistics(String id,String exportId,Locale locale,HttpServletRequest request){
    Export export = exportService.getExport(Integer.parseInt(exportId), false);
    if (export == null) {
        logger.error("export is null");
        return null;
...
@ResponseBody
public XHTMLValidation checkXHTML(String data,HttpServletRequest request){
...
public ModelAndView repairxhtml(String shortname,HttpServletRequest request,Locale locale){
    Form form;
    form = sessionService.getForm(request, shortname, false, false);
...
public ModelAndView checkInternetConnection(String shortname,HttpServletRequest request,Locale locale){
    String message = "";
    try {
...
public ModelAndView overview(String shortname,HttpServletRequest request,Locale locale){
    User user = sessionService.getCurrentUser(request);
    Form form;
    ...
    form.setSurvey(survey);
    if (!form.getSurvey().getOwner().getId().equals(user.getId()) && user.getGlobalPrivileges().get(GlobalPrivilege.FormManagement) < 2 && user.getLocalPrivileges().get(LocalPrivilege.FormManagement) < 1) {
...
public ModelAndView applyChanges(String shortname,HttpServletRequest request,Locale locale){
    Form form;
    form = sessionService.getForm(request, shortname, false, false);
    try {
        User u = sessionService.getCurrentUser(request);
...
public ModelAndView accessPOST(String shortname,HttpServletRequest request,Locale locale){
    String target = request.getParameter("target");
    if (target != null) {
...
@ResponseBody
public List<Template> saveTemplate(String shortname,HttpServletRequest request,HttpServletResponse response,Locale locale){
    Form form;
    try {
        form = sessionService.getForm(request, shortname, false, false);
        Map<String, String[]> parameterMap = Ucs2Utf8.requestToHashMap(request);
...
public ModelAndView publish(String shortname,HttpServletRequest request,Locale locale){
    Form form;
    form = sessionService.getForm(request, shortname, false, false);
    if (form.getSurvey().getIsFrozen()) {
        throw new FrozenSurveyException();
...
@ResponseBody
public ECFProfileResult ecfProfileAssessmentResultsJSON(String shortname,HttpServletRequest request){
...
public ModelAndView properties(String shortname,HttpServletRequest request,Locale locale){
    Form form;
    User user = sessionService.getCurrentUser(request);
    Survey survey = surveyService.getSurveyByShortname(shortname, true, user, request, false, true, true, false);
...
@ResponseBody
public SearchAndReplaceResult forms(String shortname,HttpServletRequest request,HttpServletResponse response,Locale locale,Model model){
    try {
        Form form;
        try {
            form = sessionService.getForm(request, shortname, false, false);
