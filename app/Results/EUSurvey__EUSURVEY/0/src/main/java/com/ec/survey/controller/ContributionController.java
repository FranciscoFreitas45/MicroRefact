...
public ModelAndView editcontributionfrombackoffice(String code,Locale locale,HttpServletRequest request){
    return editContributionInner(code, locale, request, true, true, false);
}


@GetMapping(value = "/contribution/{uid}/preview")
public ModelAndView preview(String uid,HttpServletRequest request,Locale locale){
    if (uid != null && uid.length() > 0) {
        ...
        if (answerSet != null) {
            ...
            if (answerSet.getLanguageCode() != null && !answerSet.getSurvey().getLanguage().getCode().equalsIgnoreCase(answerSet.getLanguageCode())) {
                Survey translated = SurveyHelper.createTranslatedSurvey(answerSet.getSurvey().getId(), answerSet.getLanguageCode(), surveyService, translationService, true);
...
@ResponseBody
public String deleteContribution(String code,HttpServletRequest request,Locale locale){
    ...
    if (answerSet.getSurvey().getIsDraft()) {
        this.activityService.log(405, answerSet.getUniqueCode(), null, currentUser.getId(), answerSet.getSurvey().getUniqueId());
    } else {
        this.activityService.log(402, null, answerSet.getUniqueCode(), currentUser.getId(), answerSet.getSurvey().getUniqueId());
    }
    return "success";
}
...
public ModelAndView preparecontribution(String code,Locale locale,HttpServletRequest request){
...
public ModelAndView editcontribution(String code,Locale locale,HttpServletRequest request){
    return editContributionInner(code, locale, request, false, true, false);
}


public ModelAndView editContributionInner(String code,Locale locale,HttpServletRequest request,boolean fromBackOffice,boolean useNewestSurvey,boolean isdraft){
    AnswerSet answerSet;
    String draftid = "";
    if (isdraft) {
    ...
    if (answerSet != null) {
        ...
        boolean isPDF = request.getRequestURI().contains("preparecontribution") || request.getRequestURI().contains("preparepublishedcontribution") || request.getRequestURI().contains("preparedraft");
        if (isPDF) {
        // ignore authorization for PDF export
            ...
            model.addObject(Constants.MESSAGE, resources.getMessage("error.ContributionClosedSurvey", null, "The survey has been closed and it is not possible to access the contribution anymore.", locale));
            return model;
        } else if (u == null && !draft.getChangeContribution()) {
            ModelAndView model = new ModelAndView(Constants.VIEW_ERROR_GENERIC);
            model.addObject(Constants.MESSAGE, resources.getMessage("error.ContributionEditNotAllowed", null, "The survey has been closed and it is not possible to access the contribution anymore.", locale));
            return model;
        } else if (u == null) {
        // ok
        } else {
            if (!u.getId().equals(answerSet.getSurvey().getOwner().getId()) && u.getGlobalPrivileges().get(GlobalPrivilege.FormManagement) < 2 && u.getLocalPrivileges().get(LocalPrivilege.AccessResults) < 1) {
        ...
        ModelAndView model = new ModelAndView(Constants.VIEW_ERROR_GENERIC);
        model.addObject(Constants.MESSAGE, resources.getMessage("error.ContributionNotLoaded", null, "The contribution could not be loaded.", locale));
        return model;
    }
}
...
public ModelAndView quizresults(String code,Locale locale,HttpServletRequest request){
...
public AnswerSet getAnswerSet(String code,HttpServletRequest request){
    ...
    if (answerSetOrNull.getSurvey() != null) {
        Survey draftSurvey = surveyService.getSurveyByUniqueId(answerSetOrNull.getSurvey().getUniqueId(), false, true);
...
public ModelAndView download(String code,HttpServletRequest request,HttpServletResponse response){
    try {
        AnswerSet answerSet = getAnswerSet(code, request);
        if (answerSet != null) {
...
public ModelAndView processSubmit(String code,HttpServletRequest request,Locale locale){
    try {
        ...
        if (oldAnswerSet != null) {
            ...
            SurveyHelper.parseAndMergeAnswerSet(request, origsurvey, uniqueCode, oldAnswerSet, oldAnswerSet.getLanguageCode(), null, fileService);
            Set<String> invisibleElements = new HashSet<>();
            Map<Element, String> validation = SurveyHelper.validateAnswerSet(oldAnswerSet, answerService, invisibleElements, resources, locale, request.getParameter("draftid"), request, true, u, fileService);
            ...
            ModelAndView model = new ModelAndView(Constants.VIEW_ERROR_GENERIC);
            model.addObject(Constants.MESSAGE, resources.getMessage("error.ProblemDuringSave", null, "There was a problem during the save process.", locale));
            return model;
        }
    } catch (Exception e) {
        logger.error(e.getLocalizedMessage(), e);
...
@ResponseBody
public ECFIndividualResult ecfResultJSON(HttpServletRequest request,Locale locale){
...
public ModelAndView preparedraft(String code,Locale locale,HttpServletRequest request){
...
public ModelAndView showforpublishedpdf(String id,Locale locale,HttpServletRequest request){
