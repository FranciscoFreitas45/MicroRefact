...
import com.ec.survey.DTO.QuizExecutor;
@Controller("homeController")
public class HomeController extends BasicController{

@Value("${stresstests.createdata}")
 public  String createStressData;
...
@Value("${support.recipient}")
 private  String supportEmail;

@Value("${support.recipientinternal}")
 private  String supportEmailInternal;
...
public String helpauthorsinternal(HttpServletRequest request,Locale locale,Model model){
...
public String deleteaccount(HttpServletRequest request,String id,String code,Locale locale,Model model){
    try {
...
public boolean checkValid(WrongAttempts attempts){
    if (attempts == null || attempts.getCounter() < 10 || attempts.getLockDate() == null)
        return true;
...
public String supportPOST(HttpServletRequest request,Locale locale,ModelMap model){
...
public ModelAndView publicsurveysrunner(HttpServletRequest request){
...
public String validateNewEmail(HttpServletRequest request,String id,String code,Locale locale,Model model){
    try {
...
public ModelAndView publicsurveys(HttpServletRequest request){
...
public ModelAndView editcontributionPost(HttpServletRequest request,Locale locale){
    try {
        ...
        if (!checkCaptcha(request)) {
            ...
            if (attempts == null)
                attempts = new WrongAttempts(remoteAddr);
...
@ResponseBody
public String downloadcontribution(HttpServletRequest request,HttpServletResponse response){
    try {
        if (!checkCaptcha(request)) {
            return "errorcaptcha";
        }
...
public String privacystatementinternal(HttpServletRequest request,Locale locale,Model model,boolean tos){
...
@ResponseBody
public List<Survey> publicsurveysjson(HttpServletRequest request){
    if (!enablepublicsurveys.equalsIgnoreCase("true")) {
        throw new InvalidURLException();
    }
    int itemsPerPage = 10;
...
public String getBrowserInformation(HttpServletRequest request,Locale locale){
...
public String validate(String id,String code,Locale locale,Model model){
    try {
...
public String delphi(ModelMap model,String uidOrShortname){
...
public String documentation(Locale locale,ModelMap model){
    if (isShowEcas())
        model.addAttribute("showecas", true);
    // CASOSS
    if (isCasOss())
        model.addAttribute("casoss", true);
    model.put("continueWithoutJavascript", true);
    return "home/documentation";
}
...
public String reportAbuse(HttpServletRequest request,Locale locale,Model model){
    model.addAttribute("lang", locale.getLanguage());
    model.addAttribute("runnermode", true);
...
public ModelAndView downloadcontributionGet(HttpServletRequest request){
...
public String helpparticipantsinternal(HttpServletRequest request,Locale locale,Model model){
    model.addAttribute("runnermode", true);
...
public ModelAndView reportAbusePOST(HttpServletRequest request,Locale locale,HttpServletResponse response){
