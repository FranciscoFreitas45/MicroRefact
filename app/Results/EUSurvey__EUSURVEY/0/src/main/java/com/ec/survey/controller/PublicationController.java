...
import com.ec.survey.DTO.ResultsExecutor;
import com.ec.survey.DTO.StatisticsExecutor;
@Controller
...
@Value("${stresstests.createdata}")
 private  String createStressData;


@GetMapping(value = "/exportfiles/{id}", headers = "Accept=*/*")
@ResponseBody
public String exportfiles(String id,HttpServletRequest request,HttpServletResponse response,Locale locale){
    try {
        Survey survey = surveyService.getSurvey(Integer.parseInt(id), false, true);
        if (survey != null && survey.getPublication().isShowContent() && survey.getPublication().getShowUploadedDocuments()) {
            if (survey.getPublication().getPassword() != null && survey.getPublication().getPassword().length() > 0) {
                ...
                if (publicationpassword == null || !publicationpassword.equals(survey.getPublication().getPassword())) {
            ...
            resultsExecutor.init(survey, filter, email, sender, host, fileDir, "files", resources, locale, question);
...
public ModelAndView authenticate(String shortname,HttpServletRequest request,Locale locale){
    return new ModelAndView("publication/auth", Constants.SHORTNAME, shortname);
}


@RequestMapping(value = "/{shortname}")
public ModelAndView publication(String shortname,HttpServletRequest request,Locale locale){
    try {
                ...
                if (entry.getKey().startsWith(Constants.FILTER)) {
                    ...
                    if (value.replace(";", "").trim().length() > 0) {
                        if (userFilter.getFilterValues().containsKey(questionId)) {
                            userFilter.getFilterValues().put(questionId, userFilter.getFilterValues().get(questionId) + ";" + value);
                        } else {
        ...
        if (survey != null && survey.getPublication() != null) {
            ...
            if (publication.isShowContent() || publication.isShowStatistics() || publication.isShowCharts()) {
                ...
                Form form = new Form(survey, translationService.getTranslationsForSurvey(survey.getId(), false), survey.getLanguage(), resources, contextpath);
    ...
    ModelAndView result = new ModelAndView(Constants.VIEW_ERROR_GENERIC, Constants.MESSAGE, resources.getMessage("error.NoPublishedResults", null, "This survey has no published results", locale));
...
public ModelAndView authenticatePOST(String shortname,HttpServletRequest request,Locale locale){
    Map<String, String[]> parameters = Ucs2Utf8.requestToHashMap(request);
    String publicationpassword = parameters.get("publicationpassword")[0];
    Survey survey = surveyService.getSurveyByShortname(shortname, false, null, request, false, true, true, false);
    if (survey != null) {
        if (survey.getPublication().getPassword() != null && survey.getPublication().getPassword().length() > 0 && publicationpassword != null && publicationpassword.equals(survey.getPublication().getPassword())) {
...
@ResponseBody
public Map<String,String> individualJSON(HttpServletRequest request,HttpServletResponse response){
    Map<String, String[]> parameters = Ucs2Utf8.requestToHashMap(request);
    String surveyid = "";
    ...
    try {
        if (surveyid.length() > 0 && counter.length() > 0) {
            ...
            if (survey != null && survey.getPublication().isShowContent()) {
                ...
                ResultFilter filter = survey.getPublication().getFilter();
                ResultFilter userFilter = (ResultFilter) request.getSession().getAttribute("lastPublishedFilter" + survey.getShortname());
                if (userFilter != null)
                    filter = userFilter;
                ...
                for (Answer answer : answerSet.getAnswers()) {
                    ...
                    if (question instanceof Text && matrixMapByUid.containsKey(answer.getPossibleAnswerUniqueId())) {
                        if (result.containsKey(answer.getQuestionId().toString())) {
                            result.put(answer.getQuestionId().toString(), result.get(answer.getQuestionId().toString()) + "<br />" + matrixMapByUid.get(answer.getPossibleAnswerUniqueId()).getTitle());
                        } else {
                            result.put(answer.getQuestionId().toString(), matrixMapByUid.get(answer.getPossibleAnswerUniqueId()).getTitle());
                        }
                        if (result.containsKey(answer.getQuestionUniqueId())) {
                            result.put(answer.getQuestionUniqueId(), result.get(answer.getQuestionUniqueId()) + "<br />" + matrixMapByUid.get(answer.getPossibleAnswerUniqueId()).getTitle());
                        } else {
                            result.put(answer.getQuestionUniqueId(), matrixMapByUid.get(answer.getPossibleAnswerUniqueId()).getTitle());
                        }
                    } else if (question == null && matrixMap.containsKey(answer.getPossibleAnswerId())) {
                        if (result.containsKey(answer.getQuestionId().toString())) {
                            result.put(answer.getQuestionId().toString(), result.get(answer.getQuestionId().toString()) + "<br />" + matrixMap.get(answer.getPossibleAnswerId()).getTitle());
                        } else {
                            result.put(answer.getQuestionId().toString(), matrixMap.get(answer.getPossibleAnswerId()).getTitle());
                        }
                        if (result.containsKey(answer.getQuestionUniqueId())) {
                            result.put(answer.getQuestionUniqueId(), result.get(answer.getQuestionUniqueId()) + "<br />" + matrixMap.get(answer.getPossibleAnswerId()).getTitle());
                        } else {
                            result.put(answer.getQuestionUniqueId(), matrixMap.get(answer.getPossibleAnswerId()).getTitle());
                        }
                    } else if (question instanceof ChoiceQuestion) {
                        String title = ((ChoiceQuestion) question).getPossibleAnswer(answer.getPossibleAnswerId()) != null ? ((ChoiceQuestion) question).getPossibleAnswer(answer.getPossibleAnswerId()).getTitle() : "";
                        if (title.length() == 0) {
                            title = ((ChoiceQuestion) question).getPossibleAnswerByUniqueId(answer.getPossibleAnswerUniqueId()) != null ? ((ChoiceQuestion) question).getPossibleAnswerByUniqueId(answer.getPossibleAnswerUniqueId()).getTitle() : "";
                        }
                        if (result.containsKey(answer.getQuestionId().toString())) {
                            result.put(answer.getQuestionId().toString(), result.get(answer.getQuestionId().toString()) + "<br />" + title);
                        } else {
                            result.put(answer.getQuestionId().toString(), title);
                        }
                        if (result.containsKey(answer.getQuestionUniqueId())) {
                            result.put(answer.getQuestionUniqueId(), result.get(answer.getQuestionUniqueId()) + "<br />" + title);
                        } else {
                        ...
                        if (answer.getFiles() != null && !answer.getFiles().isEmpty()) {
                            for (File file : answer.getFiles()) {
                                String name = "<a target='blank' href='" + contextpath + "/files/" + file.getUid() + "'>" + file.getName() + "</a><br />";
                                if (result.containsKey(answer.getQuestionId().toString())) {
                                    result.put(answer.getQuestionId().toString(), result.get(answer.getQuestionId().toString()) + "<br />" + name);
                                } else {
                                    result.put(answer.getQuestionId().toString(), name);
                                }
                                if (result.containsKey(answer.getQuestionUniqueId())) {
                                    result.put(answer.getQuestionUniqueId(), result.get(answer.getQuestionUniqueId()) + "<br />" + name);
                                } else {
                        ...
                        // probably a deleted question / answer
                            ...
                            if (question instanceof FreeTextQuestion && ((FreeTextQuestion) question).getIsPassword() && answer.getValue().length() > 0) {
                                result.put(answer.getQuestionId().toString(), "********");
                                result.put(answer.getQuestionUniqueId(), "********");
                            } else {
                                if (result.containsKey(answer.getQuestionId().toString())) {
                                    result.put(answer.getQuestionId().toString(), result.get(answer.getQuestionId().toString()) + "<br />" + ConversionTools.escape(answer.getValue()));
                                } else {
                                    result.put(answer.getQuestionId().toString(), ConversionTools.escape(answer.getValue()));
                                }
                                if (result.containsKey(answer.getQuestionUniqueId())) {
                                    result.put(answer.getQuestionUniqueId(), result.get(answer.getQuestionUniqueId()) + "<br />" + ConversionTools.escape(answer.getValue()));
                                } else {
                                    result.put(answer.getQuestionUniqueId(), ConversionTools.escape(answer.getValue()));
                                }
                            }
                        }
                    }
...
@ResponseBody
public String export(String type,String format,String id,HttpServletRequest request,HttpServletResponse response,Locale locale){
    try {
        if (!checkCaptcha(request)) {
            return "errorcaptcha";
        }
        Survey survey = surveyService.getSurvey(Integer.parseInt(id), false, true);
        if (survey != null && ((type.startsWith("Statistics") && survey.getPublication().isShowStatistics()) || (!type.startsWith("Statistics") && survey.getPublication().isShowContent()))) {
            if (survey.getPublication().getPassword() != null && survey.getPublication().getPassword().length() > 0) {
                String publicationpassword = (String) request.getSession().getAttribute("publicationpassword");
                if (publicationpassword == null || !publicationpassword.equals(survey.getPublication().getPassword())) {
            ...
            export.init(survey, type, format, survey.getPublication().getFilter().getHash(false), email, sender, host, locale);
...
@ResponseBody
public String exportresultsxls(String id,HttpServletRequest request,HttpServletResponse response,Locale locale){
    try {
        Survey survey = surveyService.getSurvey(Integer.parseInt(id), false, true);
        if (survey != null && survey.getPublication().isShowContent()) {
            if (survey.getPublication().getPassword() != null && survey.getPublication().getPassword().length() > 0) {
            ...
            ResultFilter filter = survey.getPublication().getFilter();
            ResultsExecutor resultsExecutor = (ResultsExecutor) context.getBean("resultsExecutor");
            resultsExecutor.init(survey, filter, email, sender, host, fileDir, "xls", resources, locale, null);
            taskExecutor.execute(resultsExecutor);
        }
    } catch (Exception e) {
        logger.error(e.getLocalizedMessage(), e);
...
@ResponseBody
public String exportresultsods(String id,HttpServletRequest request,HttpServletResponse response,Locale locale){
    try {
        Survey survey = surveyService.getSurvey(Integer.parseInt(id), false, true);
        if (survey != null && survey.getPublication().isShowContent()) {
            if (survey.getPublication().getPassword() != null && survey.getPublication().getPassword().length() > 0) {
            ...
            ResultFilter filter = survey.getPublication().getFilter();
            ResultsExecutor resultsExecutor = (ResultsExecutor) context.getBean("resultsExecutor");
            resultsExecutor.init(survey, filter, email, sender, host, fileDir, "ods", resources, locale, null);
            taskExecutor.execute(resultsExecutor);
        }
    } catch (Exception e) {
        logger.error(e.getLocalizedMessage(), e);
