...
import java.util;
import com.ec.survey.DTO.KeyValue;
@Service("ldapService")
public class LdapService extends BasicService{

 public  String LDAP_CONSTANT_PREFIX;
...
@Value("${LdapSecurityAuthentication}")
 private  String securityAuthentication;

@Value("${ldap.search.user.format:@null}")
 private  String ldapSearchUserFormat;

@Value("${ldap.search.format:@null}")
 private  String ldapSearchFormat;

@Value("${ldap.mapping.user.departmentNumber:@null}")
 private  String ldapMappingUserDepartmentNumber;

@Value("${ldap.mapping.user.sn:@null}")
 private  String ldapMappingUserSn;

@Value("${ldap.mapping.user.uid:@null}")
 private  String ldapMappingUserUid;

@Value("${ldap.mapping.user.ecMoniker:@null}")
 private  String ldapMappingUserEcMoniker;

@Value("${ldap.mapping.user.o:@null}")
 private  String ldapMappingUserO;

@Value("${ldap.mapping.user.givenName:@null}")
 private  String ldapMappingUserGivenName;

@Value("${ldap.mapping.user.mail:@null}")
 private  String ldapMappingUserMail;

@Value("${ldap.mapping.user.telephoneNumber:@null}")
 private  String ldapMappingUserTelephoneNumber;

@Value("${ldap.mapping.user.employeeType:@null}")
 private  String ldapMappingUserEmployeeType;

@Value("${ldap.mapping.user.recordStatus:@null}")
 private  String ldapMappingUserRecordStatus;

@Value("${ldap.mapping.user.modifyTimstamp:@null}")
 private  String ldapMappingUserModifyTimstamp;

@Value("${ldap.mapping.domain.o:@null}")
 private  String ldapMappingDomainO;

@Value("${ldap.mapping.domain.description:@null}")
 private  String ldapMappingDomainDescription;

@Value("${ldap.search.domains.format:@null}")
 private  String ldapSearchDomainFormat;
...
public SearchControls getSearchControls(LdapSearchTypeEnum typeSearch){
    ...
    SearchControls sc = new SearchControls(SearchControls.ONELEVEL_SCOPE, // 1L, //count limit
    0, // time limit
    0, lstAttr.toArray(new String[lstAttr.size()]), // new String[] {"departmentNumber","o"},//null,//attributes (null = all)
    // return object ?
...
public String getAttributeValue(Attributes attrs,String name,boolean convertToUpper){
    name = Tools.encodeForLDAP(name);
...
public String getFilterStartsWith(String key,String search){
    String result = "";
...
public Set<DepartmentItem> getAllDepartments(){
    ...
    try {
        ...
        if (StringUtils.isEmpty(ldapMappingUserDepartmentNumber)) {
            String message = String.format("The property %s from the spring.properties file MUST have a value", "ldap.mapping.user.departmentNumber");
            throw new MessageException(message);
        }
        if (StringUtils.isEmpty(ldapMappingUserO)) {
            String message = String.format("The property %s from the spring.properties file MUST have a value", "ldap.mapping.user.o");
            throw new MessageException(message);
        }
        ...
        if (ldapMappingUserDepartmentNumber.startsWith(LDAP_CONSTANT_PREFIX)) {
            ...
            if (!ldapMappingDomainO.startsWith(LDAP_CONSTANT_PREFIX)) {
                String message = String.format("The property %s from the spring.properties file MUST be a constant (staring with '%s' value)", "ldap.mapping.domain.o", ldapMappingDomainO);
                throw new MessageException(message);
            } else {
...
public String getFilterContains(String key,String search){
    String result = "";
    if (search != null && search.length() != 0) {
        result = "(" + key + "=*" + search + "*)";
    } else {
    ...
    try {
        ...
        try {
            ...
            while (namingEnumeration.hasMore()) {
                ...
                String email = "";
                if (userAttributes.get(ldapMappingUserMail) != null)
                    email = (String) userAttributes.get(ldapMappingUserMail).get();
                String givenName = "";
                if (userAttributes.get(ldapMappingUserGivenName) != null)
                    givenName = (String) userAttributes.get(ldapMappingUserGivenName).get();
                String ecMoniker = "";
                if (userAttributes.get(ldapMappingUserEcMoniker) != null)
                    ecMoniker = ((String) userAttributes.get(ldapMappingUserEcMoniker).get()).trim();
                String employeeType = "";
                if (userAttributes.get(ldapMappingUserEmployeeType) != null)
                    employeeType = (String) userAttributes.get(ldapMappingUserEmployeeType).get();
                String o = "";
                if (!StringUtils.isEmpty(ldapMappingUserO)) {
                    if (ldapMappingUserO.startsWith(LDAP_CONSTANT_PREFIX)) {
                        o = ldapMappingUserO.replace(LDAP_CONSTANT_PREFIX, "");
                ...
                String surname = "";
                if (userAttributes.get(ldapMappingUserSn) != null)
                    surname = (String) userAttributes.get(ldapMappingUserSn).get();
                String phone = "";
                if (userAttributes.get(ldapMappingUserTelephoneNumber) != null)
                    phone = (String) userAttributes.get(ldapMappingUserTelephoneNumber).get();
...
public String getFilterNotEquals(String key,String search){
    String result = "";
    if (search != null && search.length() != 0) {
        result = "(!(" + key + "=" + search + "))";
    }
...
public boolean isAttributeEligible(String attr){
    ...
    try {
        ...
        if (StringUtils.isEmpty(ldapSearchDomainFormat)) {
            if (StringUtils.isEmpty(ldapMappingDomainDescription)) {
                String message = String.format("Unable to retrieve the necessary information from the config file for the domain value, the property (%s) MUST contain a constant value starting with (%s) but this value is found (%s)", "ldap.mapping.domain.description", LDAP_CONSTANT_PREFIX, ldapMappingDomainDescription);
                throw new MessageException(message);
            }
            ...
            if (StringUtils.isEmpty(ldapMappingDomainO)) {
                String message = String.format("Unable to retrieve the necessary information from the config file for the domain value, the property (%s) MUST contain a constant value staring with (%s) but this value is found (%s)", "ldap.mapping.domain.o", LDAP_CONSTANT_PREFIX, ldapMappingDomainO);
                throw new MessageException(message);
            }
            // check that the value is a constant value start with $
            if (!ldapMappingDomainO.startsWith(LDAP_CONSTANT_PREFIX)) {
                String message = String.format("Unable to retrieve the necessary information from the config file for the domain value, the property (%s) MUST contain a constant value starting with (%s) but this value is found (%s)", "ldap.mapping.domain.o", LDAP_CONSTANT_PREFIX, ldapMappingDomainO);
                throw new MessageException(message);
            }
...
public String getFilterEquals(String key,String search){
    String result = "";
    if (search != null && search.length() != 0) {
        result = "(" + key + "=" + search + ")";
    }
