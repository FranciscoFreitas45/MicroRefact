...
import com.ec.survey.DTO.Paging;
...
@Value("${participant.default.domain:@null}")
...
@ResponseBody
public List<String> tokensjson(HttpServletRequest request){
...
public ModelAndView participantsPOST(String shortname,HttpServletRequest request,Locale locale){
    Form form;
    try {
...
@ResponseBody
public Paging<Attendee> participantsSearchAll(HttpServletRequest request,HttpServletResponse response){
...
@ResponseBody
public List<String> finishedguestlists(HttpServletRequest request){
...
@ResponseBody
public List<Invitation> createTokens(String shortname,HttpServletRequest request,Locale locale){
...
@ResponseBody
public List<ParticipationGroup> participantsjson(String shortname,HttpServletRequest request){
    User user = sessionService.getCurrentUser(request);
...
@ResponseBody
public Paging<Attendee> participantsSearch(HttpServletRequest request,HttpServletResponse response,boolean all){
    User user = sessionService.getCurrentUser(request);
    int owner = user.getId();
    Map<String, String[]> parameters = Ucs2Utf8.requestToHashMap(request);
    String name = "";
    ...
    for (Entry<String, String[]> entry : parameters.entrySet()) {
        try {
            if (entry.getKey().equals("owner") && entry.getValue() != null && entry.getValue().length > 0) {
                ...
                int intKey = Integer.parseInt(entry.getKey());
                if (intKey > 0 && entry.getValue() != null && entry.getValue().length > 0 && !entry.getKey().equalsIgnoreCase("name") && !entry.getKey().equalsIgnoreCase(Constants.EMAIL) && !entry.getKey().equalsIgnoreCase("newPage")) {
                    String[] value = entry.getValue();
                    if (value[0] != null && value[0].length() > 0) {
                        attributeFilter.put(entry.getKey(), entry.getValue()[0]);
                    }
...
@ResponseBody
public List<KeyValue> topDepartments(HttpServletRequest request,HttpServletResponse response){
    if (!isAjax(request)) {
...
public ModelAndView sendInvitations(String shortname,String id,HttpServletRequest request,Locale locale){
    Form form;
    try {
        form = sessionService.getForm(request, shortname, false, false);
    } catch (NoFormLoadedException ne) {
...
@ResponseBody
public String deleteTemplateJSON(HttpServletRequest request,HttpServletResponse response){
    try {
...
@ResponseBody
public String saveTemplateJSON(HttpServletRequest request,HttpServletResponse response){
    try {
...
@ResponseBody
public String participantsDelete(HttpServletRequest request,Locale locale){
...
@ResponseBody
public String saveguestlist(Map json,HttpServletRequest request,Locale locale){
    try {
        User user = sessionService.getCurrentUser(request);
        Form form = sessionService.getFormFromSessionInfo(request);
        ...
        if (id > 0) {
            activityService.log(505, null, participationGroup.getId().toString(), user.getId(), form.getSurvey().getUniqueId(), participationGroup.getNiceType());
            return "successsaved";
        } else {
            activityService.log(501, null, participationGroup.getId().toString(), user.getId(), form.getSurvey().getUniqueId(), participationGroup.getNiceType());
...
@ResponseBody
public List<KeyValue> finishedguestlistsmail(HttpServletRequest request){
...
public List<EcasUser> getInvalidEcasUsers(int participationGroupId,String selectedAttendee,HttpServletRequest request){
...
@ResponseBody
public Paging<EcasUser> usersJSON(HttpServletRequest request,HttpServletResponse response){
    User user = sessionService.getCurrentUser(request);
    Form form = sessionService.getFormFromSessionInfo(request);
    sessionService.upgradePrivileges(form.getSurvey(), user, request);
    int accessPrivilege = 0;
...
@ResponseBody
public String participantsDeactive(HttpServletRequest request,Locale locale){
...
@ResponseBody
public List children(String shortname,HttpServletRequest request){
    String id = request.getParameter("id");
...
@ResponseBody
public InvitationTemplate loadTemplateJSON(HttpServletRequest request,HttpServletResponse response){
    try {
        String id = request.getParameter("id");
        User user = sessionService.getCurrentUser(request);
        if (user == null)
...
@ResponseBody
public String participantsActivate(HttpServletRequest request,Locale locale){
    return participantsOperation(request, "activate", locale);
}


@PostMapping(value = "/sendInvitations")
public ModelAndView sendInvitationsPOST(String shortname,HttpServletRequest request,Locale locale){
    Form form;
    try {
        form = sessionService.getForm(request, shortname, false, false);
    } catch (NoFormLoadedException ne) {
...
@ResponseBody
public List<KeyValue> departments(HttpServletRequest request,HttpServletResponse response){
    return ldapService.getDepartments(request.getParameter("term"));
}


public String participantsOperation(HttpServletRequest request,String operation,Locale locale){
    try {
        String id = request.getParameter("id");
        ParticipationGroup g = participationService.get(Integer.parseInt(id), true);
        Survey survey = surveyService.getSurveyByUniqueId(g.getSurveyUid(), false, true);
...
public ModelAndView participants(String shortname,HttpServletRequest request,Locale locale){
    User u = sessionService.getCurrentUser(request);
    Form form;
