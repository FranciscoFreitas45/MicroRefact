...
public int getNumberOfOpenInvitations(String surveyuid){
    ...
    Query hquery = session.createQuery("SELECT count(*) " + "FROM Invitation i, ParticipationGroup pg " + "WHERE pg.surveyUid=:uid " + "AND i.participationGroupId = pg.id " + "AND i.answers = 0").setString("uid", surveyuid);
...
public int getNumberOfInvitations(String surveyuid){
    Session session = sessionFactory.getCurrentSession();
    Query hquery = session.createQuery("SELECT count(*) " + "FROM Invitation i, ParticipationGroup pg " + "WHERE pg.surveyUid=:uid " + "AND i.participationGroupId = pg.id ").setString("uid", surveyuid);
    return ConversionTools.getValue(hquery.uniqueResult());
}
