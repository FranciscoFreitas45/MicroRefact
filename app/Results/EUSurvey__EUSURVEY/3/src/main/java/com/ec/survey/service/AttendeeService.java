...
import com.ec.survey.DTO.Share;
...
@Transactional(readOnly = true)
public int getNumberOfAttendees(Integer ownerId,Map<String,String> attributeFilter){
...
public Invitation internalGetInvitationByUniqueId(String uniqueId){
    Session session = sessionFactory.getCurrentSession();
...
public String getSql(Integer ownerId,Map<String,String> attributeFilter,HashMap<String,Object> oQueryParameters,boolean onlywritableshares){
    ...
    if (attributeFilter != null && attributeFilter.size() > 0) {
        for (Entry<String, String> entry : attributeFilter.entrySet()) {
            if (!entry.getKey().equalsIgnoreCase("name") && !entry.getKey().equalsIgnoreCase(Constants.EMAIL) && !entry.getKey().equalsIgnoreCase("owner") && !entry.getKey().equalsIgnoreCase("_csrf") && !entry.getKey().startsWith("visibleAttendee") && entry.getValue() != null && entry.getValue().trim().length() > 0) {
    ...
    if (attributeFilter != null && attributeFilter.size() > 0) {
        ...
        if (attributeFilter.containsKey(Constants.EMAIL) && attributeFilter.get(Constants.EMAIL) != null && attributeFilter.get(Constants.EMAIL).length() > 0) {
...
@Transactional(readOnly = true)
public Invitation getInvitationByUniqueId(String uniqueId){
...
@Transactional(readOnly = false, propagation = Propagation.REQUIRES_NEW)
public void addTokens(List<String> tokens,Integer groupId){
...
@Transactional(readOnly = true)
@SuppressWarnings("unchecked")
public List<Integer> getAccessibleAttendees(Integer ownerId,Map<String,String> attributeFilter){
    Session session = sessionFactory.getCurrentSession();
    HashMap<String, Object> parameters = new HashMap<>();
    String sql = getSql(ownerId, attributeFilter, parameters, false);
...
public int markAttendeesAsDeleted(int ownerId,Map<String,String> filterValues){
    Session session = sessionFactory.getCurrentSession();
    HashMap<String, Object> parameters = new HashMap<>();
    String sql = "UPDATE ATTENDEE SET ATTENDEE_HIDDEN = 1, ATT_UPDATED = NOW(), ATTENDEE_ORIGID = ATTENDEE_ID WHERE ATTENDEE_ID IN (SELECT c.ATTENDEE_ID FROM (SELECT a.ATTENDEE_ID " + getSql(ownerId, filterValues, parameters, true) + ") as c)";
    SQLQuery query = session.createSQLQuery(sql);
    sqlQueryService.setParameters(query, parameters);
...
public void decreaseInvitationAnswer(String invitationUID){
    Session session = sessionFactory.getCurrentSession();
...
public void internalAddInvitation(Invitation invitation){
    Session session = sessionFactory.getCurrentSession();
    boolean saved = false;
    int counter = 1;
    while (!saved) {
