    ...
    try {
        ...
        if (isDelete) {
            ...
            for (Attendee oldAttendee : attendees) {
                if (parametermap.get("batchAttendee" + oldAttendee.getId().toString()) != null && parametermap.get("batchAttendee" + oldAttendee.getId().toString())[0].equals(oldAttendee.getId().toString()) && (oldAttendee.getOwnerId().equals(user.getId()) || user.getGlobalPrivileges().get(GlobalPrivilege.ContactManagement) == 2)) {
                    ...
                    for (Entry<Integer, String> entry : replacements.entrySet()) {
                        ...
                        if (!found && replacements.get(attributeId) != null) {
                            ...
                            Attribute newAttribute = new Attribute(existingAttributeName, entry.getValue());
