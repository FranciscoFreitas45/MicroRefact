        ...
        if (file.exists()) {
            if (filename.toLowerCase().endsWith("csv")) {
                ...
                try {
                    ...
                    if (// no empty values  found in first column
                    numrows == -1) {
                        numrows = maxRowCount;
                    }
                    int start = 0;
                    ...
                    if (// no empty values found in first row
                    numcolumns == -1) {
...
@ResponseBody
public List<AttributeName> configureAttributesJSON(HttpServletRequest request,HttpServletResponse response){
    User user = sessionService.getCurrentUser(request);
    Map<String, String[]> parameterMap = Ucs2Utf8.requestToHashMap(request);
...
public ModelAndView attendees(HttpServletRequest request){
    User user = sessionService.getCurrentUser(request);
    int ownerId;
    if (user.getGlobalPrivileges().get(GlobalPrivilege.ContactManagement) == 2) {
        ownerId = -1;
    ...
    if (!ids.isEmpty()) {
        ...
        if (deleted)
            result.addObject("deletedcontacts", true);
...
public ModelAndView batchEditPOST(HttpServletRequest request,Locale locale){
    User user = sessionService.getCurrentUser(request);
    boolean userChanged = false;
    @SuppressWarnings("unchecked")
    HashMap<String, String> filter = (HashMap<String, String>) request.getSession().getAttribute("attendees-filter");
...
public String getText(Sheet sheet,int row,int cell){
...
public String configureAttributes(HttpServletRequest request){
    User user = sessionService.getCurrentUser(request);
    Map<String, String[]> parameterMap = Ucs2Utf8.requestToHashMap(request);
    user.getSelectedAttributes().clear();
    String idsAsString = parameterMap.get("selectedAttributesOrder")[0];
...
@ResponseBody
public String updateguestlists(String ids,HttpServletRequest request,Locale locale){
    try {
...
@ResponseBody
public List<Attendee> attendeesjson(HttpServletRequest request){
...
public ModelAndView importAttendeesCheck(HttpServletRequest request,Locale locale){
    User user = sessionService.getCurrentUser(request);
    Map<String, String[]> parameterMap = Ucs2Utf8.requestToHashMap(request);
    @SuppressWarnings("unchecked")
...
public java.io.File validateImport1Parameters(String fileName,User user){
...
@ResponseBody
public String checkNewAttendee(HttpServletRequest request,HttpServletResponse response){
...
@SuppressWarnings("unchecked")
@PostMapping
public ModelAndView attendeesPOST(HttpServletRequest request,Locale locale){
    ...
    if (operation != null && operation.equalsIgnoreCase("batchedit")) {
        ...
        for (Attendee attendee : attendees) {
            if (selectedAttendees.contains(attendee.getId())) {
                if (!names.contains(attendee.getName()))
...
public ModelAndView importAttendees2(HttpServletRequest request,Locale locale){
    User user = sessionService.getCurrentUser(request);
    Map<String, String[]> parameterMap = Ucs2Utf8.requestToHashMap(request);
    @SuppressWarnings("unchecked")
    ArrayList<String[]> rows = (ArrayList<String[]>) request.getSession().getAttribute("imported-rows");
