...
public ModelAndView file(String uid,HttpServletRequest request,HttpServletResponse response){
        ...
        try {
            ...
            if (f.exists()) {
                ...
                if (survey != null && !(survey.getPublication().isShowContent() && survey.getPublication().getShowUploadedDocuments())) {
                    ...
                    if (!survey.getOwner().getId().equals(user.getId()) && user.getGlobalPrivileges().get(GlobalPrivilege.FormManagement) < 2 && user.getLocalPrivileges().get(LocalPrivilege.AccessResults) < 1 && (user.getLocalPrivileges().get(LocalPrivilege.AccessDraft) < 1 || !survey.getIsDraft())) {
                        throw new ForbiddenURLException();
                    }
                }
...
public String getMimeType(java.io.File f){
    try {
...
public ModelAndView fileWithComment(String uid,HttpServletRequest request,HttpServletResponse response){
    if (uid != null && uid.length() > 0) {
        File file;
        try {
            file = fileService.get(uid);
...
public ModelAndView surveyfile(String surveyuid,String uid,HttpServletRequest request,HttpServletResponse response){
    if (uid != null && uid.length() > 0) {
        File file = null;
        ...
        try {
            ...
            if (f.exists()) {
                // check if it is an uploaded file
                Survey survey = surveyService.getSurveyForUploadedFile(file.getId());
                if (survey != null && !FileChecker.isDelphiExplanationFile(file, survey) && !(survey.getPublication().isShowContent() && survey.getPublication().getShowUploadedDocuments())) {
                    User user = sessionService.getCurrentUser(request);
                    if (user == null)
                        throw new ForbiddenURLException();
                    sessionService.upgradePrivileges(survey, user, request);
