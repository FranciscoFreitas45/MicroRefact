...
public java.io.File deleteUploadedExplanationFileAndReturnParentDirectoryIfSuccessful(String surveyUid,String answerSetUniqueCode,String questionUid,String fileName){
    final java.io.File rootFolder = getSurveyExplanationUploadsFolder(surveyUid, false);
    final String filePath = rootFolder + Constants.PATH_DELIMITER + answerSetUniqueCode + Constants.PATH_DELIMITER + questionUid + Constants.PATH_DELIMITER + fileName;
...
@Transactional
public void migrateAllSurveyFiles(Survey survey){
...
public boolean checkResult(FileResult fileResult,FileFilter filter){
    if (!filter.isSystemExports() && fileResult.getFileType() != null && "results,statistics,charts,tokens,contacts,activities".contains(fileResult.getFileType())) {
    ...
    if (!filter.isSurveyUploads() && fileResult.getFileType() != null && fileResult.getFileType().equals("uploaded file")) {
        return false;
    }
    if (!filter.isUnknownFiles() && (fileResult.getSurveyUid() == null || fileResult.getSurveyUid().length() == 0)) {
        return false;
    }
    if (filter.getSurveyUid() != null && filter.getSurveyUid().length() > 0 && !fileResult.getSurveyUid().contains(filter.getSurveyUid())) {
        return false;
    }
    if (filter.getSurveyShortname() != null && filter.getSurveyShortname().length() > 0 && !fileResult.getSurveyShortname().contains(filter.getSurveyShortname())) {
        return false;
    }
    if (filter.getFilePath() != null && filter.getFilePath().length() > 0 && !fileResult.getFilePath().contains(filter.getFilePath())) {
        return false;
    }
    if (filter.getFileName() != null && filter.getFileName().length() > 0 && !fileResult.getFileName().contains(filter.getFileName())) {
        return false;
    }
    if (filter.getFileUid() != null && filter.getFileUid().length() > 0 && (fileResult.getFileUid() == null || !fileResult.getFileUid().contains(filter.getFileUid()))) {
        return false;
    }
    ...
    if (filter.getFileExtensions() != null && !filter.getFileExtensions().isEmpty()) {
        ...
        for (String ext : filter.getFileExtensions()) {
            if (ext.equals(fileResult.getFileExtension()) || (ext.equalsIgnoreCase("OTHER") && !Arrays.asList(fileextensions).contains(fileResult.getFileExtension()))) {
                found = true;
                break;
            }
        }
    ...
    return !(filter.getFilterApplied() != null && (fileResult.getFilterApplied() == null || !filter.getFilterApplied().equalsIgnoreCase(fileResult.getFilterApplied())));
}
...
@Transactional
public void migrateSurveyFiles(Survey survey){
        ...
        if (element instanceof Image) {
            ...
            if (image.getUrl() != null && image.getUrl().length() > 0) {
                ...
                if (migrateSurveyFile(survey.getUniqueId(), fileUID)) {
                    image.setUrl(servletContext.getContextPath() + "/files/" + survey.getUniqueId() + Constants.PATH_DELIMITER + fileUID);
                }
            }
    ...
    for (String label : survey.getBackgroundDocuments().keySet()) {
        ...
        if (migrateSurveyFile(survey.getUniqueId(), fileUID)) {
            survey.getBackgroundDocuments().put(label, sessionService.getContextPath() + "/files/" + survey.getUniqueId() + Constants.PATH_DELIMITER + fileUID);
        }
    }
...
public int deleteFilesForSurveys(List<Integer> surveyIDs){
    int deletecounter = 0;
...
public int deleteFilesForArchivedSurveys(){
    ...
    logger.info("finish deleteFilesForArchivedSurveys: " + deletecounter + " files deleted " + ConversionTools.getFullString(new Date()));
    return deletecounter;
}


public void deleteUploadedExplanationFiles(String surveyUid,String answerSetUniqueCode,String questionUid){
    final java.io.File rootFolder = getSurveyExplanationUploadsFolder(surveyUid, false);
...
public int deleteFilesForDeletedElements(){
...
public void deleteFilesFromDiskAndDatabase(String surveyUid,List<File> files){
...
public boolean migrateSurveyFile(String surveyuid,String fileUID){
...
public int deleteTemporaryFiles(Date tempbefore){
    ...
    logger.info("finish deleteContributions: " + deletecounter.getValue() + " files deleted " + ConversionTools.getFullString(new Date()));
...
public int deleteContributions(Date pdfbefore){
    logger.info("starting deleteContributions: " + ConversionTools.getFullString(new Date()));
    int deletecounter = 0;
    ...
    logger.info("finish deleteContributions: " + deletecounter + " files deleted " + ConversionTools.getFullString(new Date()));
    return deletecounter;
}


public void deleteFileFromDiskAndDatabase(String surveyUid,File file){
...
public File copyFile(String uid,String surveyUID){
...
public int deleteUploadedFilesSubfolders(Date before,java.io.File folder){
    if (!folder.exists())
        return 0;
...
@Transactional
public void migrateAllUserFiles(){
...
public int deleteOldAnswerPDFs(String surveyUID,Date before){
    ...
    Files.walkFileTree(dir, new SimpleFileVisitor<Path>() {
        ...
        public FileVisitResult visitFile(Path p, BasicFileAttributes attrs) throws IOException {
            if (!p.toFile().isDirectory()) {
                java.io.File candidate = p.toFile();
                if ((candidate.getName().startsWith(Constants.ANSWER) || candidate.getName().startsWith("draft")) && candidate.getName().endsWith(".pdf")) {
                    Date modified = new Date(candidate.lastModified());
...
public int deleteOldSurveyPDFs(String surveyUID,int id){
    java.io.File folder = getSurveyExportsFolder(surveyUID);
    Path dir = folder.toPath();
    final MutableInteger deletecounter = new MutableInteger(0);
    Files.walkFileTree(dir, new SimpleFileVisitor<Path>() {
...
public int deleteUploadedFilesFolders(Date before){
...
public void deleteUploadedAnswerFiles(String surveyUid,String answerSetUniqueCode){
...
public void deleteUploadedFilesDirectory(String answerSetUniqueCode,String rootFolderPath,String suffix){
    final java.io.File directory = new java.io.File(rootFolderPath + Constants.PATH_DELIMITER + answerSetUniqueCode + suffix);
    try {
...
@Transactional
public void migrateAllArchiveFiles(){
    ...
    for (Archive archive : archives) {
        ...
        if (original.exists()) {
            java.io.File copy = new java.io.File(folder.getPath() + Constants.PATH_DELIMITER + archive.getSurveyUID() + "statistics.pdf");
            Files.copy(original.toPath(), copy.toPath(), StandardCopyOption.REPLACE_EXISTING);
        }
        original = new java.io.File(archiveFileDir + archive.getSurveyUID() + "statistics.xls");
        if (original.exists()) {
            java.io.File copy = new java.io.File(folder.getPath() + Constants.PATH_DELIMITER + archive.getSurveyUID() + "statistics.xls");
            Files.copy(original.toPath(), copy.toPath(), StandardCopyOption.REPLACE_EXISTING);
        }
        ...
        if (original.exists()) {
            java.io.File copy = new java.io.File(folder.getPath() + Constants.PATH_DELIMITER + archive.getSurveyUID() + "results.xls.zip");
            Files.copy(original.toPath(), copy.toPath(), StandardCopyOption.REPLACE_EXISTING);
        }
    }
}
...
public int deleteAll(FileFilter filter,String[] files){
    int counter = 0;
    if (files == null) {
...
public void createDummyFiles(int files){
