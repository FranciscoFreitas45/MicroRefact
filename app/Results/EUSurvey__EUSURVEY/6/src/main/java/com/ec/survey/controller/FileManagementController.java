...
@ResponseBody
public Object download(String uid,HttpServletRequest request,HttpServletResponse response){
...
@PostMapping
public ModelAndView filesPOST(HttpServletRequest request,Model model,Locale locale){
    if (enablefilemanagement == null || !enablefilemanagement.equalsIgnoreCase("true")) {
        throw new InvalidURLException();
    }
    User user = sessionService.getCurrentUser(request);
    if (user == null || user.getGlobalPrivileges().get(GlobalPrivilege.FormManagement) < 2)
...
public ModelAndView files(HttpServletRequest request,Model model,Locale locale){
    ModelAndView result = new ModelAndView("administration/filemanagement");
    addStatistics(result);
...
@ResponseBody
public Object downloaduser(String userid,String uid,HttpServletRequest request,HttpServletResponse response){
    if (enablefilemanagement == null || !enablefilemanagement.equalsIgnoreCase("true")) {
        throw new InvalidURLException();
    }
    if (uid == null || uid.length() == 0)
...
public void addStatistics(ModelAndView result){
...
@ResponseBody
public Object recreate(HttpServletRequest request,HttpServletResponse response,Locale locale){
    if (enablefilemanagement == null || !enablefilemanagement.equalsIgnoreCase("true")) {
        throw new InvalidURLException();
    }
    String path = request.getParameter("path");
