...
@Value("${ecasvalidationhost}")
 private  String ecasvalidationhost;
...
public Authentication authenticate(Authentication auth){
    ...
    if (surveyLoginMode || (auth.getName() == null || auth.getName().length() == 0 || auth.getName().startsWith("oldLogin:")) && (((String) auth.getCredentials()).startsWith("ECAS_ST") || ((String) auth.getCredentials()).startsWith("ST"))) {
        ...
        if (xmlValidationAnswer.contains("<cas:authenticationSuccess>")) {
            ...
            UsernamePasswordAuthenticationToken t = new UsernamePasswordAuthenticationToken(username, "", authorities);
            t.setDetails(user);
            return t;
        } else {
    ...
    administrationService.updateUser(user);
    return new UsernamePasswordAuthenticationToken(auth.getName(), auth.getCredentials(), getAuthorities(user, false, false));
}


public void checkUserNotBanned(User user){
    if (user.isFrozen()) {
...
public Collection<GrantedAuthority> getAuthorities(User user,boolean ecas,boolean weakAuthentication){
    ...
    if (ecas)
        authList.add(new SimpleGrantedAuthority("ROLE_ECAS_USER"));
