...
@Transactional
public String setUserDeleteRequested(int id){
    Session session = sessionFactory.getCurrentSession();
    User user = (User) session.get(User.class, id);
...
@Transactional
public void confirmUserDeleteRequest(int id,String code){
    Session session = sessionFactory.getCurrentSession();
    User user = (User) session.get(User.class, id);
    if (user == null) {
...
@Transactional(readOnly = false, propagation = Propagation.REQUIRES_NEW)
public void banUser(String userId,String mailText){
    Session session = sessionFactory.getCurrentSession();
