...
public String resetPassword(String code,ModelMap model,Locale locale){
    if (isShowEcas())
        model.put("showecas", true);
    if (isCasOss())
        model.put("casoss", true);
...
public boolean checkValid(OneTimePasswordResetCode codeItem){
...
public void surveylogin(boolean error,HttpServletRequest request,HttpServletResponse response,ModelMap model){
    if (isShowEcas())
        model.put("showecas", true);
    if (isCasOss())
        model.put("casoss", true);
...
public void ecaslogin(boolean error,HttpServletRequest request,HttpServletResponse response,ModelMap model){
    if (isShowEcas())
        model.put("showecas", true);
    if (isCasOss())
        model.put("casoss", true);
...
public String resetPost(String password,String password2,String code,ModelMap model,Locale locale){
    model.put("code", code);
    if (isShowEcas())
        model.put("showecas", true);
    if (isCasOss())
...
public String getLoginPage(boolean error,HttpServletRequest request,ModelMap model,Locale locale){
    if (isShowEcas())
        model.put("showecas", true);
    if (isCasOss())
        model.put("casoss", true);
    ...
    if (request.getSession().getAttribute("ticket") != null)
        model.put("ticket", request.getSession().getAttribute("ticket"));
...
public String getLoginPageRunnerMode(boolean error,HttpServletRequest request,ModelMap model,Locale locale){
    return "redirect:/auth/login";
}
