...
public ModelAndView roles(Locale locale,Model model){
    ...
    if (isShowEcas())
        m.addObject("showecas", true);
    if (isCasOss())
        m.addObject("casoss", true);
    return m;
}


public ModelAndView createRole(String name,Locale locale){
...
public ModelAndView deleteRole(String id,Locale locale){
    ModelAndView m = new ModelAndView("administration/roles");
    try {
