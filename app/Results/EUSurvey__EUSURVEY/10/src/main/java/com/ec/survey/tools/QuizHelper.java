...
public QuizResult getQuizResult(AnswerSet answerSet,Survey survey,Set<String> invisibleElements){
    ...
    for (Element element : survey.getElements()) {
        ...
        if (element instanceof Section) {
            ...
            } else if (// individual points
            question.getScoring() == 2) {
                if (question instanceof SingleChoiceQuestion) {
                    ChoiceQuestion choice = (ChoiceQuestion) question;
                    int max = 0;
                    for (PossibleAnswer pa : choice.getAllPossibleAnswers()) {
...
public boolean ruleMatches(ScoringItem scoringItem,Date value){
    if (scoringItem.getType() == 8)
        return value == null;
    if (value == null)
        return false;
    if (scoringItem.getType() == -1) {
    ...
    switch(scoringItem.getType()) {
        ...
        case // between
        5:
            if (scoringItem.getValue2() == null || scoringItem.getValue2().length() == 0)
                return false;
...
public ScoringItem getBestMatch(List<ScoringItem> matches){
    if (matches.size() == 1)
