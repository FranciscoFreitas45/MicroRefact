...
public String getBigLetter(int pcounter){
    // we start with 0 and not 1
    int counter = pcounter - 1;
    int prefix = counter / 26;
    counter = counter % 26;
    String sprefix = "";
    if (prefix > 0)
        sprefix = "ABCDEFGHIJKLMNOPQRSTUVWXYZ".substring(prefix - 1, prefix);
    if (counter == 25) {
        return sprefix + "Z";
    }
...
public String getSmallLetter(int pcounter){
    // 1 -> a
    // we start with 0 and not 1
    int counter = pcounter - 1;
    int prefix = counter / 26;
