...
import com.ec.survey.DTO.ECFCompetency;
import com.ec.survey.DTO.Translation;
public class SurveyCreator {

 static  int SurveyCounter;

 private  Logger logger;


public Survey createDummyDependencySurvey(User owner,Language la){
...
public Survey createDummyValidationSurvey(User owner,Language l){
    Survey survey = new Survey();
    survey.setContact(owner.getEmail());
    survey.setIsDraft(true);
    survey.setCreated(new Date());
...
public Survey createDummyPasswordSurvey(User owner,Language l){
    Survey survey = new Survey();
    survey.setContact(owner.getEmail());
    survey.setIsDraft(true);
    survey.setCreated(new Date());
...
public Survey createDummyPagingSurvey(User owner,Language l){
    Survey survey = new Survey();
    survey.setContact(owner.getEmail());
    survey.setIsDraft(true);
    survey.setCreated(new Date());
...
public Survey createNewSelfRegistrationSurvey(User owner,Language la,List<Language> langs){
    Survey survey = new Survey();
    survey.setContact(owner.getEmail());
    survey.setIsDraft(true);
    survey.setCreated(new Date());
...
public Survey createDummySurvey(User owner,Language la,boolean init,Integer questions){
    Survey survey = new Survey();
    survey.setContact(owner.getEmail());
    survey.setIsDraft(true);
    survey.setCreated(new Date());
...
public AnswerSet createDummyAnswerSet(Survey survey,User user){
...
public Survey createDummyUploadSurvey(User owner,Language la){
    Survey survey = new Survey();
    survey.setContact(owner.getEmail());
    survey.setIsDraft(true);
    survey.setCreated(new Date());
...
public void createStressTestSurveys(User analyst,ServletContext servletContext,String fileDir,SurveyService surveyService,FileService fileService){
    try {
...
public void createStressTestSurvey(String path,User analyst,ServletContext servletContext,String fileDir,SurveyService surveyService,FileService fileService){
...
public Survey createDummyNumberingSurvey(User owner,Language la){
    Survey survey = new Survey();
    survey.setContact(owner.getEmail());
...
public Survey createNewECFSurvey(User owner,Language language,Set<ECFCompetency> competencies,Set<ECFProfile> profiles,Map<Integer,Map<Integer,String>> questionNumberToAnswerToText,Map<Integer,Map<Integer,String>> competencyNumberToQuestionNumberToText,String shortname){
    logger.info("Creating the ECF survey");
    logger.info("There are " + competencies.size() + " competencies");
    competencies.stream().map(c -> {
    ...
    SingleChoiceQuestion profileScQuestion = new SingleChoiceQuestion("Profiles selection", "profileSelection", UUID.randomUUID().toString());
    ...
    for (ECFCompetency ecfCompetency : competencies) {
        ...
        for (int noqfoc = 0; noqfoc < numberOfQuestionsForOneCompetency; noqfoc++) {
            ...
            SingleChoiceQuestion singleChoiceQuestion = new SingleChoiceQuestion("<b>" + ecfCompetency.getName() + ":</b> " + competencyNumberToQuestionNumberToText.get(ecfCompetency.getOrderNumber()).get(noqfoc1), "q" + noqfoc + ".competency." + ecfCompetency.getCompetenceUid(), UUID.randomUUID().toString());
...
public Translations createDummyTranslations(Language lang,Translations translations){
...
public List<Language> createBasicLanguages(){
...
public Survey createDemoSkinSurvey(User owner,Language la){
    Survey survey = new Survey();
    survey.setContact(owner.getEmail());
    survey.setIsDraft(true);
    survey.setCreated(new Date());
