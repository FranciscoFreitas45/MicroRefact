...
public boolean differsFrom(Element element){
    ...
    if (isSingleChoice != matrix.isSingleChoice)
        return true;
    if (getChildElements().size() != matrix.getChildElements().size())
        return true;
    for (int r = 0; r < getChildElements().size(); r++) {
        if (this.getChildElements().get(r).differsFrom(matrix.getChildElements().get(r)))
            return true;
    }
    if (!getDependentElementUIDsStrings().equals(matrix.getDependentElementUIDsStrings()))
        return true;
    if (!Objects.equals(minRows, matrix.minRows))
        return true;
    if (!Objects.equals(maxRows, matrix.maxRows))
        return true;
    if (!Objects.equals(order, matrix.order))
        return true;
...
public Matrix copy(String fileDir){
