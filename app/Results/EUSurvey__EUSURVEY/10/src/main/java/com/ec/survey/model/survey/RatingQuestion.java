...
public RatingQuestion copy(String fileDir){
...
public boolean differsFrom(Element element){
    ...
    if (getChildElements().size() != rating.getChildElements().size())
        return true;
    for (int r = 0; r < getChildElements().size(); r++) {
        if (this.getChildElements().get(r).differsFrom(rating.getChildElements().get(r)))
            return true;
    }
    return false;
}
