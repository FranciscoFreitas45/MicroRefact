...
import com.ec.survey.DTO.TranslationService;
import com.ec.survey.DTO.TranslationService;
import com.ec.survey.DTO.SessionService;
...
public void addDelphiAnswerComments(ArchiveOutputStream os,Session session,AnswerExplanationService answerExplanationService,FileService fileService,Survey survey,Survey activeSurvey){
...
public void addObjectAsFileToOutputStream(Object original,String fileName,ArchiveOutputStream os,Session session,FileService fileService){
...
public Object getObject(ZipFile zipFile,ZipArchiveEntry zipEntry,FileService fileService){
...
public Section parseSection(Element sectionElement,ImportResult result,Survey survey,ServletContext servletContext){
...
public void addDelphiAnswerExplanationFilesToOutputStream(ArchiveOutputStream os,AnswerService answerService,FileService fileService,SurveyService surveyService,Survey survey,List<AnswerExplanation> explanations){
...
public String getChildElementValue(Element parent,String name){
...
public void importIPMSurveyXML(InputStream f,SurveyService surveyService,User owner,ImportResult result,ServletContext servletContext){
    try {
        ...
        if (Accessibility.getElementsByTagName("Anonymous").getLength() > 0) {
            ...
            if (AntiRobot != null && AntiRobot.getTextContent().equalsIgnoreCase("true"))
        ...
        if (Private.getTextContent().equalsIgnoreCase("false"))
...
public void importIPMTranslationXML(InputStream f,ImportResult result,ServletContext servletContext){
    try {
        DocumentBuilderFactory docFactory = DocumentBuilderFactory.newInstance();
        docFactory.setFeature("http://apache.org/xml/features/nonvalidating/load-dtd-grammar", false);
        docFactory.setFeature("http://apache.org/xml/features/nonvalidating/load-external-dtd", false);
...
public void saveFile(ZipFile zipFile,ZipArchiveEntry zipEntry,String filePath){
...
public Question parseQuestion(Element questionElement,boolean template,ImportResult result){
    Question question = null;
...
public ImportResult importSurvey(java.io.File file,FileService fileService,String email){
    ...
    try {
        ...
        while (entries.hasMoreElements()) {
            ...
            if (name.equalsIgnoreCase("survey.eus")) {
                ...
                if (!fileService.isDelphiEnabled())
                    result.getSurvey().setIsDelphi(false);
                ...
                if (email != null)
                    result.getActiveSurvey().setContact(email);
                if (!fileService.isDelphiEnabled())
                    result.getSurvey().setIsDelphi(false);
                ...
                if (email != null)
                    oldSurvey.setContact(email);
                if (!fileService.isDelphiEnabled())
                    oldSurvey.setIsDelphi(false);
...
public ImportResult importIPMSurvey(java.io.File f,SurveyService surveyService,User owner,FileService fileService,ServletContext servletContext,String email){
    ImportResult result = new ImportResult();
...
public java.io.File exportSurvey(String shortname,SurveyService surveyService,boolean answers,TranslationService translationService,AnswerService answerService,String fileDir,SessionService sessionService,FileService fileService,Session session,String host,AnswerExplanationService answerExplanationService){
    try {
            ...
            if (answers) {
                ...
                for (Integer id : allsurveys) {
                    if (!id.equals(survey.getId()) && !id.equals(activeSurvey.getId())) {
                        ...
                        addObjectAsFileToOutputStream(translations, "translations-active-" + id + ".eus", os, session, fileService);
                    }
                }
            }
        }
        ...
        if (answers) {
            ...
            if (survey.getIsDelphi()) {
                addDelphiAnswerExplanations(os, session, answerService, answerExplanationService, fileService, surveyService, survey, activeSurvey);
...
public void addSurveyData(Survey survey,ArchiveOutputStream os,Session session,FileService fileService,SessionService sessionService,String fileDir,List<String> writtenFiles){
...
public void parsePossibleAnswers(Question question,org.w3c.dom.Element questionElement,ImportResult result){
...
public java.io.File getFileForObject(Object original,Session session,FileService fileService){
...
public void addDelphiAnswerExplanations(ArchiveOutputStream os,Session session,AnswerService answerService,AnswerExplanationService answerExplanationService,FileService fileService,SurveyService surveyService,Survey survey,Survey activeSurvey){
    ...
    addDelphiAnswerExplanationFilesToOutputStream(os, answerService, fileService, surveyService, survey, explanations);
    if (activeSurvey != null) {
...
public Question parseMatrix(Element matrixElement,ImportResult result,ServletContext servletContext){
