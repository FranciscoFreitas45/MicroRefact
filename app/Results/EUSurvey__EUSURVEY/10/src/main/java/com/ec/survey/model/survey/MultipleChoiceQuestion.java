...
public boolean differsFrom(Element element){
    ...
    if (numColumns != multi.numColumns || maxChoices != multi.maxChoices || minChoices != multi.minChoices || useCheckboxes != multi.useCheckboxes) {
        return true;
    }
    ...
    for (int i = 0; i < getPossibleAnswers().size(); i++) {
        if (!getPossibleAnswers().get(i).getTitle().equals(multi.getPossibleAnswers().get(i).getTitle()))
            return true;
        if (!getPossibleAnswers().get(i).getDependentElementsUIDString().equals(multi.getPossibleAnswers().get(i).getDependentElementsUIDString()))
            return true;
        if (!Tools.isEqual(getPossibleAnswers().get(i).getShortname(), multi.getPossibleAnswers().get(i).getShortname()))
            return true;
        if (getPossibleAnswers().get(i).getScoring() != null && getPossibleAnswers().get(i).getScoring().differsFrom(multi.getPossibleAnswers().get(i).getScoring())) {
            return true;
        }
    }
    return false;
...
public MultipleChoiceQuestion copy(String fileDir){
