...
public void removeDependencies(Element element){
  ...
  UriComponentsBuilder builder = UriComponentsBuilder.fromUriString(url.concat("/removeDependencies"))
...
public int repairXHTML(Translations translations,DocumentBuilder builder){
  ...
  UriComponentsBuilder builder = UriComponentsBuilder.fromUriString(url.concat("/repairXHTML"))

.queryParam("translations",translations)
.queryParam("builder",builder)
;
...
public int importSurvey(ImportResult result,User user,boolean isRestore){
  ...
  UriComponentsBuilder builder = UriComponentsBuilder.fromUriString(url.concat("/importSurvey"))

.queryParam("result",result)
.queryParam("user",user)
.queryParam("isRestore",isRestore)
;
int aux = restTemplate.getForObject(builder.toUriString(),int.class);
return aux;
}
...
public Survey publish(Survey draftSurvey,int pnumberOfAnswerSets,int pnumberOfAnswerSetsPublished,boolean deactivateAutoPublishing,int userId,boolean resetSourceIds,boolean resetSurvey){
  ...
  UriComponentsBuilder builder = UriComponentsBuilder.fromUriString(url.concat("/publish"))

.queryParam("draftSurvey",draftSurvey)
.queryParam("pnumberOfAnswerSets",pnumberOfAnswerSets)
.queryParam("pnumberOfAnswerSetsPublished",pnumberOfAnswerSetsPublished)
.queryParam("deactivateAutoPublishing",deactivateAutoPublishing)
.queryParam("userId",userId)
.queryParam("resetSourceIds",resetSourceIds)
.queryParam("resetSurvey",resetSurvey)
;
...
public Survey activate(Survey survey,boolean deactivateAutoPublishing,int userId){
  ...
  UriComponentsBuilder builder = UriComponentsBuilder.fromUriString(url.concat("/activate"))
