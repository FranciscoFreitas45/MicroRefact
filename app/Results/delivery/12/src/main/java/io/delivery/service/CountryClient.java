...
public Map<String,String> parseCountry(String response){
    Document document;
    ...
    if (code.size() != 0)
        resultMap.put("code", code.get(0).text());
    if (name.size() != 0)
        resultMap.put("name", name.get(0).text());
    if (currency.size() != 0)
        resultMap.put("curency", currency.get(0).text());
    if (currencyCode.size() != 0)
        resultMap.put("curencycode", currencyCode.get(0).text());
    return resultMap;
}
