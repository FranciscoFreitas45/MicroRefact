...
public BasketUnit findBasketUnitById(long id){
    return (BasketUnit) sessionFactory.getCurrentSession().createQuery("from BasketUnit where id = ?").setParameter(0, id).uniqueResult();
}


@Override
public List<Order> findByUserId(long uid){
    return (List<Order>) sessionFactory.getCurrentSession().createQuery("from Order where userId = ?").setParameter(0, uid).list();
}
...
public BasketUnit deleteBasketUnitById(BasketUnit basketUnit){
    ...
    Query query = sessionFactory.getCurrentSession().createQuery("delete BasketUnit where id = ?").setParameter(0, id);
    query.executeUpdate();
    return basketUnit;
}
