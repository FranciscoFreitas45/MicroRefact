...
@Autowired
public NewsSiteController(HttpSession httpSession, UserService userService, NewsService newsService, FileWebService fileWebService) {
...
public String bigImage(UUID newsHash,Integer width,Integer height,HttpServletRequest request){
    try {
        Optional<News> newsOptional = this.newsService.getEnabledByHash(newsHash);
        if (newsOptional.isPresent()) {
            FileDTO fileDTO = this.fileWebService.getImage(newsOptional.get().getBigImage().getHash(), width, height);
            if (null != fileDTO) {
                return "redirect:" + getBaseURI(request) + Constants.FILES_PATH + fileDTO.getFileName();
            } else {
