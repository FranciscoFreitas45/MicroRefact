...
public class KeepConnectedServiceImpl implements KeepConnectedService{
 ...
 private  KeepConnectedDAO keepConnectedDAO;
...
public PersistentRememberMeToken getTokenForSeries(String series){
    ...
    if (permanecerConectado.isPresent()) {
        PersistentRememberMeToken persistentRememberMeToken = new PersistentRememberMeToken(permanecerConectado.get().getUser().getName(), permanecerConectado.get().getSeries(), permanecerConectado.get().getToken(), Date.from(permanecerConectado.get().getLastUse().toInstant(ZoneOffset.UTC)));
