...
public boolean isValid(EventEditDTO eventEditDTO,ConstraintValidatorContext constraintValidatorContext){
    boolean isValid = null != eventEditDTO.getStartDateTime() && null != eventEditDTO.getEndDateTime() && eventEditDTO.getEndDateTime().isAfter(eventEditDTO.getStartDateTime());
    if (!isValid) {
        constraintValidatorContext.disableDefaultConstraintViolation();
        constraintValidatorContext.buildConstraintViolationWithTemplate("A data de término do evento deve ser posterior à data de início").addPropertyNode("endDateTime").addConstraintViolation();
        return false;
    }
    return true;
}
