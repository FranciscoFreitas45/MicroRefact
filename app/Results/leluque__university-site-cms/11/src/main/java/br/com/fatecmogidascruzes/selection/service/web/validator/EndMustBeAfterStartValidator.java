...
public boolean isValid(SelectionEditDTO selectionEditDTO,ConstraintValidatorContext constraintValidatorContext){
    boolean isValid = null != selectionEditDTO.getStartDateTime() && null != selectionEditDTO.getEndDateTime() && selectionEditDTO.getEndDateTime().isAfter(selectionEditDTO.getStartDateTime());
    if (!isValid) {
        constraintValidatorContext.disableDefaultConstraintViolation();
        constraintValidatorContext.buildConstraintViolationWithTemplate(constraintValidatorContext.getDefaultConstraintMessageTemplate()).addPropertyNode("endDateTime").addConstraintViolation();
        return false;
    }
    return true;
}
