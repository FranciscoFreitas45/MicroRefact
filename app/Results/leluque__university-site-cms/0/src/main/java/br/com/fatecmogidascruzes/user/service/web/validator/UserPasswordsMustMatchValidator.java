...
public boolean isValid(UserEditDTO userDTO,ConstraintValidatorContext constraintValidatorContext){
    boolean isValid = (null == userDTO.getPassword() && null == userDTO.getRepeatPassword()) || (null != userDTO.getPassword() && null != userDTO.getRepeatPassword() && userDTO.getPassword().equals(userDTO.getRepeatPassword()));
    if (!isValid) {
        constraintValidatorContext.disableDefaultConstraintViolation();
        constraintValidatorContext.buildConstraintViolationWithTemplate("As senhas informadas devem ser iguais").addPropertyNode("repeatPassword").addConstraintViolation();
        return false;
    }
    return true;
}
