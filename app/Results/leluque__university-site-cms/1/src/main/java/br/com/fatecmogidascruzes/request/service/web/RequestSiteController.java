...
@Autowired
public RequestSiteController(HttpSession httpSession, UserService userService, RequestService requestService, RequestWebService requestWebService, EmailService emailService, ReCaptchaService reCaptchaService) {
...
public ModelAndView view(String recaptchaResponse,RequestSituationDTO requestSituationDTO,RedirectAttributes redirectAttributes,HttpServletRequest request){
    ...
    try {
        ...
        RequestEditDTO requestEditDTO = this.requestWebService.getEnabledByProtocol(requestSituationDTO.getProtocol());
...
public ModelAndView save(String recaptchaResponse,RequestEditDTO requestEditDTO,BindingResult binding,RedirectAttributes redirectAttributes,HttpServletRequest request){
    ...
    try {
        ...
        emailService.sendRequestEMail(requestEditDTO.getEmail(), requestEditDTO.getName(), String.valueOf(savedRequest.getProtocol()), savedRequest.getRequestSituation().getName(), savedRequest.getDocumentType().getName());
...
public ModelAndView newRequest(RequestEditDTO requestEditDTO,RequestSituationDTO requestSituationDTO,RedirectAttributes redirectAttributes){
    ModelAndView modelAndView = new ModelAndView("site/solicitarDocumentos");
