...
public void save(NewsEditDTO newsEditDTO) throws BadRequestException{
    ...
    if (null != newsEditDTO.getHashString() && !newsEditDTO.getHashString().trim().isEmpty()) {
        Optional<News> newsOptional = this.newsService.getEnabledByHash(FriendlyId.toUuid(newsEditDTO.getHashString()));
        if (!newsOptional.isPresent() || !newsOptional.get().isEnabled()) {
            throw new BadRequestException("The specified news does not exist or is disabled and then cannot be updated");
        }
    ...
    if (null != newsEditDTO.getImage()) {
        if (!newsEditDTO.getImage().isEmpty()) {
            ...
            try {
                File newImage = fileService.saveImage(newsEditDTO.getImage(), newsEditDTO.getImageAlternativeDescription());
    ...
    if (null != newsEditDTO.getBigImage()) {
        if (!newsEditDTO.getBigImage().isEmpty()) {
            ...
            try {
                File newImage = fileService.saveImage(newsEditDTO.getBigImage(), newsEditDTO.getBigImageAlternativeDescription());
    ...
    if (null != newsEditDTO.getHighlightImage()) {
        if (!newsEditDTO.getHighlightImage().isEmpty()) {
            ...
            try {
                File newImage = fileService.saveImage(newsEditDTO.getHighlightImage(), newsEditDTO.getHighlightImageAlternativeDescription());
        ...
        if (null != news.getHighlightImage()) {
            news.getHighlightImage().setAlternativeDescription(newsEditDTO.getHighlightImageAlternativeDescription());
        }
    }
