...
@Override
public void save(EmployeeEditDTO employeeEditDTO) throws BadRequestException{
    Employee employee = new Employee();
    if (null != employeeEditDTO.getHashString() && !employeeEditDTO.getHashString().trim().isEmpty()) {
        Optional<Employee> categoryOptional = this.employeeService.getEnabledByHash(FriendlyId.toUuid(employeeEditDTO.getHashString()));
        if (!categoryOptional.isPresent() || !categoryOptional.get().isEnabled()) {
            throw new BadRequestException("The specified employee does not exist or is disabled and then cannot be updated");
        }
    ...
    if (null != employeeEditDTO.getImage()) {
        if (!employeeEditDTO.getImage().isEmpty()) {
            ...
            try {
                File newImage = fileService.saveImage(employeeEditDTO.getImage(), employeeEditDTO.getImageAlternativeDescription());
