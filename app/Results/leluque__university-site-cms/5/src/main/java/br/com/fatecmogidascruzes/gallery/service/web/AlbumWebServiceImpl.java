...
@Override
public void upload(AlbumPhotosEditDTO albumPhotosEditDTO) throws BadRequestException{
    Album album;
    if (null != albumPhotosEditDTO.getHashString() && !albumPhotosEditDTO.getHashString().trim().isEmpty()) {
        Optional<Album> albumOptional = this.albumService.getEnabledByHash(FriendlyId.toUuid(albumPhotosEditDTO.getHashString()));
        if (!albumOptional.isPresent() || !albumOptional.get().isEnabled()) {
            throw new BadRequestException("The specified album does not exist or is disabled and then cannot be updated");
        }
...
public void save(AlbumEditDTO albumEditDTO) throws BadRequestException{
    ...
    if (null != albumEditDTO.getHashString() && !albumEditDTO.getHashString().trim().isEmpty()) {
        Optional<Album> albumOptional = this.albumService.getEnabledByHash(FriendlyId.toUuid(albumEditDTO.getHashString()));
        if (!albumOptional.isPresent() || !albumOptional.get().isEnabled()) {
            throw new BadRequestException("The specified album does not exist or is disabled and then cannot be updated");
        }
        album = albumOptional.get();
    ...
    if (null != albumEditDTO.getCover()) {
        if (!albumEditDTO.getCover().isEmpty()) {
            ...
            try {
                File newCover = fileService.saveImage(albumEditDTO.getCover(), albumEditDTO.getCoverAlternativeDescription());
