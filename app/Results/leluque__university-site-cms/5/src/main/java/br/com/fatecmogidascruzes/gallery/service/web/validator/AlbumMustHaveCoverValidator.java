...
public boolean isValid(AlbumEditDTO albumEditDTO,ConstraintValidatorContext context){
    if ((albumEditDTO.getHashString() == null || albumEditDTO.getHashString().trim().isEmpty()) && (null == albumEditDTO.getCover() || albumEditDTO.getCover().isEmpty())) {
        context.disableDefaultConstraintViolation();
        context.buildConstraintViolationWithTemplate(context.getDefaultConstraintMessageTemplate()).addPropertyNode("cover").addConstraintViolation();
        return false;
    }
    return true;
}
