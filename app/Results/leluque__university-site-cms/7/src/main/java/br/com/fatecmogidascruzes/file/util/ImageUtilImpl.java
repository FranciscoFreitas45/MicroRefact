...
public Set<ImageVariation> generateVariations(File file) throws IOException{
    ...
    for (Double resolution : RESOLUTIONS) {
        if (biggest > resolution) {
            ...
            Thumbnails.of(storage.loadFile(file.getName())).scale(proportion).toFile(new java.io.File(storage.getPath().toAbsolutePath().toString() + getDefault().getSeparator() + variationName));
