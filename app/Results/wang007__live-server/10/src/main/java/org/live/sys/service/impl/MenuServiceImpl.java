...
public void deleteMenuById(String menuId){
    // 统计该菜单下的子菜单
    final long childLength = this.menuRepo.countChildMenuByParentId(menuId);
    if (childLength > 0) {
        throw new ServiceException("删除失败，请先删除该菜单下的子菜单");
    }
    // 统计该菜单下的子菜单
    long elementLength = this.elementRepo.countPageElementByMenuId(menuId);
    if (elementLength > 0) {
    ...
    if (parent != null) {
        // 设置父菜单的菜单类型
        // 父菜单的子菜单数量
        long parentChildLength = this.menuRepo.countChildMenuByParentId(parent.getId());
...
public List<ZtreeNode> getZtreeNodesByRoles(String roleId){
    ...
    // 该角色所拥有的菜单id
    List<String> menuIds = this.menuRepo.findMenuIdByRoleId(roleId);
...
public void findChildNodesByParentId(String parentId,MenuNode rootMenuNode,MenuNode dumpMenuNode){
    if (StringUtils.equals(rootMenuNode.getId(), parentId)) {
        // 当前id = parentid，就把当前菜单的子菜单保存到临时list集合中
...
public Menu getAlreadyAssembleMenuTree(){
    // 查询全部菜单.
    List<Menu> allMenu = this.menuRepo.findAllMenu();
    // 执行菜单拼装
    List<Menu> alreadyAssembleMenus = (List<Menu>) CombinableUtils.executeCombination(null, allMenu);
    // 根目录
    return alreadyAssembleMenus != null && alreadyAssembleMenus.size() > 0 ? alreadyAssembleMenus.get(0) : new Menu();
}
...
public void saveMenuInfo(MenuVo menuVo){
    ...
    if (saveType == 2) {
        // 保存为子级菜单
        if (!SysConstants.MENU_NODE_TYPE_FOLDER.equals(referenceMenu.getMenuType())) {
            // 父菜单的类型不是folder
...
public void updateMenuInfo(MenuVo menuVo){
    ...
    // 修改菜单信息
    CopyPropertiesUtils.copyPropertiesIgnoreNull(menuPersistent, menuVo);
    Permission permission = menuPersistent.getPermission();
    if (permission != null) {
...
public MenuNode getAlreadyAssembleMenuNodeTree(){
    ...
    // 执行菜单节点的拼装.
    List<MenuNode> alreadyAssembleMenuNodes = (List<MenuNode>) CombinableUtils.executeCombination(null, allMenuNode);
...
public List<TreeViewNode> loadTreeViewNodesByParentId(String parentId){
    ...
    // 查询该父节点下的所有子节点（包括子孙节点）
    this.findChildNodesByParentId(parentId, rootMenNode, dumpMenuNode);
...
public void saveMenuRoleRel(String roleId,String[] menuIds){
    if (menuIds != null) {
        ...
        // 客户端发过来的menuIds，需要被处理的
        List<String> needsHandleMenuIds = Arrays.asList(menuIds);
        // 该角色中已经存在的与菜单的关联关系
        List<String> alreadyExistMenuIds = this.menuRepo.findMenuIdByRoleId(roleId);
        // needDeleteRoleMenuRel是暂存需要删除的MenuRoleRel
        List<String> needDeleteMenuIds = new ArrayList<String>();
        // 已经存在数据库的菜单id，在needsHandleMenuIds也有，即，这集合中的数据，是不需要处理的
        List<String> alreadyHaveMenuIds = new ArrayList<String>();
        if (alreadyExistMenuIds != null) {
            for (String alreadyExistMenuId : alreadyExistMenuIds) {
                if (!needsHandleMenuIds.contains(alreadyExistMenuId)) {
                    // 数据库中存在，客户端不存在，那么删除
        ...
        for (String needDeleteMenuId : needDeleteMenuIds) {
            // 删除关联关系
            rolePermissionRepo.deleteRolePermissionByIds(roleId, needDeleteMenuId);
        }
        for (String needHandleMenuId : needsHandleMenuIds) {
            // 添加关联关系
            if (!alreadyHaveMenuIds.contains(needHandleMenuId)) {
                // 需要处理的
