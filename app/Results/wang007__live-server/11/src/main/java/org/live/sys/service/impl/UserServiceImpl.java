...
import org.live.DTO.PermissionRepository;
import org.live.DTO.RoleRepository;
@Service("userServie")
...
public void saveUserInfo(UserVo userVo,String password){
    ...
    if (userVo.getGroupId() != null && !"".equals(userVo.getGroupId())) {
        // 设置用户组
    ...
    // 盐
    String salt = UUIDGenerator.createSalt();
    // 
...
public void updateUserInfo(UserVo userVo){
    if (userVo != null) {
        ...
        if (group != null) {
            // 查询用户是否有所属用户组
            if (!StringUtils.equals(group.getId(), userVo.getGroupId())) {
                // 所属用户组是否跟修改后的一样
