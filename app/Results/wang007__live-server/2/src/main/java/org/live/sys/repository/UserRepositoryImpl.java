...
public Page<UserVo> findUsers(Pageable pageable,UserVo userVo,boolean isDelete){
     ...
     */
    final String xsql = "select u.id as id, u.username as username, u.user_type as userType, " + "u.name as name, u.register_time as registerTime, u.is_lock as isLock, u.last_login_time as lastLoginTime, " + "u.last_login_ip as lastLoginIp, g.group_name as userGroupName from sys_user u left join sys_group g on u.group_id=g.id where u.is_delete=?0" + "/~ and u.username like '%[username]%' ~/" + "/~ and u.name like '%[name]%' ~/" + "/~ and u.group_id='[groupId]' ~/" + " order by u.register_time ";
