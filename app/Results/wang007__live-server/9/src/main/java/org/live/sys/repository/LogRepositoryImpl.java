...
public Page<Log> findLogInfoAll(Pageable pageable,LogVo logVo){
    final String xsql = "select l from Log l " + "where 1=1 " + "/~ and l.ip like '%[ip]%' ~/" + "/~ and l.name like '%[name]%' ~/" + "/~ and l.username like '%[username]%'~/" + "/~ and l.handleTime >= '[beginTime]'~/" + "/~ and l.handleTime <= '[endTime]'~/" + "/~ and l.description like '%[description]%'~/" + "/~ and l.logLevel like '%[logLevel]%'~/" + "/~ and l.operateType like '%[operateType]%'~/" + "order by l.handleTime desc";
