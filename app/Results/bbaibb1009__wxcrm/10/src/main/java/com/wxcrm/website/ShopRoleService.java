...
public List<WcShopRole> queryShopRoleForAdminUpd0(Integer adminId){
    String sql = " select a.WSR_ROLE_ID, a.WSR_ROLE_NAME " + " from WC_SHOP_ROLE a " + " where not exists ( " + " select 1 from WC_SHOP_ADMIN_ROLE b " + " where a.WSR_ROLE_ID = b.WSAR_ROLE_ID and b.WSAR_ADMIN_ID = ? ) " + " order by a.WSR_ROLE_ID desc ";
    List<Map<String, Object>> list = jdbcDao.queryForList(sql, new Object[] { adminId });
    return chgShopRoleList(list);
}


public List<WcShopRole> chgShopRoleList(List<Map<String,Object>> list){
...
public void addShopRoleOther(WcShopRole role){
...
public Page queryShopRole(WcShopRole role){
    ...
    StringBuilder sql = new StringBuilder(" select WSR_ROLE_ID, WSR_ROLE_NAME, WSR_ROLE_DESC " + " from WC_SHOP_ROLE " + " where 1 = 1 ");
    StringBuilder sqlCnt = new StringBuilder(" select count(*) from WC_SHOP_ROLE where 1 = 1 ");
...
public List<WcShopRole> queryShopRoleForAdminUpd1(Integer adminId){
    String sql = " select b.WSR_ROLE_ID, b.WSR_ROLE_NAME " + " from WC_SHOP_ADMIN_ROLE a " + " join WC_SHOP_ROLE b on a.WSAR_ROLE_ID = b.WSR_ROLE_ID " + " where a.WSAR_ADMIN_ID = ? " + " order by b.WSR_ROLE_ID desc ";
    List<Map<String, Object>> list = jdbcDao.queryForList(sql, new Object[] { adminId });
    return chgShopRoleList(list);
}
