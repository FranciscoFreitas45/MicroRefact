...
public List<String> chgList(List<Map<String,Object>> list,String colName){
...
public Page queryRole(WcRole role){
    ...
    StringBuilder sql = new StringBuilder(" select WRO_ROLE_ID, WRO_ROLE_NAME, WRO_ROLE_DESC " + " from WC_ROLE " + " where 1 = 1 ");
    StringBuilder sqlCnt = new StringBuilder(" select count(*) from WC_ROLE where 1 = 1 ");
...
public void addRoleOther(WcRole role){
...
public List<WcRole> chgRoleList(List<Map<String,Object>> list){
...
public List<WcRole> queryRoleForAdminUpd1(Integer adminId){
    String sql = " select b.WRO_ROLE_ID, b.WRO_ROLE_NAME " + " from WC_ADMIN_ROLE a " + " join WC_ROLE b on a.WAR_ROLE_ID = b.WRO_ROLE_ID " + " where a.WAR_ADMIN_ID = ? " + " order by b.WRO_ROLE_ID desc ";
...
public List<String> queryRoleMenusById(Integer roleId){
    List<Map<String, Object>> menuList = jdbcDao.queryForList("select WRM_ROLE_ID, WRM_MENU_ID from WC_ROLE_MENU where WRM_ROLE_ID = ?", new Object[] { roleId });
...
public List<WcRole> queryRoleForAdminUpd0(Integer adminId){
    String sql = " select a.WRO_ROLE_ID, a.WRO_ROLE_NAME " + " from WC_ROLE a " + " where not exists ( " + " select 1 from WC_ADMIN_ROLE b " + " where a.WRO_ROLE_ID = b.WAR_ROLE_ID and b.WAR_ADMIN_ID = ? ) " + " order by a.WRO_ROLE_ID desc ";
    List<Map<String, Object>> list = jdbcDao.queryForList(sql, new Object[] { adminId });
    return chgRoleList(list);
}
