...
public List<Map<String,Object>> queryMenuToCache(){
    String sql = " select " + " WME_ID id,WME_NAME name,WME_LEVEL menuLevel,WME_PARENT_ID pId, " + " case when WME_LEVEL = '1' then 'true' " + "      when WME_LEVEL = '2' then 'true' " + "      else 'false' end isParent,        " + " case when WME_LEVEL = '1' then 'true' " + "      else 'false' end 'open'           " + " from WC_MENU " + " order by WME_LEVEL asc, WME_ORDER asc ";
...
public List<Map<String,Object>> queryShopMenuToCache(){
    String sql = " select " + " WSM_ID id,WSM_NAME name,WSM_LEVEL menuLevel,WSM_PARENT_ID pId, " + " case when WSM_LEVEL = '1' then 'true' " + "      when WSM_LEVEL = '2' then 'true' " + "      else 'false' end isParent,        " + " case when WSM_LEVEL = '1' then 'true' " + "      else 'false' end 'open'           " + " from WC_SHOP_MENU " + " order by WSM_LEVEL asc, WSM_ORDER asc ";
    return jdbcDao.queryForList(sql);
}
...
public Page queryMenu(WcMenu menu){
    ...
    StringBuilder sql = new StringBuilder(" select a.WME_ID,a.WME_NAME,a.WME_URL,a.WME_LEVEL,a.WME_ORDER,a.WME_DESC,a.WME_PARENT_ID,a.WME_REGISTOR,a.WME_REGIST_DATE, b.WME_NAME parent_menu " + " from WC_MENU a " + " left join WC_MENU b on a.WME_PARENT_ID = b.WME_ID " + " where 1 = 1 ");
    StringBuilder sqlCnt = new StringBuilder(" select count(*) from WC_MENU a where 1 = 1 ");
