...
public JRDataSource getDetailReport(Integer applyId){
    ...
    if (!apply.getComputeItems().isEmpty()) {
        ...
        for (ComputeItem computeItem : apply.getComputeItems()) {
            if (ComputeConstant.ECSServerType.Small_CPUx1_Memoryx1GB_DISKx20GB.toInteger().equals(computeItem.getServerType())) {
                ...
                middle++;
            } else if (ComputeConstant.ECSServerType.Large_CPUx4_Memoryx4GB_DISKx20GB.toInteger().equals(computeItem.getServerType())) {
                large++;
            }
        }
        ...
        if (small != 0) {
            ...
            detailReport.setPrice(BigDecimal.valueOf(comm.costService.computeCost(ComputeConstant.ECSServerType.Small_CPUx1_Memoryx1GB_DISKx20GB, workTime, small)));
        ...
        if (middle != 0) {
            ...
            detailReport.setPrice(BigDecimal.valueOf(comm.costService.computeCost(ComputeConstant.ECSServerType.Middle_CPUx2_Memoryx2GB_DISKx20GB, workTime, middle)));
        ...
        if (large != 0) {
            ...
            detailReport.setPrice(BigDecimal.valueOf(comm.costService.computeCost(ComputeConstant.ECSServerType.Large_CPUx4_Memoryx4GB_DISKx20GB, workTime, large)));
    ...
    if (!apply.getStorageItems().isEmpty()) {
        ...
        if (netapp != 0) {
            ...
            detailReport.setPrice(BigDecimal.valueOf(comm.costService.es3Cost(StorageConstant.StorageType.Netapp_高IOPS, workTime, netapp)));
        ...
        if (fimas != 0) {
            ...
            detailReport.setPrice(BigDecimal.valueOf(comm.costService.es3Cost(StorageConstant.StorageType.Fimas_高吞吐量, workTime, fimas)));
            detailReport.setNumber(1);
            reports.add(detailReport);
        }
    }
    ...
    if (!apply.getNetworkEipItems().isEmpty()) {
        ...
        if (unicom != 0) {
            ...
            detailReport.setPrice(BigDecimal.valueOf(comm.costService.eipCost(NetworkConstant.ISPType.中国联通, workTime, unicom)));
        ...
        if (telecom != 0) {
            ...
            detailReport.setPrice(BigDecimal.valueOf(comm.costService.eipCost(NetworkConstant.ISPType.中国电信, workTime, telecom)));
