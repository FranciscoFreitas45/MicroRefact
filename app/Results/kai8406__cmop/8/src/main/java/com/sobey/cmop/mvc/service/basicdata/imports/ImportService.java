...
public HostServer saveHostServer(List<HostServer> hostServerList,ServerBean serverInfo,int serverType){
    HostServer hostServer = comm.hostServerService.findByDisplayName(serverInfo.getDisplayName());
    if (hostServer == null) {
        hostServer = new HostServer(serverType, IpPoolConstant.PoolType.私网IP池.toInteger(), serverInfo.getDisplayName(), // IP池默认为私网IP池
        new Date());
...
public List<String> getServerAlias(Map map){
        ...
        for (ServerBean serverInfo : list1) {
            ...
            if (!serverInfo.getInnerIp().equals("")) {
                ...
                if (comm.ipPoolService.findIpPoolByIpAddress(serverInfo.getInnerIp()) != null) {
                    ...
                    comm.ipPoolService.saveIpPool(serverInfo.getInnerIp(), IpPoolConstant.PoolType.私网IP池.toInteger(), IpPoolConstant.IpStatus.已使用.toInteger(), comm.vlanService.getVlan(1), // 默认西安虚拟机内网VLAN
                    hostServer);
                }
            }
        }
...
@Transactional(readOnly = false)
public void updateHostServer(List<HostServer> hostServerList,Map map){
