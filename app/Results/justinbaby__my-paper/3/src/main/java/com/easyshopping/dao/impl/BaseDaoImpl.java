...
import com.easyshopping.Filter.Operator;
import com.easyshopping.Order;
import com.easyshopping.Order.Direction;
import com.easyshopping.Page;
import com.easyshopping.Pageable;
import com.easyshopping.dao.BaseDao;
import com.easyshopping.entity.OrderEntity;
import org.apache.commons.lang.StringUtils;
import org.springframework.util.Assert;
import com.easyshopping.DTO.Order;
public class BaseDaoImpl implements BaseDao<T, ID>{
...
public void addRestrictions(CriteriaQuery<T> criteriaQuery,Pageable pageable){
...
public Long count(CriteriaQuery<T> criteriaQuery,List<Filter> filters){
...
public Page<T> findPage(CriteriaQuery<T> criteriaQuery,Pageable pageable){
    Assert.notNull(criteriaQuery);
    Assert.notNull(criteriaQuery.getSelection());
    Assert.notEmpty(criteriaQuery.getRoots());
    if (pageable == null) {
...
public void addOrders(CriteriaQuery<T> criteriaQuery,Pageable pageable){
    if (criteriaQuery == null || pageable == null) {
        return;
    }
    Root<T> root = getRoot(criteriaQuery);
...
public void copyFetches(Fetch<?,?> from,Fetch<?,?> to){
    for (Fetch<?, ?> fetch : from.getFetches()) {
        Fetch<?, ?> toFetch = to.fetch(fetch.getAttribute().getName());
        copyFetches(fetch, toFetch);
    }
...
public List<T> findList(CriteriaQuery<T> criteriaQuery,Integer first,Integer count,List<Filter> filters,List<Order> orders){
    Assert.notNull(criteriaQuery);
    Assert.notNull(criteriaQuery.getSelection());
    Assert.notEmpty(criteriaQuery.getRoots());
    CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
...
public Root<T> getRoot(CriteriaQuery<?> criteriaQuery,Class<T> clazz){
