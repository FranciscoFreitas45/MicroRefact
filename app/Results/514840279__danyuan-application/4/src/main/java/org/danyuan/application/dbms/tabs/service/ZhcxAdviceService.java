...
import org.danyuan.application.DTO.SysDbmsAdviMessInfo;
@Service
...
public void startConfixOracleTableCloumnIndexConfig(SysDbmsTabsInfo sysZhcxTab,Map<String,DataSource> multiDatasource,SysDbmsAdviMessInfoDao sysAdviceMessDao,JdbcTemplate jdbcTemplate2,SysDbmsTabsColsInfo sysZhcxCol){
...
public void startConfixMysqlTableCloumnIndexConfig(SysDbmsTabsInfo sysZhcxTab,SysDbmsTabsColsInfo sysZhcxCol,List<Map<String,Object>> resultlist,SysDbmsAdviMessInfoDao sysDbmsAdviMessInfoDao){
...
public void startConfixOracleTableColumnsConfig(SysDbmsTabsInfo sysZhcxTab,Map<String,DataSource> multiDatasource,SysDbmsAdviMessInfoDao sysAdviceMessDao,JdbcTemplate jdbcTemplate2,List<SysDbmsTabsColsInfo> list){
...
public void startConfixMysqlTableConfig(SysDbmsTabsInfo sysZhcxTab,Map<String,DataSource> multiDatasource,SysDbmsAdviMessInfoDao sysDbmsAdviMessInfoDao,JdbcTemplate jdbcTemplate2){
...
public void startConfixMysqlTableColumnsConfig(SysDbmsTabsInfo sysZhcxTab,Map<String,DataSource> multiDatasource,SysDbmsAdviMessInfoDao sysDbmsAdviMessInfoDao,JdbcTemplate jdbcTemplate2,List<SysDbmsTabsColsInfo> colList){
    // 列配置比较建议修正,平台隐藏，实际长度修改(列修改，列配置修改,列统计信息)
    // 列信息处理会有多个同时执行，遮里配置和列的注释都有可能为空，需要对比
    // 统计信息包含陪的实际最大长度 给出建议缩小列的长度类型， 列空值比例，当空超过60% 给出建议平台默认不展示
    // 实际长度暂不处理
...
public void startConfixOracleTableConfig(SysDbmsTabsInfo sysZhcxTab,Map<String,DataSource> multiDatasource,SysDbmsAdviMessInfoDao sysAdviceMessDao,JdbcTemplate jdbcTemplate2) throws SQLException{
    logger.info("startConfixTableConfig", ZhcxAdviceService.class);
    // 表配置比较建议修正 (表修改，表配置修改)
    // 表修改需要人工确认，所以当前不会生成表修改的类型
    // 表配置修改包括： 修改注释翻译对照的mess和 更新配置表中的数据两块大小
