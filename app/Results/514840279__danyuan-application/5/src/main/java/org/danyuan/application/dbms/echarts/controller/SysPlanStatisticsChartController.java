...
import org.danyuan.application.DTO.SysDbmsTabsInfo;
@RestController
...
@Autowired
 private SysPlantBarOrLineStatisticsChartService sysPlantBarOrLineStatisticsChartService;

@Autowired
 private SysPlantPieStatisticsChartService sysPlantPieStatisticsChartService;

@Autowired
 private SysPlantMapStatisticsChartService sysPlantMapStatisticsChartService;
...
public void buildWhereSql(List<SysDbmsChartDimensionData> list,List<SysDbmsTabsColsInfo> confs,StringBuilder sbWhere){
    if (list.size() == 0) {
        return;
    }
    // 参数分组
