...
import DTO.TemplateUtil;
import DTO.UploadFile;
import DTO.OfficeHtmlUtil;
@Controller
...
public AjaxJson save(CgformFtlEntity cgformFtl,HttpServletRequest request){
    ...
    if (StringUtil.isNotEmpty(cgformFtl.getId())) {
        message = "更新成功";
        CgformFtlEntity t = cgformFtlService.get(CgformFtlEntity.class, cgformFtl.getId());
        try {
...
public AjaxJson saveCkeditor(HttpServletRequest request,CgformFtlEntity cgformFtlEntity,String contents){
    ...
    if (StringUtil.isNotEmpty(cgformFtlEntity.getId())) {
        CgformFtlEntity t = systemService.get(CgformFtlEntity.class, cgformFtlEntity.getId());
        try {
...
public AjaxJson saveEditor(CgformFtlEntity cgformFtl,HttpServletRequest request){
    ...
    if (StringUtil.isNotEmpty(fileKey)) {
        ...
        if (cgformFtl.getFtlContent() != null && cgformFtl.getFtlContent().indexOf("<form") < 0) {
            ...
            String ls_form = "<form action=\"cgFormBuildController.do?saveOrUpdate\" id=\"formobj\" name=\"formobj\" method=\"post\">" + "<input type=\"hidden\" name=\"tableName\" value=\"${tableName?if_exists?html}\" />" + "<input type=\"hidden\" name=\"id\" value=\"${id?if_exists?html}\" />" + "<input type=\"hidden\" id=\"btn_sub\" class=\"btn_sub\" />#{jform_hidden_field}<table";
...
public AjaxJson saveWordFiles(HttpServletRequest request,HttpServletResponse response,CgformFtlEntity cgformFtl){
    ...
    // 文件ID
    String fileKey = oConvertUtils.getString(request.getParameter("id"));
    String cgformId = oConvertUtils.getString(request.getParameter(// formid
    "cgformId"));
    String cgformName = oConvertUtils.getString(request.getParameter(// formname
    "cgformName"));
    String ftlStatus = oConvertUtils.getString(request.getParameter(// formStatus
    "ftlStatus"));
    if (oConvertUtils.isEmpty(ftlStatus)) {
        ftlStatus = "0";
    }
    ...
    try {
        ...
        String realPath = uploadFile.getMultipartRequest().getSession().getServletContext().getRealPath(File.separator) + // 文件的硬盘真实路径
        path;
        ...
        for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
            ...
            if (uploadFile.isRename()) {
                noextfilename = DateUtils.getDataString(DateUtils.yyyymmddhhmmss) + // 自定义文件名称
                StringUtil.random(8);
                // 自定义文件名称
...
public void datagrid(CgformFtlEntity cgformFtl,HttpServletRequest request,HttpServletResponse response,DataGrid dataGrid){
    ...
    // 查询条件组装器
    org.jeecgframework.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, cgformFtl, request.getParameterMap());
