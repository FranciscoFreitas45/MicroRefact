...
public ResponseEntity<?> update(JformOrderMainPage jformOrderMainPage){
...
public ResponseEntity<?> create(JformOrderMainPage jformOrderMainPage,UriComponentsBuilder uriBuilder){
    // 调用JSR303 Bean Validator进行校验，如果出错返回含400错误码及json格式的错误信息.
    Set<ConstraintViolation<JformOrderMainPage>> failures = validator.validate(jformOrderMainPage);
    if (!failures.isEmpty()) {
        return new ResponseEntity(BeanValidators.extractPropertyAndMessage(failures), HttpStatus.BAD_REQUEST);
...
public AjaxJson doBatchDel(String ids,HttpServletRequest request){
    ...
    try {
        for (String id : ids.split(",")) {
            JformOrderMainEntity jformOrderMain = systemService.getEntity(JformOrderMainEntity.class, id);
            jformOrderMainService.delMain(jformOrderMain);
...
public String exportXlsByT(ModelMap map){
    map.put(NormalExcelConstants.FILE_NAME, "订单主信息");
    map.put(NormalExcelConstants.CLASS, JformOrderMainPage.class);
    map.put(NormalExcelConstants.PARAMS, new ExportParams("订单主信息列表", "导出人:" + ResourceUtil.getSessionUser().getRealName(), "导出信息"));
...
public void upwardQueryParents(Map<String,Map<String,Object>> dataMap,List<Map<String,Object>> dateList,String pid){
