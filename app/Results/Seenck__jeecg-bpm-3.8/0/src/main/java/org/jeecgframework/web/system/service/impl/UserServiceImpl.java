...
@Service("userService")
public class UserServiceImpl extends CommonServiceImplimplements UserService{
...
public String trueDel(TSUser user){
    ...
    if (!user.getStatus().equals(Globals.User_ADMIN)) {
        if (roleUser.size() > 0) {
            ...
            delRoleUser(user);
            // 删除 用户-机构 数据
            this.commonDao.executeSql("delete from t_s_user_org where user_id=?", user.getId());
...
public void saveUserOrgList(TSUser user,String[] orgIds){
...
public void saveRoleUser(TSUser user,String[] roleIds){
