...
public String exportXls(JfromOrderEntity jfromOrder,HttpServletRequest request,HttpServletResponse response,DataGrid dataGrid,ModelMap map){
    ...
    map.put(NormalExcelConstants.PARAMS, new ExportParams("订单列表列表", "导出人:Jeecg", "导出信息"));
...
public AjaxJson doBatchDel(String ids,HttpServletRequest request){
    ...
    try {
        for (String id : ids.split(",")) {
            JfromOrderEntity jfromOrder = systemService.getEntity(JfromOrderEntity.class, id);
...
@ResponseBody
public ResponseMessage<?> update(JfromOrderPage jfromOrderPage){
...
public String exportXlsByT(ModelMap map){
    map.put(NormalExcelConstants.FILE_NAME, "订单列表");
    map.put(NormalExcelConstants.CLASS, JfromOrderPage.class);
    map.put(NormalExcelConstants.PARAMS, new ExportParams("订单列表列表", "导出人:" + ResourceUtil.getSessionUser().getRealName(), "导出信息"));
...
@ResponseBody
public ResponseMessage<?> create(JfromOrderPage jfromOrderPage,UriComponentsBuilder uriBuilder){
    // 调用JSR303 Bean Validator进行校验，如果出错返回含400错误码及json格式的错误信息.
    Set<ConstraintViolation<JfromOrderPage>> failures = validator.validate(jfromOrderPage);
    if (!failures.isEmpty()) {
        return Result.error(JSONArray.toJSONString(BeanValidators.extractPropertyAndMessage(failures)));
