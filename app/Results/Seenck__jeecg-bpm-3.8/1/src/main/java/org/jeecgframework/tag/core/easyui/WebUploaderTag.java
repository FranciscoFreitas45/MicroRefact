...
public void end(StringBuffer sb){
    ...
    // 当文件被加入队列以后触发。
    sb.append(// '+file.name+'---等待上传---</span>
    "uploader.on( 'fileQueued', function( file ) {" + "var id='" + name + "'+file.id;var name=file.name;var text='--等待上传--';addtrFile(id,name,text,0,0);" + name + "addFile(file,'');\r\n" + "});");
    // 上传过程中触发，携带上传进度。
    ...
    sb.append("uploader.on( 'uploadSuccess', function(file,response) {showUploadProgress(100,function(){if(response.success){$('#" + name + "_progress_bar').removeClass('active');updatetdState(file.id,'上传成功');reset_" + name + "_dataTypeInpVal(1);" + "var filepath=response['" + name + "']||response.obj;$('#" + name + "'+file.id+' td:first').append('<input type=\"hidden\" name=\"" + nameWithspchar + "\" value=\"'+filepath+'\" />');" + name + "addFile(file, filepath);");
    // 如果是文件配置swf转换参数则支持文件预览
    if ("file".equals(type) && swfTransform) {
