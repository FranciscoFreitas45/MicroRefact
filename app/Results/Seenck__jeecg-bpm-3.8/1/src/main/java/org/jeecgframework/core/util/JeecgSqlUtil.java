...
public class JeecgSqlUtil {
 ...
 private  String PACKAGE_SQL;

 private  String SUFFIX_SQL;

 private  String SUFFIX_D;

 private  String SUFFIX_X;
...
public String getMethodSqlLogicJar(String methodUrl){
...
public String getFlieTxt(String fileUrl){
...
public String getMethodSqlLogic(String methodUrl){
    String head = methodUrl.substring(0, methodUrl.indexOf(KEY_01));
    String end = methodUrl.substring(methodUrl.indexOf(KEY_02) + KEY_02.length()).replace(KEY_03, "_");
    String sqlurl = head + PACKAGE_SQL + end;
    sqlurl = sqlurl.replace(SUFFIX_D, SUFFIX_X);
...
@Deprecated
public String getMethodSqlLogicOld(String methodUrl){
    // 从Spring中获取Request
...
public String getMethodSql(String methodUrl){
    ...
    } else // [2].发布模式：pub SQL文件只加载一次
    if (ConstantsDefs.MODE_PUBLISH.equals(bundle.getObject("sqlReadMode"))) {
