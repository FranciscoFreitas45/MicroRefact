...
public class DbTableProcess {
 ...
 private  DbTableHandleI dbTableHandle;

public DbTableProcess(Session session) {
    dbTableHandle = DbTableUtil.getTableHandle(session);
}
public void createTable(String xml,CgFormHeadEntity table,Session session){
    // FIXME 考虑JNDI的情况
    // 重新构建一个Configuration
...
public Object getRootMap(CgFormHeadEntity table,String dataType){
    Map map = new HashMap();
...
public List<String> updateTable(CgFormHeadEntity table,Session session){
...
public Map<String,ColumnMeta> getColumnMetadataFormDataBase(String schemaName,String tableName,Session session){
...
public String getUpdateSpecialSql(ColumnMeta cgformcolumnMeta,ColumnMeta datacolumnMeta){
    return dbTableHandle.getSpecialHandle(cgformcolumnMeta, datacolumnMeta);
}


public Configuration getConfig(String resource){
...
public Map<String,String> getNewAndOldFieldName(CgFormHeadEntity table){
...
public void createOrDropTable(CgFormHeadEntity table,Session session){
    Template t;
...
public String judgeIsNumber(String text){
...
public int updateFieldName(String columnName,String id,Session session){
    return session.createSQLQuery("update cgform_field set old_field_name= '" + columnName + "' where id='" + id + "'").executeUpdate();
}


public Map<String,ColumnMeta> getColumnMetadataFormCgForm(CgFormHeadEntity table){
