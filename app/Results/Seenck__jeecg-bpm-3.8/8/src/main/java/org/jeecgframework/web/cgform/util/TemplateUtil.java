...
public String GetListctrl(JSONObject item,Map<String,Object> formData,String action){
    ...
    String btnAdd = "<span class=\"pull-right\"><button id='listAdd' class=\"btn btn-small btn-success listAdd\" type=\"button\" tbname=\"" + name + // 添加按钮
    "\">添加一行</button></span>";
    // 头部模版
    ...
    for (int row = 0; row < rowCount; row++) {
        ...
        for (int i = 0; i < tdCount; i++) {
            ...
            row == 0) {
                ...
                if (sum != "") {
                    isExistSum = true;
                    tdSum += MessageFormat.format("<td>合计：<input class=\"input-small\" type=\"text\" value=\"value{0}\" name=\"{1}[total]\" {2}\">{3}</td>", i, tdname, sum, "");
                } else {
                    tdSum += "<td></td>";
                }
            }
