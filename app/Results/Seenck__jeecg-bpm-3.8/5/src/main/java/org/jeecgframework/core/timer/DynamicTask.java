...
public boolean startTask(TSTimeTaskEntity task){
    try {
...
public boolean endTask(TSTimeTaskEntity task){
    try {
        // update--begin--author:Yandong date:20180525 for:TASK #2719 【bug】定时任务日志包冲突--
        // quartz 2.2
...
public void scheduleJob(TSTimeTaskEntity task){
    // build 要执行的任务
    JobDetail jobDetail = JobBuilder.newJob(MyClassLoader.getClassByScn(task.getClassName())).withIdentity(task.getId()).storeDurably().requestRecovery().build();
    // 根据Cron表达式 build 触发时间对象
    CronScheduleBuilder cronScheduleBuilder = CronScheduleBuilder.cronSchedule(task.getCronExpression());
    // build 任务触发器
    CronTrigger cronTrigger = TriggerBuilder.newTrigger().withIdentity("cron_" + task.getId()).withSchedule(// 标明触发时间
