...
public String exportXls(TSCompanyPositionEntity tSCompanyPosition,HttpServletRequest request,HttpServletResponse response,DataGrid dataGrid,ModelMap modelMap){
    ...
    modelMap.put(NormalExcelConstants.PARAMS, new ExportParams("职务管理列表", "导出人:" + ResourceUtil.getSessionUser().getRealName(), "导出信息"));
    modelMap.put(NormalExcelConstants.DATA_LIST, tSCompanyPositions);
    return NormalExcelConstants.JEECG_EXCEL_VIEW;
}


public void populateTree(List<TSCompanyPositionEntity> list,List<TSUserPositionRelEntity> selectlist,List<Map<String,Object>> dataList){
...
public AjaxJson doBatchDel(String ids,HttpServletRequest request){
    ...
    try {
        for (String id : ids.split(",")) {
            TSCompanyPositionEntity tSCompanyPosition = systemService.getEntity(TSCompanyPositionEntity.class, id);
...
@ResponseBody
public ResponseEntity<?> update(TSCompanyPositionEntity tSCompanyPosition){
...
public AjaxJson saveUserCompanyPosition(HttpServletRequest request){
    ...
    try {
        if (StringUtils.isNotEmpty(departid) && StringUtils.isNotEmpty(userId)) {
            // 查找直接上级公司
...
public List<Map<String,Object>> getTreeData(HttpServletResponse response,HttpServletRequest request){
    ...
    try {
        ...
        String hql = "select up from TSUserPositionRelEntity up,TSCompanyPositionEntity p where  p.companyId = up.companyId " + " and up.companyId = ? and up.userId = ?";
...
public String exportXlsByT(TSCompanyPositionEntity tSCompanyPosition,HttpServletRequest request,HttpServletResponse response,DataGrid dataGrid,ModelMap modelMap){
    modelMap.put(NormalExcelConstants.FILE_NAME, "职务管理");
    modelMap.put(NormalExcelConstants.CLASS, TSCompanyPositionEntity.class);
    modelMap.put(NormalExcelConstants.PARAMS, new ExportParams("职务管理列表", "导出人:" + ResourceUtil.getSessionUser().getRealName(), "导出信息"));
...
public String getCompanyId(String departid){
...
@ResponseBody
public ResponseEntity<?> create(TSCompanyPositionEntity tSCompanyPosition,UriComponentsBuilder uriBuilder){
    // 调用JSR303 Bean Validator进行校验，如果出错返回含400错误码及json格式的错误信息.
    Set<ConstraintViolation<TSCompanyPositionEntity>> failures = validator.validate(tSCompanyPosition);
    if (!failures.isEmpty()) {
        return new ResponseEntity(BeanValidators.extractPropertyAndMessage(failures), HttpStatus.BAD_REQUEST);
