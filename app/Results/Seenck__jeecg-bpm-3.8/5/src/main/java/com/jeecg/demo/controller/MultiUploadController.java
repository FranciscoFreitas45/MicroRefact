...
public String exportXls(MultiUploadEntity multiUpload,HttpServletRequest request,HttpServletResponse response,DataGrid dataGrid,ModelMap modelMap){
    ...
    modelMap.put(NormalExcelConstants.PARAMS, new ExportParams("测试多文件上传列表", "导出人:" + ResourceUtil.getSessionUser().getRealName(), "导出信息"));
...
public AjaxJson doBatchDel(String ids,HttpServletRequest request){
    ...
    try {
        for (String id : ids.split(",")) {
            MultiUploadEntity multiUpload = systemService.getEntity(MultiUploadEntity.class, id);
...
public ResponseEntity<?> update(MultiUploadEntity multiUpload){
...
public String exportXlsByT(MultiUploadEntity multiUpload,HttpServletRequest request,HttpServletResponse response,DataGrid dataGrid,ModelMap modelMap){
    modelMap.put(NormalExcelConstants.FILE_NAME, "测试多文件上传");
    modelMap.put(NormalExcelConstants.CLASS, MultiUploadEntity.class);
    modelMap.put(NormalExcelConstants.PARAMS, new ExportParams("测试多文件上传列表", "导出人:" + ResourceUtil.getSessionUser().getRealName(), "导出信息"));
...
@ResponseBody
public ResponseEntity<?> create(MultiUploadEntity multiUpload,UriComponentsBuilder uriBuilder){
    // 调用JSR303 Bean Validator进行校验，如果出错返回含400错误码及json格式的错误信息.
    Set<ConstraintViolation<MultiUploadEntity>> failures = validator.validate(multiUpload);
    if (!failures.isEmpty()) {
        return new ResponseEntity(BeanValidators.extractPropertyAndMessage(failures), HttpStatus.BAD_REQUEST);
