...
public String exportXls(TSFillRuleEntity tSFillRule,HttpServletRequest request,HttpServletResponse response,DataGrid dataGrid,ModelMap modelMap){
    ...
    modelMap.put(NormalExcelConstants.PARAMS, new ExportParams("填值规则表列表", "导出人:" + ResourceUtil.getSessionUser().getRealName(), "导出信息"));
...
public AjaxJson doBatchDel(String ids,HttpServletRequest request){
    ...
    try {
        for (String id : ids.split(",")) {
            TSFillRuleEntity tSFillRule = systemService.getEntity(TSFillRuleEntity.class, id);
...
@ResponseBody
public ResponseMessage<?> update(TSFillRuleEntity tSFillRule){
...
public String exportXlsByT(TSFillRuleEntity tSFillRule,HttpServletRequest request,HttpServletResponse response,DataGrid dataGrid,ModelMap modelMap){
    modelMap.put(NormalExcelConstants.FILE_NAME, "填值规则表");
    modelMap.put(NormalExcelConstants.CLASS, TSFillRuleEntity.class);
    modelMap.put(NormalExcelConstants.PARAMS, new ExportParams("填值规则表列表", "导出人:" + ResourceUtil.getSessionUser().getRealName(), "导出信息"));
...
@ResponseBody
public ResponseMessage<?> create(TSFillRuleEntity tSFillRule,UriComponentsBuilder uriBuilder){
    // 调用JSR303 Bean Validator进行校验，如果出错返回含400错误码及json格式的错误信息.
    Set<ConstraintViolation<TSFillRuleEntity>> failures = validator.validate(tSFillRule);
    if (!failures.isEmpty()) {
        return Result.error(JSONArray.toJSONString(BeanValidators.extractPropertyAndMessage(failures)));
