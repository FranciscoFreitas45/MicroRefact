...
public boolean connectISMG(){
    ...
    connect.setAuthenticatorSource(MsgUtils.getAuthenticatorSource(MsgConfig.getSpId(), // md5(企业代码+密匙+时间戳)
    MsgConfig.getSpSharedSecret()));
    // 时间戳(MMDDHHMMSS)
    connect.setTimestamp(Integer.parseInt(MsgUtils.getTimestamp()));
    // connect.setTimestamp(1104230904);//时间戳(MMDDHHMMSS)423202240
    // 版本号 高4bit为3，低4位为0
    connect.setVersion((byte) 0x30);
...
public boolean sendLongWapPushMsg(String url,String desc,String cusMsisdn){
    try {
        List<byte[]> dataList = new ArrayList<byte[]>();
        // length 12
...
public boolean sendShortWapPushMsg(String url,String desc,String cusMsisdn){
    try {
        // length 12
...
public boolean sendShortMsg(String msg,String cusMsisdn){
    try {
        int seq = MsgUtils.getSequence();
        MsgSubmit submit = new MsgSubmit();
