...
public AjaxJson doBatchDel(String ids,HttpServletRequest request){
    ...
    try {
        for (String id : ids.split(",")) {
            JformOrderMain2Entity jformOrderMain2 = systemService.getEntity(JformOrderMain2Entity.class, id);
...
public ResponseMessage<?> update(JformOrderMain2Page jformOrderMain2Page){
...
public ResponseMessage<?> create(JformOrderMain2Page jformOrderMain2Page,UriComponentsBuilder uriBuilder){
    // 调用JSR303 Bean Validator进行校验，如果出错返回含400错误码及json格式的错误信息.
    Set<ConstraintViolation<JformOrderMain2Page>> failures = validator.validate(jformOrderMain2Page);
    if (!failures.isEmpty()) {
        return Result.error(JSONArray.toJSONString(BeanValidators.extractPropertyAndMessage(failures)));
