...
public BaseObjectBean updateUserProfile(ProfileVo profileVo,HttpServletRequest request){
    ...
    // 회원 기본 정보 조회
    weUser = commonService.getUserInfo(profileVo.getWeUserIdx());
    // 회원 프로파일 정보 조회
    weProfile = commonService.getUserProfileInfo(profileVo.getWeUserIdx());
    ...
    try {
        ...
        // 사용자 홈피
        weProfile.setWe_user_site(profileVo.getWeUserSite());
        // 수정일
        weProfile.setWe_upd_date(new Date());
        // 파일 정보가 있을 경우 파일정보까지 수정됨.
