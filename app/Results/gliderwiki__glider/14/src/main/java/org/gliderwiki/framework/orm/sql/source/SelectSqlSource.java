...
public class SelectSqlSource extends DynamicSqlBuilder{

public SelectSqlSource(SqlSourceBuilder sqlSourceParser, Class<?> clazz) {
    super(sqlSourceParser);
    staticSql = String.format("SELECT %1$s FROM %2$s ", ParserUtil.join(AnnotationUtil.getColumnNames(clazz), ", "), AnnotationUtil.getTableName(clazz));
}
}