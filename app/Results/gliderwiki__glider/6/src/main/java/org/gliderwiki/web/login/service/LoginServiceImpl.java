...
import org.gliderwiki.DTO.WeSendMail;
...
public int sendMailAuth(WeUser weUser,HttpServletRequest request){
    ...
    try {
        ...
        if (result == 1) {
            ...
            // 사용자 아이디가 이메일임
            weProfile.setWe_user_email(currentUser.getWe_user_id());
            weProfile.setWe_ins_date(new Date());
            // 기본 등급 세팅
            weProfile.setWe_grade(SystemConst.NEW_USER_GRADE);
            ...
            if (result == 1) {
                ...
                String emailMsgTxt = "안녕하세요. " + siteOwner + " 입니다. <br><br>" + "자동 가입 방지를 위한 인증 페이지주소를 보내 드립니다. <br><br>" + "아래의 인증 페이지 링크를 클릭하세요. <br><br>" + "주소 : <a href='" + targetUrl + "' target='_blank'>" + targetUrl + // 내용
                "</a>";
                // 제목
                String emailTitle = siteOwner + " Wiki 가입 인증 안내입니다.";
                try {
...
public BaseObjectBean findUserPassword(String userId,String userNick,HttpServletRequest request){
    ...
    try {
        ...
        if (searchUser != null) {
            ...
            String emailMsgTxt = "안녕하세요. " + siteOwner + " Wiki 입니다. <br><br>" + "요청한 정보는 아래와 같습니다.<br><br>" + "비밀번호 : " + userPass;
            // 제목
            String emailTitle = siteOwner + " Wiki 정보 확인 안내입니다.";
            try {
                SendMailSMTP sender = new SendMailSMTP();
                result = sender.sendSimpleMail(targetID, emailTitle, emailMsgTxt, request);
...
public int updateUserAuth(String code){
    ...
    try {
        ...
        // 기본 등급은 1
        weProfile.setWe_grade(1);
        weProfile.setWe_visit_date(new Date());
        // 최초 로그인 시 1점
        weProfile.setWe_point(1);
        // 프로필 정보 수정
